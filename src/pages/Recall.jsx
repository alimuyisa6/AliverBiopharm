    ...
    <div className="recall-page">
      <div className="section recall-page-section">
        ...
        {dueTotal > 0 && !sessionActive && !showReport && !reviewMode && (
          <div className="recall-due-banner">
            <div className="recall-due-banner-content">
              ...
              <div>
                <h3 className="recall-due-banner-title">{dueTotal} reviews due</h3>
                <p className="recall-due-banner-text">Review these while they're fresh</p>
              </div>
            </div>
            ...
            {feedbackResult && (
              <Card className="recall-feedback-card">
                ...
                <div className="recall-confidence-section">
                  <span className="recall-confidence-label">How well did you recall this?</span>
        ...
        {reviewMode && reviewQuestion && (
          <div>
            <Card className="recall-question-card">
              <div className="recall-review-badge">
                <Icon name="calendar-clock" /> Due Review
              </div>
              <h3 className="recall-question-heading">{reviewQuestion.question_text}</h3>
              <p className="recall-review-answer">
                <strong>Answer:</strong> {reviewQuestion.correct_answer}
              </p>
              {reviewQuestion.explanation && (
                <p className="recall-review-explanation">{reviewQuestion.explanation}</p>
              )}
        ...
        <Modal open={dueQueueOpen} onClose={closeDueQueue} title={`Review Queue (${dueTotal} due)`}>
          <div className="recall-due-modal-list">
            {dueQueue.length === 0 ? (
              <p className="recall-topic-modal-empty">No reviews due. Great job!</p>
            ) : (
              dueQueue.map((item) => (
                <button
                  ...
                  disabled={locked}
                >
                  <span className="recall-due-modal-question">{item.question_text}</span>
                  <span className={`badge ${item.is_weak_concept ? 'badge-warm' : 'badge-primary'}`}>
