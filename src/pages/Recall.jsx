...
export default function BioRecall() {
  ...
  async function loadDueQueue() {
    try {
      ...
      setDueQueue(Array.isArray(result?.items) ? result.items : []);
  ...
  async function handleReviewConfidence(confidence) {
    ...
    try {
      await submitRecallConfidence('review-queue', reviewQuestion.question_id, confidence);

      addToast(`Marked as ${confidence}`, 'success');
  ...
  async function handleSessionConfidence(confidence) {
    if (locked || !currentQuestion || !sessionId) return;

    setSubmittingConfidence(true);

    try {
      await submitRecallConfidence(sessionId, currentQuestion.id, confidence);

      if (confidence === 'again') {
        addToast('Will show again soon', 'info');
      } else {
        addToast(`Scheduled for review`, 'success');
      }

      loadDueQueue();
    } catch {
      addToast('Failed to record confidence', 'error');
    } finally {
      setSubmittingConfidence(false);
    }
  }
  ...
  if (loading && !sessionActive && !showReport && !reviewMode) {
    return (
    ...
    <div className="recall-page">
      <div className="section recall-page-section">
        <nav className="breadcrumb font-mono">
          ...
          <span className="font-maven-pro">Recall Practice</span>
        </nav>

        <div className="recall-header">
          <span className="sec-label font-mono">BioRecall</span>
          ...
          {levelName && (
            <div className="recall-chips-row">
              <span className="chip recall-chip-level font-maven-pro">{levelName}</span>
              {classLabel && <span className="chip recall-chip-class font-maven-pro">{groupLabel}: {classLabel}</span>}
            </div>
          )}
        </div>

        {dueTotal > 0 && !sessionActive && !showReport && !reviewMode && (
          <div className="recall-due-banner">
            <div className="recall-due-banner-content">
              <Icon name="calendar-clock" className="recall-due-banner-icon" />
              <div>
                <h3 className="recall-due-banner-title font-poppins">{dueTotal} reviews due</h3>
                <p className="recall-due-banner-text font-source-sans">Review these while they're fresh</p>
              </div>
            </div>
            <Button variant="warm" size="sm" onClick={openDueQueue}>
              <Icon name="layer-group" /> Review Now
            </Button>
          </div>
        )}

        {!sessionActive && !showReport && !reviewMode && (
          <>
            {sectionLoading ? (
                ...
                <div className="card recall-intro-card">
                  ...
                  <div className="recall-intro-stats">
                    <div>
                      ...
                      <span className="recall-intro-stat-text font-source-sans">{streakDays} Day Streak</span>
                    </div>
                    <div>
                      <Icon name="star" className="recall-intro-stat-icon is-accent" />
                      <span className="recall-intro-stat-text font-source-sans">Level {xpProgress.level} · {xpTotal} XP · {rankTitle}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 recall-cards-grid">
                  <Card>
                    <div className="recall-stat-card-inner">
                      ...
                      <p className="recall-stat-card-sub font-source-sans">Level {xpProgress.level} · {rankTitle}</p>
                      <ProgressBar value={xpProgress.xpIntoLevel} max={100} variant="gradient" />
                      <p className="recall-stat-card-footer font-mono">{xpProgress.xpIntoLevel} / 100 XP to next level</p>
                    </div>
                  </Card>
                  ...
                  <Card>
                    <div className="recall-stat-card-inner">
                      ...
                      <p className="recall-stat-card-footer font-mono">{accuracy}% across all sessions</p>
                    </div>
                  </Card>
                    ...
                    <div className="recall-stat-card-inner">
                      ...
                      <div className="recall-badge-row">
                        {achievementsList.slice(0, 6).map((achievement) => (
                          <span key={achievement.id} className="badge badge-accent font-comfortaa" title={achievement.name}>
                  ...
                  {leaderboard.length === 0 ? (
                    ...
                    leaderboard.map((entry, index) => (
                      <div key={entry.user_id || index} className="card recall-leaderboard-row">
                        ...
                        <span className="recall-entry-name font-poppins">{entry.display_name || 'Anonymous Learner'}</span>
          ...
          <div>
            ...
            <Card className="recall-question-card">
              {currentQuestion.question_type === 'cloze' && currentQuestion.cloze_template ? (
                ...
                <>
                  <h3 className="recall-question-heading font-poppins">{currentQuestion.question_text}</h3>
              ...
              {analyzing && (
                <div className="recall-analyzing-row">
                  <Spinner size="sm" />
                  <span className="recall-analyzing-label font-maven-pro">Checking</span>
                </div>
              )}
            ...
            {feedbackResult && (
              <Card className="recall-feedback-card">
                <div className="recall-feedback-header">
                  ...
                  />
                  <div>
                    <h4 className="font-poppins">{feedbackResult.strength === 'excellent' ? 'Perfect' : feedbackResult.strength === 'strong' ? 'Close' : 'Needs Review'}</h4>
                    <p className="recall-feedback-xp font-mono">+{feedbackResult.xp_earned} XP</p>
                  </div>
                </div>
                <p className="font-source-sans"><strong>Correct answer:</strong> {feedbackResult.correct_answer}</p>
                {feedbackResult.note && <p className="recall-feedback-note font-open-sans">{feedbackResult.note}</p>}
                {feedbackResult.explanation && <p className="recall-feedback-explanation font-open-sans">{feedbackResult.explanation}</p>}

                <div className="recall-confidence-section">
                  <span className="recall-confidence-label font-poppins">How well did you recall this?</span>
                  <div className="recall-confidence-buttons">
                    {CONFIDENCE_LEVELS.map((item) => (
                      <Button
                        ...
                        size="sm"
                        loading={submittingConfidence}
                        onClick={() => handleSessionConfidence(item.key)}
                      >
          ...
          <div>
            <Card className="recall-question-card">
              <div className="recall-review-badge font-comfortaa">
                <Icon name="calendar-clock" /> Due Review
              </div>
              <h3 className="recall-question-heading font-poppins">{reviewQuestion.question_text}</h3>
              <p className="recall-review-answer font-source-sans">
                <strong>Answer:</strong> {reviewQuestion.correct_answer}
              </p>
              {reviewQuestion.explanation && (
                <p className="recall-review-explanation font-open-sans">{reviewQuestion.explanation}</p>
              )}
                ...
                <div className="recall-confidence-buttons">
                  {CONFIDENCE_LEVELS.map((item) => (
                    <Button
                      ...
                      size="sm"
                      loading={submittingConfidence}
                      onClick={() => handleReviewConfidence(item.key)}
                    >
                      <Icon name={item.icon} /> {item.label}
                    </Button>
                  ))}
              ...
              <div className="recall-answer-actions">
                <Button variant="secondary" onClick={() => { setReviewMode(false); setReviewQuestion(null); }}>
        ...
        {showReport && sessionReport && (
          <Card className="recall-report-card">
            ...
            <h2 className="font-fraunces">Session Complete</h2>

            <div className="recall-report-stats">
              <div>
                <div className="recall-report-value is-success font-poppins">{sessionReport.excellent || 0}</div>
                <div className="recall-report-label font-source-sans">Excellent</div>
              </div>
              <div>
                <div className="recall-report-value is-primary font-poppins">{sessionReport.strong || 0}</div>
                <div className="recall-report-label font-source-sans">Strong</div>
              </div>
              <div>
                <div className="recall-report-value is-warm font-poppins">{sessionReport.developing || 0}</div>
                <div className="recall-report-label font-source-sans">Needs Review</div>
              </div>
            </div>

            <p className="font-source-sans">Mastery Score: {sessionReport.mastery_score || 0}%</p>
            <p className="recall-report-time font-mono">Total time: {sessionReport.total_time_formatted} · Avg: {sessionReport.avg_time_formatted} per question</p>

            {newlyAwarded.length > 0 && (
              <div className="recall-report-achievements">
                <h4 className="font-poppins">New Achievements</h4>
                <div className="recall-badge-row">
                  {newlyAwarded.map((achievement) => (
                    <span key={achievement.id} className="badge badge-accent font-comfortaa" title={achievement.description || achievement.name}>
        ...
        <Modal open={topicModalOpen} onClose={closeTopicModal} title={`Select a ${unitLabel}${levelName ? ` in ${levelName}` : ''}${classLabel ? ` – ${classLabel}` : ''}`}>
          <div className="recall-topic-modal-list">
            {topicList.length === 0 && (
              <p className="recall-topic-modal-empty font-open-sans">No {unitLabel.toLowerCase()}s available for your level.</p>
            )}
            ...
            {topicList.map((topic) => (
              ...
              >
                ...
                <span className="recall-topic-modal-count font-mono">{topic.question_count} questions</span>
              </button>
            ))}
          </div>
        </Modal>

        <Modal open={dueQueueOpen} onClose={closeDueQueue} title={`Review Queue (${dueTotal} due)`}>
          <div className="recall-due-modal-list">
            {dueQueue.length === 0 ? (
              <p className="recall-topic-modal-empty font-open-sans">No reviews due. Great job!</p>
            ) : (
              dueQueue.map((item) => (
                <button
                  ...
                  disabled={locked}
                >
                  <span className="recall-due-modal-question font-source-sans">{item.question_text}</span>
                  <span className={`badge font-comfortaa ${item.is_weak_concept ? 'badge-warm' : 'badge-primary'}`}>
                    {item.is_weak_concept ? 'Weak' : `${item.repetitions} reps`}
                  </span>
                </button>
              ))
            )}
