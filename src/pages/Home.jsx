...
export default function Home() {
  ...
  const [weeklyChallengeStatus, setWeeklyChallengeStatus] = useState(null);
  ...
  async function fetchAllData() {
    try {
      ...
      if (siteSections?.weekly_challenge && user) {
        checkWeeklyChallengeStatus(siteSections.weekly_challenge.week_start);
      }
      if (siteSections?.site_config?.logo_url) {
        document.querySelectorAll('.logo-link img').forEach(img => img?.remove());
      }
  ...
  async function fetchFlashcardProgress() {
    try {
      const data = await getFlashcards(); // progress endpoint not available, simulate
      const progress = {};
  ...
  async function checkWeeklyChallengeStatus(weekStart) {
    try {
      const status = await submitWeeklyChallenge(weekStart, null);
      setWeeklyChallengeStatus(status);
    } catch (err) { console.error(err); }
  }
  ...
  return (
    <div className="homepage">
      ...
      <section id="team" className="section reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.team_title || 'Meet Our Expert Faculty'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.team_subtitle || 'Learn from distinguished pharmacologists and molecular biologists.'}</p>
      ...
      <section id="testimonials" className="section alt-bg reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.testimonials_title || 'Learner Success Stories'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.testimonials_subtitle || 'Hear from students who transformed their understanding.'}</p>
      ...
      <section id="courses" className="section reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.courses_title || 'Learning Resources'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.courses_subtitle || 'Browse our comprehensive library.'}</p>
      ...
      <section id="flashcards" className="section reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.flashcards_title || 'Flashcard Decks'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.flashcards_subtitle || 'Active recall is the most effective way to retain complex scientific concepts.'}</p>
              ...
              <div className="flashcard-grid">
                {cards.slice(0, flashcardCurrentDeck === deckName ? cards.length : 3).map((card, idx) => (
                  <div key={card.id} className="flashcard-deck" data-id={card.id}>
                    <div className={`flashcard-face ${flashcardCurrentDeck === deckName && flashcardCurrentIndex === idx ? 'active-card' : ''}`} onClick={() => { setFlashcardCurrentDeck(deckName); setFlashcardCurrentIndex(idx); document.querySelector(`.flashcard-face[data-id="${card.id}"]`)?.classList.toggle('flipped'); }}>
                      <div className="front">
            ...
            <div className="pdf-content-wrapper">
              ...
              <div className="pdf-subtopics-column">
                ...
                <div className="pdf-subtopics-list">
                  {Array.from(new Set(pdfs.map(p => p.topic || 'General'))).map(topic => (
                    <div key={topic} className={`pdf-subtopic-item ${pdfSelectedTopic === topic ? 'active' : ''}`} onClick={() => { setPdfSelectedTopic(topic); const filtered = pdfs.filter(p => (p.topic || 'General') === topic); document.querySelector('.pdf-cards-area').innerHTML = renderFilteredPdfsHtml(filtered); }}>
                        ...
                        {groupedNotes[notesSelectedLevel][notesSelectedTopic].map(item => (
                          <div key={item.subtopic_id} className="subtopic-card" style={{ background: 'var(--clr-navy-card)', borderRadius: '20px', border: '1px solid var(--clr-border-glow)', overflow: 'hidden' }}>
                            <div style={{ padding: '20px' }}>
                              ...
                              <button className="read-note-btn" onClick={() => loadNoteContent(item.subtopic_id, notesSelectedLevel, notesSelectedTopic, item.subtopic_name)} style={{ width: '100%', background: 'transparent', border: '2px solid var(--clr-cyan)', color: 'var(--clr-cyan)', padding: '12px 20px', borderRadius: '50px', fontWeight: 700, cursor: 'pointer' }}><i className="fa-solid fa-book-open-reader"></i> Read Full Note</button>
                            </div>
                          </div>
                        ))}
                      </div>
      ...
      <section id="pricing" className="section alt-bg reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.pricing_title || 'Membership Plans'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.pricing_subtitle || 'Choose the plan that fits your learning journey.'}</p>
      ...
      <section id="blog" className="section alt-bg reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.blog_title || 'Latest Articles & Insights'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.blog_subtitle || 'Stay informed with the latest developments.'}</p>
        <div className="grid-3">
      ...
      <section id="faq" className="section reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.faq_title || 'Frequently Asked Questions'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.faq_subtitle || 'Quick answers to common questions.'}</p>
      ...
      <section id="contact" className="section alt-bg reveal">
        ...
        <h2 className="section-title">{sections?.section_headings?.contact_title || 'Get in Touch'}</h2>
        <p className="section-subtitle">{sections?.section_headings?.contact_subtitle || 'Our support team typically responds within 24 hours.'}</p>
...
function renderFilteredPdfsHtml(pdfs) {
  if (!pdfs.length) return '<div class="pdf-loading">No PDFs in this topic.</div>';
  let html = '<div class="pdf-cards-grid">';
  pdfs.forEach(pdf => {
    html += `
      <div class="pdf-card" data-pdf-id="${pdf.id}" data-pdf-title="${escapeHtml(pdf.title)}" data-pdf-url="${escapeHtml(pdf.file_url)}">
