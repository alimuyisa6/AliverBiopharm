...
function Quiz() {
  ...
  const saveQuizStateToBackend = async () => {
    if (!user) return;
    try {
      const response = await fetch('/api/interactions?path=save_quiz_state', {
        method: 'POST',
  ...
  const trackEvent = async (eventName, eventData = {}) => {
    if (!user) return;
    try {
      await fetch('/api/interactions?path=track_event', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
    ...
    const load = async () => {
      try {
        ...
        if (user) {
          ...
          try {
            const res = await fetch('/api/interactions?path=get_quiz_state', { credentials: 'include' });
            if (res.ok) {
              const data = await res.json();
              if (data.state && !currentTopic && !quizQuestions.length) {
