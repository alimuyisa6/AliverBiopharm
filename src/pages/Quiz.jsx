...
export default function Quiz() {
  const { user, profile } = useAuth();          // ← now using profile for active_group_id
  const { isReady } = useRequireOnboarding();
  const access = useContentAccess();
  const { level, class_name, displayName } = useLevelFilter();
  const addToast = useToast();

  const activeGroupId = profile?.active_group_id;   // ← authoritative group id
  ...
  // ---------- Reset all quiz state when active group changes ----------
  useEffect(() => {
  ...
  // ---------- Fetch topics scoped to active group ----------
  useEffect(() => {
    if (!isReady || !access.canAccess || access.isPending) return;
    listQuizTopics(activeGroupId)   // ← pass groupId as cache breaker
  ...
  const submitBlock = async () => {
    ...
    try {
      ...
      const topicsRes = await listQuizTopics(activeGroupId);   // refetch with group
