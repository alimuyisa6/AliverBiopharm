...
export default function Quiz() {
  ...
  async function openTopicBlocks(topic, total) {
    // Ensure total is a number (convert if needed)
    const totalBlocksNum = parseInt(total, 10) || 0;
    setCurrentTopic(topic);
    setTotalBlocks(totalBlocksNum);
    setQuizQuestions([]);
    setResultData(null);
    // Ensure loading is false so block navigation appears immediately
    setLoading(false);
  }
  ...
  // Loading indicator – only when fetching initial topics OR during quiz load
    ...
    <div className="quiz-page">
      ...
      <main className="section">
        ...
        {!currentTopic ? (
          ...
          // BLOCK NAVIGATION – this is what should appear after clicking a topic
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            <h2>{currentTopic}</h2>
            <div className="block-nav">
              {totalBlocks > 0 ? Array.from({ length: totalBlocks }).map((_, i) => {
