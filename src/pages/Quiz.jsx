...
function Quiz() {
  ...
  const getFirstUnansweredIndex = (answers) => {
    return answers.findIndex(a => a === null);
  };

  const canNavigateTo = (targetIndex, answers) => {
    if (answers[targetIndex] !== null) return true;
    const firstUnanswered = getFirstUnansweredIndex(answers);
    return targetIndex === firstUnanswered;
  };
  ...
  const handleResume = () => {
    ...
    setCurrentTopic(state.topic || (state.questions?.[0]?.topic || 'Unknown'));
    ...
    const targetIndex = state.index !== undefined ? state.index : 0;
    setCurrentIndex(targetIndex);
    setQuizStartTime(state.startTime || new Date());
    setResumeData(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (saved && !currentTopic && !quizQuestions.length && !resumeData) {
    ...
    function handleKey(e) {
      ...
      if (e.key === 'ArrowRight') {
        ...
        if (firstUnanswered !== -1 && firstUnanswered !== currentIndex) {
          ...
          if (canNavigateTo(nextIdx, userAnswers)) setCurrentIndex(nextIdx);
        }
      }
      if (e.key === 'ArrowLeft') {
        ...
        {!currentTopic ? (
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              ...
              {quizQuestions.map((_, idx) => {
                ...
                return (
                  <button
                    key={idx}
                    onClick={() => {
                      if (canNavigateTo(idx, userAnswers)) setCurrentIndex(idx);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1rem' }}>
              ...
              {userAnswers[currentIndex] !== null && (firstUnanswered !== -1 && firstUnanswered !== currentIndex ? (
          ...
          <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--clr-navy-card)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', maxWidth: '420px', width: '90%' }}>
              ...
              <div style={{ fontSize: '0.9rem', lineHeight: '1.5' }}>
                <p><strong>Topic:</strong> {resumeData.topic || (resumeData.questions?.[0]?.topic || 'Unknown')}</p>
