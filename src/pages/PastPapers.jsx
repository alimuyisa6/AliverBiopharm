...
export default function PastPapers() {
  ...
  const getEmptyStateImage = useCallback((key) => {
  ...
  useEffect(() => {
    ...
    const loadFilterOptions = async () => {
      ...
      try {
        const result = await getPastPaperFilterOptions();

        if (!cancelled) {
          setFilterOptions({
            subjects: Array.isArray(result?.subjects) ? result.subjects : [],
            years: Array.isArray(result?.years) ? result.years : [],
            exam_boards: Array.isArray(result?.exam_boards)
              ? result.exam_boards
              : [],
            paper_types: Array.isArray(result?.paper_types)
              ? result.paper_types
              : []
          });
        }
        ...
        console.error(
          '[PAST_PAPER_FILTER_OPTIONS_ERROR]',
          error
        );
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const loadUserInteractions = async () => {
      try {
        ...
        setBookmarkedIds(
          new Set(
            (bookmarked?.papers || []).map((paper) => paper.id)
          )
        );

        setDownloadedIds(
          new Set(
            (downloaded?.papers || []).map((paper) => paper.id)
          )
        );
      } catch (error) {
        if (!cancelled) {
          console.error(
            '[PAST_PAPER_USER_INTERACTIONS_ERROR]',
            error
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const loadPapers = async () => {
      ...
      try {
        ...
        setPapers(Array.isArray(result?.papers) ? result.papers : []);
        setTotalPages(
          Math.max(1, Number(result?.total_pages) || 1)
        );
  ...
  const loadReviews = async (paperId) => {
    ...
    try {
      const result = await getPaperReviews(
        paperId,
        1,
        20
      );
  ...
  const handleDownload = async (paper) => {
    ...
    if (paper.locked || paper.is_premium) {
      addToast(
        'This paper requires premium access',
        'warning'
      );
      return;
    }
    ...
    try {
      const result =
        await getPastPaperDownloadUrl(paper.id);
      ...
      console.error(
        '[PAST_PAPER_DOWNLOAD_ERROR]',
        error
      );

      if (error?.status === 403) {
        addToast(
          'Premium access required for this paper',
          'warning'
        );
      } else {
        addToast(
  ...
  const handleBookmark = async (paperId) => {
    ...
    try {
      const result =
        await togglePaperBookmark(paperId);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      console.error(
        '[PAST_PAPER_REVIEW_SUBMIT_ERROR]',
        error
      );
  ...
  const handleDeleteReview = async () => {
    ...
    try {
      ...
      console.error(
        '[PAST_PAPER_REVIEW_DELETE_ERROR]',
        error
      );
  ...
  const handleDeletePreset = async (presetId) => {
    try {
      ...
      console.error(
        '[PAST_PAPER_PRESET_DELETE_ERROR]',
        error
      );
      ...
      >
        ...
        {showFilters && activeTab === 'all' && (
          <div className="past-papers-filters">
            ...
            <Select
              label="Year"
              options={filterOptions.years.map(
                (year) => ({
                  value: String(year),
                  label: String(year)
                })
