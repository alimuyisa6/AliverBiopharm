...
export default function PastPapers() {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      await ratePaper(selectedPaper.id, reviewRating, reviewComment.trim() || null);
  ...
  return (
    <div className="past-papers-page">
      ...
      <div className="section" style={{ paddingTop: 'var(--space-6)' }}>
        ...
        {papersLoading ? (
          ...
          <div className="past-papers-grid">
            {papers.map((paper) => (
              <div key={paper.id} className={`card paper-card${paper.is_premium ? ' paper-card-premium' : ''}`} onClick={() => handlePaperOpen(paper)}>
      ...
      {selectedPaper && (
        <div className="paper-modal-overlay" onClick={handleCloseModal}>
          <div className="paper-modal" onClick={(event) => event.stopPropagation()}>
            <div className="paper-modal-header">
              <div>
                <h2 className="paper-modal-title">{selectedPaper.title}</h2>
                <p className="paper-modal-subject">{selectedPaper.subject}</p>
              </div>
              <button className="paper-modal-close" onClick={handleCloseModal}>
                <Icon name="xmark" />
              </button>
            </div>
            ...
            <div className="paper-modal-body">
              <div className="paper-modal-meta">
                {selectedPaper.level && <span className="chip">{selectedPaper.level}</span>}
                {selectedPaper.year && <span className="chip chip-accent">{selectedPaper.year}</span>}
                {selectedPaper.paper_type && <span className="chip chip-primary">{selectedPaper.paper_type}</span>}
                {selectedPaper.exam_board && <span className="chip">{selectedPaper.exam_board}</span>}
                {selectedPaper.class_name && <span className="chip">{selectedPaper.class_name}</span>}
              </div>
              ...
              <div className="paper-review-form">
                <span className="paper-review-label">Rate this paper</span>
                <div className="paper-rating-stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      className={`paper-star-btn${star <= reviewRating ? ' active' : ''}`}
                ...
                <Button size="sm" variant="primary" loading={submittingReview} onClick={handleSubmitReview}>
              ...
              {reviewsLoading ? (
                ...
                <p className="paper-reviews-empty">No reviews yet. Be the first to review.</p>
              ) : (
                <div className="paper-reviews-list">
                  {reviews.map((review) => (
                    <div key={review.id} className="paper-review-item">
                      <div className="paper-review-header">
                        <div className="paper-review-author">
                          ...
                          <div>
                            <span className="paper-review-name">{review.display_name || 'Anonymous'}</span>
                            <div className="paper-review-stars">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <span
                                  key={star}
                                  style={{ color: star <= review.rating ? 'var(--warm)' : 'var(--border-default)' }}
                                >
                                  <Icon name="star" />
                                </span>
                              ))}
                        ...
                        <div className="paper-review-meta">
                          ...
                          {review.user_id === user?.id && (
                            <button className="paper-review-delete" onClick={handleDeleteReview}>
                              <Icon name="trash" /> Delete
                            </button>
                          )}
                        </div>
                      </div>
                      {review.comment && <p className="paper-review-comment">{review.comment}</p>}
                    </div>
                  ))}
                </div>
              )}
