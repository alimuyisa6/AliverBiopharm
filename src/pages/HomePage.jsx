...
window.speakText = function (text, btn) {
  ...
  if (!cleanText) { if (btn) btn.classList.remove('playing'); return; }
  ...
  const setVoice = () => {
    ...
    const preferredVoice = voices.find(voice => voice.lang === 'en-US' && (voice.name.includes('Google') || voice.name.includes('Samantha') || voice.name.includes('Female')));
...
function HomePage() {
  ...
  function renderResourcesHTML(container, groups) {
    container.innerHTML = Object.entries(groups).map(([name, items]) => {
      return `<div style="margin-bottom:3rem;"><h2 style="font-family:'Poppins',sans-serif;font-size:1.6rem;color:var(--clr-cyan);margin-bottom:1.25rem;padding-left:1rem;border-left:4px solid var(--clr-magenta);">${esc(name)}</h2><div class="resources-grid">` +
        items.map(item => {
    ...
    if (sections.hero?.slides?.length) {
      ...
      if (heroSection) {
        heroSection.innerHTML = sections.hero.slides.map((s, i) =>
          `<div class="carousel-slide${i === 0 ? ' active' : ''}" style="background-image:url('${esc(s.background_image)}');">
            <div class="slide-overlay">
              <h1 class="font-display font-black text-4xl md:text-6xl mb-4 hero-title">${esc(s.title)}</h1>
              <p class="text-lg md:text-xl max-w-2xl mb-8 hero-subtitle">${esc(s.subtitle)}</p>
              <a href="${esc(s.cta_link)}" class="btn-primary"><i class="${esc(s.icon || 'fa-solid fa-arrow-right')}" aria-hidden="true"></i> ${esc(s.cta_text)}</a>
            </div>
          </div>`
        ).join('') +
          '<button class="carousel-arrow carousel-prev" aria-label="Previous slide"><i class="fa-solid fa-chevron-left" aria-hidden="true"></i></button>' +
          '<button class="carousel-arrow carousel-next" aria-label="Next slide"><i class="fa-solid fa-chevron-right" aria-hidden="true"></i></button>' +
    ...
    if (sections.team?.members?.length) {
      ...
      teamGrid.innerHTML = sections.team.members.map(p => `
        <div class="team-card" style="min-width:280px;max-width:320px;">
          <div class="team-avatar">${p.avatar_url ? `<img src="${esc(p.avatar_url)}" alt="${esc(p.name)}">` : '<i class="fa-solid fa-user-tie"></i>'}</div>
          <h3>${esc(p.name)}</h3>
          <div class="team-title">${esc(p.title || 'Faculty Member')}</div>
          <p>${esc(p.bio || '')}</p>
          <div class="team-social">${p.linkedin ? `<a href="${esc(p.linkedin)}" target="_blank" aria-label="LinkedIn"><i class="fa-brands fa-linkedin-in"></i></a>` : ''} ${p.twitter ? `<a href="${esc(p.twitter)}" target="_blank" aria-label="Twitter"><i class="fa-brands fa-x-twitter"></i></a>` : ''}</div>
        </div>`).join('');
    ...
    if (sections.pricing?.plans?.length) {
      document.getElementById('pricing-grid').innerHTML = sections.pricing.plans.map(pl => `
        <div class="card pricing-card${pl.featured ? ' featured' : ''}">
          <h3>${esc(pl.name)}</h3>
          <p style="font-size:0.84rem;color:var(--clr-text-dim);">${esc(pl.description || '')}</p>
          <div class="price my-3">${esc(pl.price)}<span style="font-size:1rem;color:var(--clr-text-dim);">${esc(pl.period || '')}</span></div>
          <ul class="pricing-features">${(pl.features || []).map(f => `<li><i class="fa-solid fa-check"></i> ${esc(f)}</li>`).join('')}</ul>
          <button class="btn-primary mt-4" style="width:auto; display:inline-flex; margin-left:auto; margin-right:auto;">${esc(pl.cta_text || 'Subscribe')}</button>
        </div>`).join('');
    }
    if (sections.blog?.posts?.length) {
      document.getElementById('blog-grid').innerHTML = sections.blog.posts.map(p => `
        <article class="card">
          ${p.image_url ? `<img src="${esc(p.image_url)}" alt="${esc(p.title)}" style="width:100%;height:200px;object-fit:cover;border-radius:var(--radius-md);margin-bottom:1rem;">` : ''}
          <div class="flex gap-4 text-xs" style="color:var(--clr-text-muted);margin-bottom:0.5rem;"><span><i class="fa-regular fa-calendar" aria-hidden="true"></i> ${esc(p.date || '')}</span><span><i class="fa-regular fa-user" aria-hidden="true"></i> ${esc(p.author || '')}</span></div>
          <h3 style="font-family:'Poppins',sans-serif;font-weight:700;font-size:1.15rem;color:var(--clr-white);">${esc(p.title)}</h3>
          <p style="font-size:0.84rem;line-height:1.7;color:var(--clr-text-dim);">${esc(p.excerpt || '')}</p>
          <a href="#" style="color:var(--clr-magenta);font-weight:600;font-size:0.875rem;">Read Article <i class="fa-solid fa-arrow-right" aria-hidden="true"></i></a>
        </article>`).join('');
    }
    if (sections.faq?.items?.length) {
      document.getElementById('faq-list').innerHTML = sections.faq.items.map(i => `
        <div class="faq-item"><button class="faq-question" onclick="this.parentElement.classList.toggle('active')"><span>${esc(i.question)}</span><span style="color:var(--clr-cyan);">+</span></button><div class="faq-answer" role="region"><p>${esc(i.answer)}</p></div></div>`).join('');
    }
    if (sections.contact?.info) {
      const contactInfoCard = document.getElementById('contact-info-card');
      if (contactInfoCard) {
        contactInfoCard.innerHTML += sections.contact.info.map(c => `
          <div style="display:flex;align-items:center;gap:1rem;padding:0.8rem 0;border-bottom:1px solid var(--clr-border-glow);">
            <div class="contact-icon"><i class="${esc(c.icon)}" aria-hidden="true"></i></div>
            <div><div style="font-size:0.7rem;color:var(--clr-text-muted);">${esc(c.label)}</div><a href="${esc(c.href)}" style="color:var(--clr-white);text-decoration:none;">${esc(c.value)}</a></div>
          </div>`).join('');
      }
    }
    if (sections.footer) {
      ...
      if (footerGrid) {
        footerGrid.innerHTML = (sections.footer.columns || []).map(c => `
  ...
  async function initQuizCard(weeklyChallenge) {
    ...
    async function render() {
      ...
      if (status.already_answered) {
        ...
        quizHTML = '<div style="text-align:center;margin-top:0.5rem;">' + weeklyChallenge.options.map((opt, i) =>
          `<button class="quiz-option-btn" data-idx="${i}">${String.fromCharCode(65 + i)}) ${esc(opt)}</button>`
        ).join('') + `<p style="color:var(--clr-text-muted);font-size:0.8rem;margin-top:0.5rem;">Submissions close in ${countdownStr}</p></div>`;
      }
      const existingCard = container.querySelector('.weekly-challenge-card');
      if (existingCard) {
        ...
        if (!status.already_answered) {
          existingCard.querySelectorAll('.quiz-option-btn').forEach(btn => {
            btn.addEventListener('click', async function (e) {
              ...
              try { await apiCall('submit_weekly_challenge', { week_start: weeklyChallenge.week_start, selected_option: idx }); } catch (e) {}
              setTimeout(() => {
    ...
    container.innerHTML = Object.entries(decks).map(([category, cards]) => {
      ...
      return `<div class="flashcard-category" data-category="${esc(category)}">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:8px;">
          ...
          <div style="display:flex;gap:8px;align-items:center;"><div style="width:100px;"><div class="progress-bar"><div class="progress-fill" style="width:${progressPercent}%;"></div></div><small style="color:var(--clr-text-muted);font-size:0.7rem;">${progress.reviewed}/${cards.length} reviewed</small></div>
          <button class="btn-download" onclick="focusDeck('${escapedCat}')" style="white-space:nowrap;"><i class="fa-solid fa-play"></i> Study</button></div>
        </div>
    ...
    return `<div class="flashcard-deck" data-id="${card.id}" data-known="${isKnown}" data-deck="${esc(deckName)}" data-index="${cardIndex}">
      <div class="flashcard-face${activeClass}" onclick="handleCardClick(this, '${card.id}', '${escapedDeckName}', ${cardIndex})">
        <div class="front${hasImage ? '' : ' no-image'}">
          ...
          <div class="flashcard-question"><strong>${esc(card.front_text || 'No question')}</strong>
          ${card.audio_url ? ` <button class="audio-btn" onclick="event.stopPropagation();speakText('${esc(card.front_text || card.back_text || 'No text').replace(/'/g, "\\'")}', this)" title="Pronounce term"><i class="fa-solid fa-volume-high"></i></button>` : ''}
          </div>
        </div>
      ...
      <div style="display:flex;gap:4px;align-items:center;"><button class="bookmark-btn${isKnown ? ' bookmarked' : ''}" onclick="toggleBookmark('${card.id}', this)" title="Bookmark"><i class="fa-solid fa-bookmark"></i></button>
      <button class="btn-download" onclick="toggleKnown('${card.id}', this)">${isKnown ? 'Known' : 'Mark Known'}</button></div></div>
      <div class="interaction-section"><div class="like-comment-bar"><button class="like-btn" onclick="toggleLike('${card.id}', this)"><i class="fa-regular fa-heart"></i><span>0</span></button>
      <button class="comment-toggle-btn" onclick="toggleCommentForm('${card.id}', this)"><i class="fa-regular fa-comment"></i><span>0</span></button></div>
    ...
    try {
      ...
      if (deck && interactions) {
        ...
        if (commentsContainer && interactions.comments) {
          commentsContainer.innerHTML = interactions.comments.map(c =>
            `<div class="comment-item"><strong>${esc(c.user_name || 'User')}:</strong> ${esc(c.comment)}</div>`).join('');
        }
      }
    } catch (e) {}
  }
  ...
  window.rateFlashcard = async function (difficulty) {
    ...
    if (!isAuthenticated) { alert('Sign in to rate flashcards.'); return; }
    try {
  ...
  window.checkQuizAnswer = async function (cardId, inputElement) {
    if (!isAuthenticated) { alert('Sign in to check answers.'); return; }
  ...
  window.toggleBookmark = async function (cardId, btn) {
    if (!isAuthenticated) { alert('Sign in to bookmark cards.'); return; }
    try {
  ...
  window.toggleLike = async function (resourceId, btn) {
    if (!isAuthenticated) { alert('Sign in to like.'); return; }
    try {
  ...
  async function loadComments(cardId) {
    try {
      ...
      if (container && interactions?.comments) {
        container.innerHTML = interactions.comments.map(c =>
          `<div class="comment-item"><strong>${esc(c.user_name)}:</strong> ${esc(c.comment)}<small style="color:var(--clr-text-muted);">${new Date(c.created_at).toLocaleDateString()}</small></div>`).join('');
      }
    } catch (e) {}
  }

  window.toggleKnown = async function (id, btn) {
    if (!isAuthenticated) { alert('Sign in to track progress.'); return; }
    try {
  ...
  async function loadPdfsByLevel(level) {
    ...
    try {
      ...
      for (const [topic, pdfs] of Object.entries(groupedByTopic)) {
        ...
        pdfs.forEach(pdf => {
          cardsHtml += `<div class="pdf-card" data-pdf-id="${pdf.id}" data-pdf-title="${esc(pdf.title)}" data-pdf-author="${esc(pdf.author || 'Unknown')}" data-pdf-url="${esc(pdf.file_url)}" data-pdf-topic="${esc(pdf.topic)}">
            <div class="pdf-card-icon"><i class="fa-solid fa-file-pdf"></i></div>
            <div class="pdf-card-title">${esc(pdf.title.length > 45 ? pdf.title.substring(0, 42) + '...' : pdf.title)}</div>
            <div class="pdf-card-author">${esc(pdf.author || 'Unknown')}</div>
          </div>`;
        });
        cardsHtml += `</div></div>`;
      }
  ...
  function renderTopicsColumn(topics) {
    ...
    topicsList.innerHTML = topics.map(topic => `<div class="pdf-subtopic-item" data-topic="${esc(topic)}">
      <div class="pdf-subtopic-title">${esc(topic)}</div>
      <div class="pdf-subtopic-author">${topicCounts[topic] || 0} resource${topicCounts[topic] !== 1 ? 's' : ''}</div>
    </div>`).join('');
  ...
  function filterPdfsByTopic(topic) {
    ...
    filteredPdfs.forEach(pdf => {
      cardsHtml += `<div class="pdf-card" data-pdf-id="${pdf.id}" data-pdf-title="${esc(pdf.title)}" data-pdf-author="${esc(pdf.author || 'Unknown')}" data-pdf-url="${esc(pdf.file_url)}">
        <div class="pdf-card-icon"><i class="fa-solid fa-file-pdf"></i></div>
        <div class="pdf-card-title">${esc(pdf.title.length > 45 ? pdf.title.substring(0, 42) + '...' : pdf.title)}</div>
        <div class="pdf-card-author">${esc(pdf.author || 'Unknown')}</div>
      </div>`;
  ...
  async function handlePdfCardClick(pdfId, pdfTitle, pdfUrl, pdfAuthor) {
    if (!isAuthenticated) { alert('Please sign in to access PDF resources.'); return; }
  ...
  function renderLevelButtons() {
    ...
    container.querySelectorAll('.level-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        window.notesCurrentLevel = level; window.notesCurrentTopic = null; window.notesCurrentSubtopic = null;
        renderLevelButtons();
  ...
  function renderTopicsForLevel(level) {
    ...
    container.querySelectorAll('.topic-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        window.notesCurrentTopic = topic; window.notesCurrentSubtopic = null;
  ...
  function renderSubtopicsForTopic(level, topic) {
    ...
    subtopics.forEach(sub => {
      html += `<div class="subtopic-card" data-subtopic-id="${sub.id}" data-subtopic-name="${esc(sub.name)}" data-level="${level}" style="background:var(--clr-navy-card); border-radius:20px; border:1px solid var(--clr-border-glow); overflow:hidden; transition:all 0.3s ease;">
        <div style="padding:20px;"><span class="topic-badge" style="background:rgba(184,135,58,0.12); color:var(--clr-magenta); padding:4px 14px; border-radius:30px; font-size:0.7rem; font-weight:700; text-transform:uppercase; margin-bottom:16px; display:inline-block;">${esc(topic)}</span>
    ...
    document.querySelectorAll('.read-note-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const level = btn.dataset.level, subtopicId = btn.dataset.subtopicId, subtopicName = btn.dataset.subtopicName;
        loadNoteContentDirect(subtopicId, level, window.notesCurrentTopic, subtopicName);
      });
    });
  }
    ...
    try {
      ...
      const contentHtml = `<div class="notes-content-container" style="padding:24px; background:var(--clr-navy-card); border-radius:16px; animation:slideIn 0.3s ease;">
        ...
        <div class="comment-section" style="margin-top:20px;">
          <div class="comment-input-group" style="display:flex; gap:8px; margin-bottom:16px;">
            <input type="text" id="comment-input-${subtopicId}" placeholder="Add a comment or question..." style="flex:1; padding:10px 16px; border-radius:30px; border:1px solid var(--clr-border-glow); background:var(--clr-navy-card); color:var(--clr-white);">
            <button class="btn-primary" onclick="submitNoteComment('${subtopicId}')" style="padding:8px 20px;">Post</button>
          </div>
  ...
  window.toggleNoteReaction = async function (noteId, reactionType) {
    if (!isAuthenticated) { alert('Please sign in to react.'); return; }
    try {
  ...
  window.submitNoteComment = async function (noteId) {
    if (!isAuthenticated) { alert('Please sign in to comment.'); return; }
    ...
    try {
      ...
      if (container) {
        if (!interactions.comments || interactions.comments.length === 0) {
          ...
          container.innerHTML = interactions.comments.map(c => `<div class="comment-item"><strong>${esc(c.user_name)}:</strong> ${esc(c.comment)}<small>${new Date(c.created_at).toLocaleDateString()}</small></div>`).join('');
        }
      }
    } catch (e) {}
  }
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div className="pdf-main-container" id="pdf-main-container" style={{ background: 'var(--clr-navy-card)', backdropFilter: 'blur(12px)', borderRadius: '20px', overflow: 'hidden' }}>
            ...
            <div className="pdf-content-wrapper" id="pdf-content-wrapper" style={{ display: 'flex', minHeight: '520px' }}>
              <div className="pdf-cards-area" id="pdf-cards-area" style={{ flex: 1, padding: '20px', overflowY: 'auto', maxHeight: '600px' }}>
                <div className="pdf-loading">Loading PDFs...</div>
              </div>
              <div className="pdf-subtopics-column" id="pdf-subtopics-column" style={{ width: '280px', minWidth: '280px', background: 'rgba(0,0,0,0.03)', borderLeft: '1px solid var(--clr-border-glow)', display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="pdf-subtopics-list" id="pdf-subtopics-list" style={{ flex: 1, overflowY: 'auto', padding: '12px', maxHeight: '540px' }}>
                  <p className="pdf-subtopics-placeholder" style={{ textAlign: 'center', color: 'var(--clr-text-dim)', fontSize: '0.85rem', padding: '30px 20px' }}>Select a level to view topics</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div id="pdf-preview-modal" className="pdf-preview-modal active">
          <div className="pdf-preview-content">
            <div className="pdf-preview-header">
              ...
              <button className="pdf-preview-close" id="pdf-preview-close" onClick={() => setPdfModalOpen(false)}>&times;</button>
            </div>
            <div className="pdf-preview-body" id="pdf-preview-body">
              <iframe id="pdf-preview-iframe" src="about:blank" frameBorder="0"></iframe>
            </div>
