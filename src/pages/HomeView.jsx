        ...
        <div className="grid-3">
          {(sections?.blog?.posts || []).filter(Boolean).map(post => (
            <article key={post.title} className="card">
              ...
              <div className="flex gap-4 text-xs" style={{ color: 'var(--clr-text-muted)', padding: '1.2rem 1.5rem 0', marginBottom: '0.5rem' }}><span><i className="fa-regular fa-calendar"></i> {post.date}</span><span><i className="fa-regular fa-user"></i> {post.author}</span></div>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: '1.15rem' }}>{post.title}</h3>
              <p style={{ fontSize: '0.84rem', lineHeight: 1.7 }}>{post.excerpt}</p>
      ...
      <footer className="footer-fat">
        ...
        <div style={{ maxWidth: 'var(--max-width)', margin: '2rem auto 0', paddingTop: '1.5rem', borderTop: '1px solid var(--clr-border-glow)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <nav style={{ display: 'flex', gap: '22px' }}>
            ...
            <Link to="/About Us" style={{ fontSize: '.875rem', color: 'var(--clr-text-dim)' }}>Accessibility</Link>
          </nav>
        </div>
      </footer>
