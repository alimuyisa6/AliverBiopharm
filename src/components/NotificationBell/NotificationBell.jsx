...
export default function NotificationBell({ user }) {
  ...
  return (
    <div className="notification-bell" ref={dropdownRef}>
      ...
      {open && (
        <div className="dropdown-menu notification-dropdown" style={{ width: 360 }}>
          ...
          <div style={{ maxHeight: 400, overflowY: 'auto' }}>
            ...
            {notifications.map((n) => (
              <div
                key={n.id}
                className={`dropdown-item ${n.is_read ? '' : 'notification-unread'}`}
                onClick={() => handleMarkRead(n.id, n.action_url)}
                style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 'var(--space-1)' }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', width: '100%' }}>
                  <Icon name={n.icon || 'bell'} style={{ color: n.color || 'var(--primary)' }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: n.is_read ? 400 : 600, fontSize: 'var(--text-sm)' }}>{n.title}</div>
                    {n.body && <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-dim)' }}>{n.body}</div>}
                    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>{formatTime(n.created_at)}</div>
                  </div>
                  <button className="btn btn-ghost btn-sm btn-icon" onClick={(e) => handleDismiss(e, n.id)}>
                    <Icon name="xmark" />
                  </button>
                </div>
              </div>
