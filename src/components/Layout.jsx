...
// Links already surfaced in the user dropdown / mobile auth section.
// Filtered out of the CMS-driven navigation array so they never appear twice.
const RESERVED_NAV_PATHS = new Set(['/dashboard', '/profile']);
...
export default function Layout({ children, headerExtras, showFooter = true }) {
  ...
  const visibleNavigation = (navigation || []).filter(
    (link) => !RESERVED_NAV_PATHS.has((link.href || '').toLowerCase())
  );
          ...
          {!isAuthPage && (
            <header className={`site-header${scrolled ? ' scrolled' : ''}`}>
              <div className="header-container">
                <Link to="/" className="logo-link header-logo-wrap" aria-label={`${siteName} Home`}>
                  {logo ? <img src={logo} alt={siteName} loading="eager" /> : <span className="logo-text">{siteName}</span>}
                </Link>

                <nav aria-label="Main navigation">
                  <ul className="main-nav">
                    {visibleNavigation.map((link) => <li key={link.href}>{renderNavLink(link)}</li>)}
                  </ul>
                </nav>
                ...
                <motion.div key="mobile-nav-panel" className="mobile-nav-panel" variants={mobilePanelVariants} initial="hidden" animate="visible" exit="exit">
                  <div className="mobile-nav-panel-inner">
                    ...
                    <nav className="mobile-nav-links">
                      {visibleNavigation.map(renderMobileNavLink)}
                      {!authLoading && isAuthenticated && (
                        <>
