...
export function useHomePageEffects(siteData, isAuthenticated) {
  useEffect(() => {
    ...
    if (themeToggle) {
      const updateIcon = () => {
        ...
        if (icon)
          icon.className = `fa-solid ${document.body.classList.contains('dark-mode') ? 'fa-sun' : 'fa-moon'}`;
      };
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        document.querySelectorAll('.filter-option input').forEach((i) => {
          i.checked = false;
        });
        document.querySelectorAll('.filter-option input[value=""]').forEach((i) => {
          i.checked = true;
        });
    ...
    const loadAllSections = (sections) => {
        ...
        if (heroSection) {
          heroSection.innerHTML =
            sections.hero.slides
              .map(
                (s, i) =>
                  `<div class="carousel-slide${i === 0 ? ' active' : ''}" style="background-image:url('${esc(s.background_image)}');"><div class="slide-overlay"><h1 class="font-display font-black text-4xl md:text-6xl mb-4 hero-title">${esc(s.title)}</h1><p class="text-lg md:text-xl max-w-2xl mb-8 hero-subtitle">${esc(s.subtitle)}</p><a href="${esc(s.cta_link)}" class="btn-primary"><i class="${esc(s.icon || 'fa-solid fa-arrow-right')}" aria-hidden="true"></i> ${esc(s.cta_text)}</a></div></div>`
              )
              .join('') +
            '<button class="carousel-arrow carousel-prev" aria-label="Previous slide"><i class="fa-solid fa-chevron-left" aria-hidden="true"></i></button><button class="carousel-arrow carousel-next" aria-label="Next slide"><i class="fa-solid fa-chevron-right" aria-hidden="true"></i></button><div class="carousel-controls">' +
            sections.hero.slides
              .map(
                (_, i) =>
                  `<span class="carousel-dot${i === 0 ? ' active' : ''}" data-index="${i}" role="button" tabindex="0" aria-label="Go to slide ${i + 1}"></span>`
              )
              .join('') +
            '</div>';
      ...
      if (sections.hero_title) {
        ...
        document.getElementById('dynamic-main-title').innerHTML = mainTitle
          .split(' ')
          .map(
            (word, i) =>
              `<span class="title-word ${i % 2 === 0 ? 'magenta-word' : 'cyan-word'}">${esc(word)}</span>`
          )
          .join('');
        document.getElementById('title-sub-line').innerHTML = subLine
          .split(' ')
          .map((word) => {
            if (word.toLowerCase() === '&')
              return `<span class="sub-word magenta-word">&</span>`;
            if (word.toLowerCase() === 'biology' || word.toLowerCase() === 'pharmacy')
              return `<span class="sub-word cyan-word">${esc(word)}</span>`;
      ...
      if (sections.daily_facts || sections.weekly_challenge) {
        ...
        if (sections.daily_facts?.length) {
          ...
          let fact =
            sections.daily_facts.find((f) => f.date === today) ||
            sections.daily_facts[sections.daily_facts.length - 1];
          if (fact) {
      ...
      if (sections.testimonials?.quotes?.length) {
        ...
        function showQuote(i) {
          ...
          if (slider) {
            slider.innerHTML = `<blockquote class="testimonial-quote">"${esc(quotes[i].text)}"</blockquote><cite class="testimonial-author">— ${esc(quotes[i].author)}</cite><div class="testimonial-nav">${quotes
              .map(
                (_, j) =>
                  `<span class="testimonial-dot${j === i ? ' active' : ''}" data-index="${j}" role="button" tabindex="0" aria-label="Testimonial ${j + 1}"></span>`
              )
              .join('')}</div>`;
      ...
      if (sections.pricing?.plans?.length) {
        document.getElementById('pricing-grid').innerHTML = sections.pricing.plans
          .map(
            (pl) =>
              `<div class="card pricing-card${pl.featured ? ' featured' : ''}"><h3>${esc(pl.name)}</h3><p style="font-size:0.84rem;color:var(--clr-text-dim);">${esc(pl.description || '')}</p><div class="price my-3">${esc(pl.price)}<span style="font-size:1rem;color:var(--clr-text-dim);">${esc(pl.period || '')}</span></div><ul class="pricing-features">${(pl.features || [])
                .map((f) => `<li><i class="fa-solid fa-check"></i> ${esc(f)}</li>`)
                .join('')}</ul><button class="btn-primary mt-4" style="width:auto; display:inline-flex; margin-left:auto; margin-right:auto;">${esc(pl.cta_text || 'Subscribe')}</button></div>`
          )
          .join('');
      }
      if (sections.blog?.posts?.length) {
        document.getElementById('blog-grid').innerHTML = sections.blog.posts
          .map(
            (p) =>
              `<article class="card">${p.image_url ? `<img src="${esc(p.image_url)}" alt="${esc(p.title)}" style="width:100%;height:200px;object-fit:cover;border-radius:var(--radius-md);margin-bottom:1rem;">` : ''}<div class="flex gap-4 text-xs" style="color:var(--clr-text-muted);margin-bottom:0.5rem;"><span><i class="fa-regular fa-calendar" aria-hidden="true"></i> ${esc(p.date || '')}</span><span><i class="fa-regular fa-user" aria-hidden="true"></i> ${esc(p.author || '')}</span></div><h3 style="font-family:'Poppins',sans-serif;font-weight:700;font-size:1.15rem;color:var(--clr-white);">${esc(p.title)}</h3><p style="font-size:0.84rem;line-height:1.7;color:var(--clr-text-dim);">${esc(p.excerpt || '')}</p><a href="#" style="color:var(--clr-magenta);font-weight:600;font-size:0.875rem;">Read Article <i class="fa-solid fa-arrow-right" aria-hidden="true"></i></a></article>`
          )
          .join('');
      }
      if (sections.faq?.items?.length) {
        document.getElementById('faq-list').innerHTML = sections.faq.items
          .map(
            (i) =>
              `<div class="faq-item"><button class="faq-question" onclick="this.parentElement.classList.toggle('active')"><span>${esc(i.question)}</span><span style="color:var(--clr-cyan);">+</span></button><div class="faq-answer" role="region"><p>${esc(i.answer)}</p></div></div>`
          )
          .join('');
      }
      if (sections.contact?.info) {
        ...
        if (contactInfoCard) {
          contactInfoCard.innerHTML += sections.contact.info
            .map(
              (c) =>
                `<div style="display:flex;align-items:center;gap:1rem;padding:0.8rem 0;border-bottom:1px solid var(--clr-border-glow);"><div class="contact-icon"><i class="${esc(c.icon)}" aria-hidden="true"></i></div><div><div style="font-size:0.7rem;color:var(--clr-text-muted);">${esc(c.label)}</div><a href="${esc(c.href)}" style="color:var(--clr-white);text-decoration:none;">${esc(c.value)}</a></div></div>`
            )
            .join('');
        }
      }
      if (sections.footer) {
        ...
        if (footerGrid) {
          footerGrid.innerHTML = (sections.footer.columns || [])
            .map(
              (c) =>
                `<div><h4 style="font-weight:700;color:var(--clr-white);font-size:0.9rem;margin-bottom:16px;">${esc(c.heading)}</h4><ul style="list-style:none;display:flex;flex-direction:column;gap:10px;">${(c.items || [])
                  .map(
                    (i) =>
                      `<li><a href="${esc(i.href || '#')}" style="font-size:0.875rem;color:var(--clr-text-dim);text-decoration:none;">${i.icon ? `<i class="${esc(i.icon)}" style="color:var(--clr-magenta);margin-right:0.5rem;"></i>` : ''}${esc(i.label)}</a></li>`
                  )
                  .join('')}</ul></div>`
            )
            .join('');
        ...
        if (footerSocial) {
          footerSocial.innerHTML = (sections.footer.social_links || [])
            .map(
              (s) =>
                `<a href="${esc(s.url)}" aria-label="${esc(s.platform)}" target="_blank" rel="noopener noreferrer"><i class="${esc(s.icon)}" aria-hidden="true"></i></a>`
            )
            .join('');
        }
      }
      ...
      if (!isAuthenticated) {
        ...
        if (existingCard)
          existingCard.innerHTML +=
            '<p style="text-align:center;color:var(--clr-text-dim);">Sign in to attempt the weekly challenge.</p>';
        return;
      }
      ...
      async function render() {
        ...
        if (status.already_answered) {
          ...
          quizHTML =
            ...
            weeklyChallenge.options
              .map(
                (opt, i) =>
                  `<button class="quiz-option-btn" data-idx="${i}">${String.fromCharCode(65 + i)}) ${esc(opt)}</button>`
              )
              .join('') +
            `<p style="color:var(--clr-text-muted);font-size:0.8rem;margin-top:0.5rem;">Submissions close in ${countdownStr}</p></div>`;
        }
        const existingCard = container.querySelector('.weekly-challenge-card');
        if (existingCard) {
        ...
        if (container && categories.length) {
          ...
          categories.forEach((cat) => {
            ...
            label.querySelector('input').addEventListener('change', () => {
              document.getElementById('selected-category').textContent = cat;
            });
    ...
    async function buildResources() {
      ...
      try {
        ...
        container.innerHTML = '<p style="text-align:center;padding:2rem;color:var(--clr-magenta);">Failed to load resources.</p>';
      }
    }

    window.renderFilteredResources = function () {
      const container = document.getElementById('resources-container');
      if (!container) return;
    ...
    window.focusDeck = function (category) {
      window.flashcardState.currentDeck = category;
      window.flashcardState.currentCardIndex = 0;
      window.flashcardState.quizAnswered = false;
      filterDecks();
      setTimeout(() => {
        document.getElementById('deck-' + category.replace(/\s+/g, '-'))?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
    };

    window.handleCardClick = function (faceElement, cardId, deckName, cardIndex) {
    ...
    function renderActiveDeck() {
      ...
      if (deckContainer) {
        ...
        if (grid && cards) {
          ...
          if (counter) {
            counter.textContent = 'Card ' + (window.flashcardState.currentCardIndex + 1) + ' of ' + cards.length;
          }
          grid.querySelectorAll('.flashcard-face').forEach((face, idx) => {
            face.classList.toggle('active-card', idx === window.flashcardState.currentCardIndex);
          });
        }
      }
    }
    ...
    function renderDecksHTML(container, decks, knownIds) {
      ...
      container.innerHTML = Object.entries(decks)
        .map(([category, cards]) => {
        ...
        if (deck && interactions) {
          ...
          if (commentsContainer && interactions.comments) {
            commentsContainer.innerHTML = interactions.comments
              .map((c) => `<div class="comment-item"><strong>${esc(c.user_name || 'User')}:</strong> ${esc(c.comment)}</div>`)
              .join('');
          }
        }
      } catch (e) {}
    ...
    window.checkQuizAnswer = async function (cardId, inputElement) {
      ...
      try {
        ...
        if (face) {
          setTimeout(() => face.classList.add('flipped'), 800);
        }
        setTimeout(() => {
          navigateCards(1);
    ...
    window.toggleCommentForm = function (cardId, btn) {
    ...
    async function loadComments(cardId) {
      try {
        ...
        if (container && interactions?.comments) {
          container.innerHTML = interactions.comments
            .map(
              (c) =>
                `<div class="comment-item"><strong>${esc(c.user_name)}:</strong> ${esc(c.comment)}<small style="color:var(--clr-text-muted);">${new Date(c.created_at).toLocaleDateString()}</small></div>`
            )
            .join('');
        }
      } catch (e) {}
    ...
    async function loadPdfsByLevel(level) {
      ...
      try {
        ...
        cardsArea.innerHTML = '<div class="pdf-loading">Error loading PDFs.</div>';
      }
    }
    ...
    function renderTopicsColumn(topics) {
      ...
      if (!topics || topics.length === 0) {
        topicsList.innerHTML = '<p class="pdf-subtopics-placeholder">No topics available</p>';
        return;
      }
      const topicCounts = {};
      window.currentPdfs.forEach((pdf) => {
        const t = pdf.topic || 'General';
        topicCounts[t] = (topicCounts[t] || 0) + 1;
      });
      topicsList.innerHTML = topics
        .map(
          (topic) =>
            `<div class="pdf-subtopic-item" data-topic="${esc(topic)}"><div class="pdf-subtopic-title">${esc(topic)}</div><div class="pdf-subtopic-author">${topicCounts[topic] || 0} resource${topicCounts[topic] !== 1 ? 's' : ''}</div></div>`
        )
        .join('');
    ...
    function filterPdfsByTopic(topic) {
      ...
      if (!filteredPdfs.length) {
        cardsArea.innerHTML = '<div class="pdf-loading">No PDFs in this topic.</div>';
        return;
      }
      ...
      document.querySelectorAll('.pdf-card').forEach((card) => {
        card.addEventListener('click', (e) => {
          ...
          const pdfId = card.dataset.pdfId,
            pdfTitle = card.dataset.pdfTitle,
            pdfUrl = card.dataset.pdfUrl,
            pdfAuthor = card.dataset.pdfAuthor;
    ...
    async function previewPdf(pdfId, pdfTitle, pdfUrl) {
      try {
        ...
        if (restriction.is_restricted) {
          alert(`Preview restricted: ${restriction.reason}`);
          return;
        }
        window.homePageSetPdfModalOpen(true);
    ...
    async function downloadPdf(pdfId, pdfTitle, pdfUrl) {
      try {
        ...
        if (restriction.is_restricted) {
          alert(`Download restricted: ${restriction.reason}`);
          return;
        }
        ...
        window.homePageSetPdfModalOpen(false);
      } catch (err) {
    ...
    async function loadNotesStructure() {
      try {
        ...
        console.error('loadNotesStructure failed:', err);
    ...
    function renderLevelButtons() {
      ...
      if (!container) return;
      if (!window.notesStructureData.length) {
        container.innerHTML = '<p style="color:var(--clr-text-dim);">No levels available.</p>';
        return;
      }
      const levels = {};
      window.notesStructureData.forEach((item) => {
        if (!item.level) return;
        levels[item.level] = { level_order: Number(item.level_order) || 999 };
      });
    ...
    function renderTopicsForLevel(level) {
      ...
      window.notesStructureData.forEach((item) => {
        if (item.level === level) {
          if (!topics[item.topic]) topics[item.topic] = { topic_order: Number(item.topic_order) || 999 };
        }
      });
      ...
      try {
        ...
        if (container) {
          if (!interactions.comments || interactions.comments.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--clr-text-muted); padding:20px;">No comments yet. Be the first!</div>';
          } else {
            container.innerHTML = interactions.comments
              .map((c) => `<div class="comment-item"><strong>${esc(c.user_name)}:</strong> ${esc(c.comment)}<small>${new Date(c.created_at).toLocaleDateString()}</small></div>`)
              .join('');
          }
        }
      } catch (e) {}
    ...
    window.downloadNoteAsPDF = function () {
      ...
      win.document.write(
        `<html><head><title>${document.querySelector('.notes-content-container h1')?.innerText || 'Note'}</title></head><body>${contentDiv.innerHTML}</body></html>`
      );
      win.document.close();
      win.print();
    };
    ...
    async function loadCommunityActivity() {
      ...
      try {
        ...
        if (!activities || !activities.length) {
          container.innerHTML = '<p style="text-align:center;color:var(--clr-text-dim);">No recent activity.</p>';
          return;
        }
        container.innerHTML = activities
          .map(
            (a) =>
              `<div class="stream-item"><i class="fa-solid fa-${a.type === 'download' ? 'download' : 'graduation-cap'}" style="color:var(--clr-cyan);"></i><span>${esc(a.message)}</span><small style="margin-left:auto;color:var(--clr-text-muted);">${new Date(a.time).toLocaleDateString()}</small></div>`
          )
          .join('');
      } catch (e) {
        container.innerHTML = '<p style="text-align:center;color:var(--clr-magenta);">Could not load stream.</p>';
      }
    }
    ...
    async function loadContinueLearning() {
      ...
      section.innerHTML =
        '<span class="sec-label">YOUR JOURNEY</span><h2 class="section-title">Continue Learning</h2><div class="continue-learning-grid" id="continue-grid">Loading...</div>';
      ...
      try {
        ...
        const v = results[0].value || [],
          f = results[1].value || [],
          s = results[2].value || { count: 0 },
          a = results[3].value || [];
        if (s && s.count > 0)
          html += `<div class="continue-card"><i class="fa-solid fa-fire" style="color:var(--clr-magenta);"></i> <strong>${s.count}-Day Streak</strong><p style="font-size:0.8rem;">Keep it up!</p></div>`;
        if (v && v.length)
          html += `<div class="continue-card"><strong>Recent Views</strong><ul style="list-style:none;margin-top:0.4rem;">${v.map((r) => `<li><a href="#" style="color:var(--clr-cyan);text-decoration:none;">${esc(r.title || 'Resource')}</a></li>`).join('')}</ul></div>`;
        if (f && f.length)
          html += `<div class="continue-card"><strong>Favorites</strong><ul style="list-style:none;margin-top:0.4rem;">${f.slice(0, 3).map((r) => `<li>${esc(r.title || 'Resource')}</li>`).join('')}</ul></div>`;
        if (a && a.length)
          html += `<div class="continue-card"><strong>Achievements</strong><div style="display:flex;gap:0.5rem;margin-top:0.4rem;">${a.map((b) => `<span title="${esc(b.badge)}" style="font-size:1.4rem;">🏅</span>`).join('')}</div></div>`;
    ...
    moodEmojis.forEach((emoji) => {
      emoji.addEventListener('click', (e) => {
        if (!isAuthenticated) {
          alert('Sign in to share how you feel.');
          return;
        }
    ...
    document.getElementById('mood-submit-btn').addEventListener('click', async () => {
      if (!window.selectedMood || !isAuthenticated) return;
    ...
    lazyLoad('#pdf-library', () => {
      if (document.getElementById('pdf-main-container')) initPdfLibrary();
    });
    lazyLoad('#stats', () => {
      loadPublicStats();
    });
