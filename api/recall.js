...
function verifyCsrf(req, secret, userId, ip) {
  ...
  try {
    ...
    if (ts !== currentWindow && ts !== prevWindow) throw new Error();
...
async function handleGetSession(userId, { level, topic }) {
  ...
  if (existing && existing.question_ids?.length) {
    ...
    const ordered = existing.question_ids
  ...
  const { data: completion } = await supabase
    ...
    .eq('topic_key', topicKey)
    .maybeSingle();
  if (completion?.last_completed === today)
    throw new Error('You already completed this topic today. Come back tomorrow.');
...
async function handleContinueSession(userId, { session_id }) {
  ...
  const { data: session, error: sessionError } = await supabase
    .from('recall_sessions')
    .select('question_ids, all_question_ids')
...
async function handleSubmitAnswer(userId, params, ip) {
  ...
  const { data: nonceEntry } = await supabase
    .from('request_nonces')
    .select('nonce')
    .eq('nonce', nonce)
    .eq('user_id', userId)
  ...
  if (session.user_answers?.some(a => a.question_id === question_id))
  ...
  const { data: stats } = await supabase
    ...
    .select('total_xp, recall_level, current_streak, mastery, best_streak, best_mastery, milestones, last_session_date')
    .eq('user_id', userId)
    .maybeSingle();
  let statsData = stats || {
    total_xp: 0,
  ...
  await supabase
    ...
    .upsert(
      { user_id: userId, topic: topicName, xp: result.xp, streak: 1, last_activity_date: today },
    ...
    .upsert(
      {
        ...
        total_questions: (statsData.total_questions || 0) + 1,
  ...
  if (completed) {
    ...
    await supabase
      .from('user_topic_completion')
      .upsert(
        { user_id: userId, topic_key: topicKey, last_completed: today },
...
async function handleGetStats(userId) {
  ...
  const masteryPercent = masteryValues.length
  ...
  const heatmap = {};
  (daily || []).forEach(d => {
    heatmap[d.activity_date] = d.count;
  });
  ...
  const topicXp = {},
    topicStreak = {};
  (topics || []).forEach(t => {
    topicXp[t.topic] = t.xp;
    topicStreak[t.topic] = t.streak;
  });
...
async function handleGetDashboard(userId) {
  ...
  if (!stats) {
    const empty = {
      ...
      totalQuestions: 0,
      milestones: [],
      dailyChallenge: { completed: 0, target: 10, progressPercent: 0 },
      gardenStage: 'seedling',
      subjectIllustration: 'fa-flask',
      quote: 'Welcome! Start your first recall session.',
      brainEnergy: 100
    };
    setCached(cacheKey, empty, 30000);
    return empty;
  ...
  const result = {
    ...
    dailyChallenge: {
      ...
      target: 10,
      progressPercent: (dailyQuestions / 10) * 100
    },
    gardenStage,
...
async function handleSetLevel(userId, level) {
  ...
  if (existing?.selected_level && !isAdmin) {
    throw new Error('Level already set and cannot be changed');
  }

  await supabase
    .from('user_recall_stats')
    .upsert({ user_id: userId, selected_level: level }, { onConflict: 'user_id' });
  invalidateUserCache(userId);
  return { success: true };
}
...
async function handleGetLevel(userId) {
  ...
  const { data: admin } = await supabase
    ...
    .maybeSingle();
  return {
    selected_level: data?.selected_level || null,
...
export default async function handler(req, res) {
  ...
  if (token) {
    ...
    if (session) {
      ...
      const { data: sessionData } = await supabase
        ...
        .single();
      csrfSecret = sessionData?.csrf_secret || null;
    }
  }
  ...
  if (!(await checkRateLimit(ip, userId))) {
  ...
  try {
    ...
    switch (path) {
      case 'session':
        result = await handleGetSession(userId, { level: req.query.level, topic: req.query.topic });
        break;
      case 'session_check':
        result = await handleSessionCheck(userId, { level: req.query.level, topic: req.query.topic });
        break;
      case 'stats':
        result = await handleGetStats(userId);
        break;
      case 'achievements':
        result = await handleGetAchievements(userId);
        break;
      case 'dashboard':
        result = await handleGetDashboard(userId);
        break;
      case 'topics':
        if (!req.query.level) throw new Error('Level required');
        result = await getTopicsForLevel(req.query.level);
        break;
      case 'first_visit':
        if (!req.query.level) throw new Error('Level required');
      ...
      case 'get_selected_level':
        result = await handleGetLevel(userId);
        break;

      case 'continue':
        if (!csrfSecret) throw new Error('CSRF secret not available');
        verifyCsrf(req, csrfSecret, userId, ip);
        result = await handleContinueSession(userId, body);
        break;
      case 'answer':
        if (!csrfSecret) throw new Error('CSRF secret not available');
        verifyCsrf(req, csrfSecret, userId, ip);
        result = await handleSubmitAnswer(userId, body, ip);
        break;
      case 'complete':
        if (!csrfSecret) throw new Error('CSRF secret not available');
        verifyCsrf(req, csrfSecret, userId, ip);
        result = await handleCompleteSession(userId, body);
        break;
      case 'set_selected_level':
        if (!csrfSecret) throw new Error('CSRF secret not available');
        verifyCsrf(req, csrfSecret, userId, ip);
        result = await handleSetLevel(userId, body.level);
        break;
    ...
    const status =
      error.message.includes('already completed') || error.message.includes('already answered')
        ? 400
        : 500;
