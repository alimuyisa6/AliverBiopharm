...
function checkAnswerRateLimit(userId, blockKey) {
  const key = `${userId}:${blockKey}`;
  const now = Date.now();
  const entry = answerRateLimit.get(key) || { count: 0, resetAt: now + 60 * 60 * 1000 };
  if (now > entry.resetAt) {
    answerRateLimit.set(key, { count: 1, resetAt: now + 60 * 60 * 1000 });
    return true;
  }
  if (entry.count >= 15) return false;
  entry.count++;
  answerRateLimit.set(key, entry);
  return true;
}

function cleanupRateLimit() {
  const now = Date.now();
  for (const [key, entry] of answerRateLimit.entries()) {
    if (now > entry.resetAt) answerRateLimit.delete(key);
  }
}
...
async function checkUserRestriction(userId) {
  const { data } = await supabase.from('user_restrictions').select('restriction_type, expires_at').eq('user_id', userId).maybeSingle();
  if (!data) return null;
  if (data.restriction_type === 'disabled' || data.restriction_type === 'suspended') return data.restriction_type;
  if (data.restriction_type === 'locked' && data.expires_at && new Date(data.expires_at) > new Date()) return 'locked';
  return null;
}
...
async function getQuizBlock(req, res, userId) {
  ...
  if (restriction) return res.status(403).json({ error: 'Your account is restricted.' });
  ...
  const shuffled = seededShuffle(questions, `${userId}:${topic}:${block_number}`);

  return res.status(200).json({ block_number: parseInt(block_number), questions: shuffled, total_in_block: questions.length });
}
...
async function checkQuizAnswer(req, res, userId) {
  ...
  const { question_id, selected_option, topic, block_number, level } = req.body;

  if (!question_id || !selected_option || !topic || block_number === undefined || !level) {
    return res.status(400).json({ error: 'question_id, selected_option, level, topic and block_number required' });
  }
  ...
  const blockKey = `${level}:${topic}:${block_number}`;
  if (!checkAnswerRateLimit(userId, blockKey)) {
    return res.status(429).json({ error: 'Too many answer attempts. Please slow down.' });
  }

  const { data: question, error } = await supabase
    .from('quiz_questions')
    .select('id,correct_option,option_a,option_b,option_c,option_d,level,topic')
    .eq('id', question_id)
    .eq('level', level)
    .eq('topic', topic)
    .single();
  ...
  const validIds = (blockQuestions || []).map(q => q.id);
  if (!validIds.includes(question_id)) {
    return res.status(403).json({ error: 'Question does not belong to this block.' });
  }
...
async function submitQuizBlock(req, res, userId) {
  ...
  if (typeof time_taken === 'number' && time_taken < 30) {
    return res.status(400).json({ error: 'Submission rejected. Minimum time not met.' });
  }

  if (answers.length > 10) return res.status(400).json({ error: 'Too many answers submitted.' });

  for (const a of answers) {
    if (!a.id || !['A', 'B', 'C', 'D', 'X'].includes(a.selectedOption)) {
  ...
  const offset = parseInt(block_number) * 10;
  const { data: blockQuestions, error: bqe } = await supabase
    .from('quiz_questions')
    .select('id')
    .eq('level', level)
    .eq('topic', topic)
  ...
  if (bqe) return res.status(500).json({ error: bqe.message });

  const validIds = new Set((blockQuestions || []).map(q => q.id));
  const submittedIds = answers.map(a => a.id);
  const allValid = submittedIds.every(id => validIds.has(id));
  if (!allValid) return res.status(403).json({ error: 'One or more submitted questions do not belong to this block.' });

  const { data: questions, error: qe } = await supabase.from('quiz_questions').select('id,correct_option,explanation,question_text,option_a,option_b,option_c,option_d,difficulty').in('id', submittedIds);
  ...
  await supabase.from('user_quiz_sessions').delete().eq('user_id', userId);
...
async function addQuizQuestionsBatch(req, res, adminData) {
  ...
  const { data: existingTopic } = await supabase.from('quiz_topics').select('id,question_count,total_blocks').eq('level', level).eq('topic_name', topic).maybeSingle();
  let topicId;
  let currentCount = 0;

  if (existingTopic) {
    topicId = existingTopic.id;
    currentCount = existingTopic.question_count || 0;
  } else {
    const { data: newTopic } = await supabase.from('quiz_topics').insert({ level, topic_name: topic, is_active: true, question_count: 0, total_blocks: 0 }).select().single();
    topicId = newTopic.id;
  }
  ...
  const newBlocks = Math.floor(newCount / 10);
  await supabase.from('quiz_topics').update({ question_count: newCount, total_blocks: newBlocks, updated_at: new Date().toISOString() }).eq('id', topicId);

  return res.status(200).json({ success: true, questions_added: questions.length, total_questions: newCount, total_blocks: newBlocks });
}
