...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (err instanceof SecurityError) { console.error(`[403 DEBUG] ${moduleName}/${path}:`, err.message); return res.status(err.statusCode).json({ error: err.message }); }
    if (!res.writableEnded) { const statusCode = err.statusCode || 500; const message = statusCode === 500 ? 'Internal server error' : sanitizeError(err); console.error(`[ERROR] ${moduleName}/${path}:`, err.message); res.status(statusCode).json({ error: message }); }
  }
}
