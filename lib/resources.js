...
async function getPdfsByLevel(req, res) {
  ...
  let query = supabase.from('pdf_resources').select('id,title,author,level,topic,subtopic,file_url,file_size,download_count,preview_count').eq('is_active', true).order('topic', { ascending: true });
...
async function getContinueReading(req, res, ctx) {
  ...
  const { data, error } = await supabase.from('user_interactions').select('resource_id, value, metadata, created_at').eq('user_id', ctx.userId).eq('interaction_type', 'reading_progress').neq('value', 100).gt('value', 5).order('created_at', { ascending: false }).limit(limit);
...
async function saveReadingProgress(body, res, ctx) {
  ...
  if (existing) {
    ...
    await supabase.from('user_interactions').insert({ user_id: ctx.userId, interaction_type: 'reading_progress', resource_id: numericNoteId, value: scroll_percentage, metadata: { note_id, scroll_position: scroll_position || 0, time_spent: time_spent || 0, completed: completed || false, started_at: new Date().toISOString() } });
  }
  return res.status(200).json({ success: true });
}
