...
async function getUserLevelAccess(userId) {
  if (!userId) return { level: null, class_name: null, isAdmin: false, showAll: false, pending: false };

  const adminData = await isAdmin(userId, 'unknown');
  if (adminData && adminData.admin_role) {
    return { level: null, class_name: null, isAdmin: true, showAll: true, pending: false };
  }
  ...
  if (!profile) return { level: null, class_name: null, isAdmin: false, showAll: false, pending: false };
  ...
  if (profile.role === 'teacher') {
    if (!profile.is_approved_teacher) {
      return { level: null, class_name: null, isAdmin: false, showAll: false, pending: true };
    }
    if (profile.approved_track === 'ALL') {
      return { level: null, class_name: null, isAdmin: false, showAll: true, pending: false };
    }
    return { 
  ...
  return { level: null, class_name: null, isAdmin: false, showAll: false, pending: false };
}
...
async function getResources(req, res, ctx) {
  ...
  let query = supabase
    .from('biology_notes')
    .select('id,title,description,author,level,category,tag,section_type,file_url,file_size,download_count,created_at,class_name')
  ...
  if (userClass && !showAll && !isAdminUser) {
...
async function getPdfsByLevel(req, res, ctx) {
  ...
  let query = supabase
    .from('pdf_resources')
    .select('id,title,author,level,topic,subtopic,file_url,file_size,download_count,preview_count,class_name')
    .eq('is_active', true)
...
async function getNotesStructure(req, res, ctx) {
  ...
  if (!showAll && !isAdminUser && userLevel) {
    filteredData = filteredData.filter(item => item.level === userLevel);
  }

  if (userClass && !showAll && !isAdminUser) {
    filteredData = filteredData.filter(item => item.class_name === userClass);
  }
...
async function getNoteContent(req, res, ctx) {
  ...
  if (ctx.authenticated && ctx.userId) {
    ...
    if (noteMeta) {
      ...
      const access = await getUserLevelAccess(ctx.userId);
      if (!isAdminUser && !access.showAll) {
        if (access.class_name && noteMeta.class_name && access.class_name !== noteMeta.class_name) {
          throw new SecurityError('This content is not available for your class', 403);
        }
      }
    }
  }
...
async function getContinueReading(req, res, ctx) {
  ...
  const userClass = access.class_name;
  ...
  for (const item of (data || [])) {
    const { data: noteData } = await supabase
      .from('notes_structure')
      .select('subtopic_name, topic, level, class_name')
    ...
    if (noteData) {
      ...
      if (!showAll && userClass && noteData.class_name && noteData.class_name !== userClass) {
        continue;
      }
      notes.push({
