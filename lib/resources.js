...
async function trackPdfPreview(body, res, ctx) {
  const { pdf_id } = body;
  await supabase.from('pdf_resources').update({ preview_count: supabase.raw('preview_count + 1') }).eq('id', pdf_id);
  return res.status(200).json({ success: true });
}

async function trackPdfDownload(body, res, ctx) {
  const { pdf_id } = body;
  await supabase.from('pdf_resources').update({ download_count: supabase.raw('download_count + 1') }).eq('id', pdf_id);
  return res.status(200).json({ success: true });
}
