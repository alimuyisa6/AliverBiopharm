...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      case 'list': return listTutors(req, res);
      case 'detail': return getTutorDetail(req, res);
      case 'search': return searchTutors(req, res);
...
async function listTutors(req, res) {
  ...
  if (search) {
    const tsquery = search.replace(/[^a-zA-Z0-9 ]/g, '').split(' ').filter(w => w).map(w => `${w}:*`).join(' & ');
...
async function getTutorDetail(req, res) {
  ...
  const [{ data: units }, { data: employment }, { data: verifications }] = await Promise.all([
  ...
  return res.status(200).json({
    ...profile,
    subjects: units?.map(u => u.unit_id) || [],
...
async function searchTutors(req, res) {
  ...
  const tsquery = q.replace(/[^a-zA-Z0-9 ]/g, '').split(' ').filter(w => w).map(w => `${w}:*`).join(' & ');
...
async function createOrUpdateProfile(body, res, ctx) {
  ...
  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: existing ? 'update_tutor_profile' : 'create_tutor_profile', targetType: 'tutor_profile', targetId: profileId });
...
async function updateEmployment(body, res, ctx) {
  ...
  await auditLog({ actorId: ctx.userId, action: 'update_tutor_employment', targetType: 'tutor_profile', targetId: profile.id });
...
async function uploadVerification(body, res, ctx) {
  ...
  const validTypes = ['identity', 'degree_certificate', 'teaching_certificate', 'teaching_license', 'recommendation_letter', 'curriculum_vitae', 'employment_proof', 'other'];
  if (!validTypes.includes(verification_type)) throw new SecurityError('Invalid verification_type', 400);

  const profile = await ensureProfileOwnership(ctx.userId, true);

  const { data: file } = await supabase.from('user_files').select('id').eq('id', file_id).eq('user_id', ctx.userId).maybeSingle();
  if (!file) throw new SecurityError('File not found', 404);

  const { data: check } = await supabase.from('tutor_verification_checks').insert({
    profile_id: profile.id,
    file_id,
    verification_type,
  ...
  await auditLog({ actorId: ctx.userId, action: 'tutor_upload_verification', targetType: 'tutor_verification_check', targetId: check.id });
...
async function activateListing(body, res, ctx) {
  ...
  await createNotification(ctx.userId, 'payment_received', { display_name: profile.display_name, expiry_date: expiresAt });
...
async function sendContactRequest(body, res, ctx) {
  ...
  await createNotification(tutor_id, 'new_contact_request', { requester_name: await getUserProfileName(ctx.userId) || 'A student' });
...
async function respondContactRequest(body, res, ctx) {
  ...
  if (!request_id || !['accept', 'reject'].includes(action)) throw new SecurityError('request_id and action (accept/reject) required', 400);
  ...
  await supabase.from('tutor_contact_requests').update({ status: newStatus, responded_at: new Date().toISOString() }).eq('id', request_id);

  await createNotification(request.requester_id, newStatus === 'accepted' ? 'tutor_accepted_request' : 'tutor_rejected_request', {});
...
async function adminVerifyDocument(body, res, ctx) {
  ...
  const { data: check } = await supabase.from('tutor_verification_checks').select('*').eq('id', check_id).maybeSingle();
  ...
  const { data: profile } = await supabase.from('tutor_profiles').select('*').eq('id', check.profile_id).maybeSingle();
  if (profile) {
    const newScore = action === 'approve' ? Math.min(100, profile.verification_score + 20) : profile.verification_score;
    await supabase.from('tutor_profiles').update({ verification_score: newScore, verification_status: 'verified' }).eq('id', profile.id);
    await createNotification(profile.user_id, action === 'approve' ? 'verification_approved' : 'verification_rejected', { verification_type: check.verification_type, reason: rejection_reason || '' });
  }

  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: action === 'approve' ? 'verify_document_approved' : 'verify_document_rejected', targetType: 'tutor_verification_check', targetId: check_id });
...
async function adminManageProfile(body, res, ctx) {
  ...
  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: 'admin_update_tutor_profile', targetType: 'tutor_profile', targetId: profile_id, metadata: updates });
