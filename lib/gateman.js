...
/**
 * Every module request must pass through here before it reaches any lib
 * handler. Order: IP block -> threat scoring -> session/auth resolution ->
 * account restriction -> rate limit -> CSRF. Returns an authenticated ctx
 * object on success, or null after having already written a response.
 */
...
/**
 * Lib handlers can call this when they detect something suspicious that
 * doesn't rise to an immediate SecurityError (e.g. a premium probe, a
 * malformed but non-crashing payload, repeated failed lookups). Logs the
 * event and escalates to auto-suspension if the same user racks up enough
 * incidents inside the rolling window.
 */
...
export function gatemanErrorResponse(res, err, moduleName, path) {
