...
async function getContentGuideImage(req, res, ctx) {
  ...
  if (error) {
    throw new SecurityError('Failed to fetch content guide image', 500);
  }
...
async function getContentGuideImages(req, res, ctx) {
  ...
  if (error) {
    throw new SecurityError('Failed to fetch content guide images', 500);
  }
...
async function updateContentGuideImage(body, res) {
  ...
  if (!VALID_LEVELS.includes(level)) {
    throw new SecurityError(`level must be one of: ${VALID_LEVELS.join(', ')}`, 400);
  }
  ...
  if (error) {
    throw new SecurityError('Failed to update content guide image', 500);
  }
...
async function deleteContentGuideImage(body, res) {
  ...
  if (error) {
    throw new SecurityError('Failed to delete content guide image', 500);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      try {
        ...
        reject(new SecurityError('Invalid JSON body', 400));
      }
    });
    req.on('error', reject);
  });
