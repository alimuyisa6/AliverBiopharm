...
async function getTopics(req, res, userId) {
  ...
  // Get the unit and its level for access checks
  const { data: unit, error: unitError } = await supabase
    .from('curriculum_units')
    .select('id, name, group_id, curriculum_groups(level_id)')
    .eq('id', unit_id)
    .maybeSingle();
  if (unitError || !unit) throw new SecurityError('Unit not found', 404);
  const levelId = unit.curriculum_groups?.level_id;
  ...
  if (userId) {
    const { data: activity } = await supabase
      ...
      .eq('user_id', userId)
      .eq('level', levelId)
      .eq('topic', unit.name);   // we still store topic name for backward compatibility
    if (activity) {
...
async function checkDailyRetry(req, res, userId) {
  ...
  // Check integrity lock (tab switch violation)
...
async function getSessionStatus(req, res, userId) {
  ...
  if (unit_id && block_number !== undefined) {
    query = query.eq('unit_id', unit_id).eq('block_number', parseInt(block_number)).maybeSingle();
  } else {
...
async function submitWithSession(body, res, userId) {
  ...
  // Check if all blocks for this unit are completed and optionally advance class
