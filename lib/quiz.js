...
async function getUnitWithValidation(ctx, unitId) {
  ...
  return {
    id: unit.id,
    name: unit.name,
    levelId: unit.curriculum_groups?.level_id || null,
    levelName: unit.curriculum_groups?.curriculum_levels?.display_name || null,
  };
}
...
async function getTopics(req, res, ctx) {
  ...
  const { data: activity } = await supabase
    ...
    .eq('level', unit.levelName)       // stored as text level name
    .eq('topic', unit.name);
...
async function checkDailyRetry(req, res, ctx) {
  ...
  // integrity lock from tab switch
...
async function getSessionStatus(req, res, ctx) {
  ...
  // fallback: latest session in user's active level
  const scope = await getUserCurriculumScope(ctx.userId);
...
async function submitWithSession(body, res, ctx) {
  ...
  if (xpEarned > 0) {
    await addXp(ctx.userId, xpEarned, 'quiz_block');
  }
...
async function addQuestionsBatch(body, res) {
  ...
  // validate unit exists (admin can add to any unit)
  const { data: unit } = await supabase
    .from('curriculum_units')
    .select('id')
    .eq('id', unit_id)
