import { supabase, addXp, recordPlatformActivity, updateTopicXp, updateUserRecords, updateDailyChallengeProgress } from './core.js';
import {
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    ...
    switch (path) {
      ...
      case 'list_quiz_topics':     return listTopics(req, res, ctx);
...
async function getUnitWithValidation(ctx, unitId, blockNumber = null) {
  ...
  console.log('[DEBUG] getUnitWithValidation: scope =', JSON.stringify(scope));
  if (!scope || !scope.active_group_id) {
    console.error('[DEBUG] getUnitWithValidation: no active group');
  ...
  if (!unit) {
    console.error('[DEBUG] getUnitWithValidation: unit not found', { unitId, active_group_id: scope.active_group_id });
...
async function getSession(ctx, unit, blockNum) {
  // Look up session without filtering on level to avoid mismatch when level name changes
  console.log('[DEBUG] getSession: looking up with', {
    userId: ctx.userId,
    topic: unit.name,
    blockNum,
  });
  ...
  if (error) {
    console.error('[DEBUG] getSession: query error', error);
    throw new SecurityError('Failed to verify quiz session', 500);
  }

  // If found, but level differs from current unit.levelName, update it silently
  if (session && session.level !== unit.levelName) {
    console.warn('[DEBUG] getSession: level mismatch, updating session level', {
      sessionId: session.id,
      oldLevel: session.level,
      newLevel: unit.levelName,
    });
    await supabase
      .from('user_quiz_sessions')
      .update({ level: unit.levelName, updated_at: new Date().toISOString() })
      .eq('id', session.id);
    session.level = unit.levelName;
  }
...
async function listTopics(req, res, ctx) {
  ...
  for (const unit of units) {
    ...
    const lockedBlocks = topicActivity
      .filter(a => (now - new Date(a.completed_at).getTime()) < STANDARD_LOCK_HOURS * 60 * 60 * 1000)
      .map(a => a.block_number);
...
async function getBlock(req, res, ctx) {
  ...
  console.log('[DEBUG] getBlock: unit resolved', {
    unitId: unit.id,
    unitName: unit.name,
  ...
  if (!session) {
    // Additional diagnostic: look for any session ignoring level
    const { data: anyRow } = await supabase
      .from('user_quiz_sessions')
      .select('id, level, topic, block_number, user_id')
      .eq('user_id', ctx.userId)
      .eq('topic', unit.name)
      .eq('block_number', blockNum);
    console.error('[DEBUG] getBlock: no session found. Any session ignoring level:', anyRow);
...
async function startSession(body, res, ctx) {
  ...
  console.log('[DEBUG] startSession: resolved unit', {
    unitId: unit.id,
    unitName: unit.name,
    levelName: unit.levelName,
    blockNum,
  ...
  // Check for existing session using the new robust getSession (ignores level mismatch)
  ...
  if (existing) {
    ...
    console.log('[DEBUG] startSession: existing session found, returning resumed', {
  ...
  console.log('[DEBUG] startSession: creating new session');
  ...
  if (insertError) {
    console.error('[DEBUG] startSession: insert failed', insertError);
    throw new SecurityError('Failed to create quiz session', 500);
  }

  console.log('[DEBUG] startSession: session created successfully');
  return res.status(200).json({
    success: true,
    resumed: false,
    tab_switches: 0,
...
async function submitWithSession(body, res, ctx) {
  ...
  const isPerfect = !autoSubmitted && percentage === 100;
  ...
  if (xpEarned > 0) {
    await Promise.all([
      ...
      updateTopicXp(ctx.userId, session.topic, xpEarned),
    ]);
  }

  if (!autoSubmitted) {
    await updateUserRecords(ctx.userId, { score: percentage, timeTakenSeconds: passed ? timeTaken : null, isPerfect });
    await updateDailyChallengeProgress(ctx.userId, (await getUserCurriculumScope(ctx.userId))?.active_group_id, 'blocks_completed', 1).catch(() => null);
  }
