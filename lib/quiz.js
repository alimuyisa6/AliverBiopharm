    ...
    if (req.method === 'GET') {
      ...
      switch (path) {
        case 'get_quiz_topics':
          return await getTopics(req, res, ctx);
        case 'list_quiz_topics':
          return await listTopics(req, res, ctx);
        case 'get_quiz_block':
          return await getBlock(req, res, ctx);
    ...
    if (req.method === 'POST') {
      ...
      switch (path) {
        ...
        case 'quiz_check_answer':
          requireAuth(ctx);
          return await checkAnswer(body, res, ctx);
...
async function getTopics(req, res, ctx) {
  ...
  const completedBlocks = (activity || []).map((item) => item.block_number);
  const now = Date.now();

  const lockedBlocks = (activity || [])
    .filter((item) => now - new Date(item.completed_at).getTime() < STANDARD_LOCK_HOURS * 60 * 60 * 1000)
...
async function listTopics(req, res, ctx) {
  ...
  for (const item of activity || []) {
    if (!activityByTopic.has(item.topic)) {
      activityByTopic.set(item.topic, []);
    }

    activityByTopic.get(item.topic).push(item);
  }
  ...
  for (const unit of units) {
    ...
    const completedBlocks = topicActivity.map((item) => item.block_number);
...
async function getBlock(req, res, ctx) {
  ...
  const answeredSoFar = questions.map((question) => priorAnswers[question.id] || null);
    ...
    if (insertError.code === '23505') {
      ...
      if (concurrentSession) {
        ...
        return res.status(200).json({
          success: true,
          resumed: true,
          tab_switches: concurrentSession.tab_switches || 0,
          max_allowed: concurrentSession.max_tab_switches || 3,
          time_left: Math.max(0, BLOCK_TIME_LIMIT_SECONDS - elapsed)
        });
      }
    }
...
async function checkAnswer(body, res, ctx) {
  ...
  const question = (questions || []).find((item) => String(item.id) === String(question_id));
  ...
  if (answers[question_id]) {
    ...
    return res.status(200).json({
      ...
      correct_answer_text: question[correctOptKey] || null
    });
  }
...
async function submitWithSession(body, res, ctx) {
  ...
  const reviewAnswers = questions.map((question, idx) => {
    const fromSession = priorAnswers[question.id];
  ...
  if (xpEarned > 0) {
    await Promise.all([
      addXp(
        ...
        'quiz_block',
        'quiz',
        { unit_id: unit.id, topic: unit.name, block_number: blockNum },
        unit.id,
        unit.groupId,
        unit.levelId
      ),
  ...
  if (!autoSubmitted) {
    await updateUserRecords(ctx.userId, {
      score: percentage,
      timeTakenSeconds: passed ? timeTaken : null,
