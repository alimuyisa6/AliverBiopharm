...
async function getUnitWithValidation(ctx, unitId, blockNumber = null) {
 ...
 async function getSession(ctx, unit, blockNum) {
  ...
  if (session.group_id !== unit.groupId) {
    return null;
  }
...
function secondsElapsed(session) {
  const startedStr =
    session.started_at ||
    session.created_at ||
    (session.state && session.state.started_at) ||
    session.updated_at;
...
async function listTopics(req, res, ctx) {
 ...
 async function getBlock(req, res, ctx) {
  ...
  if (session.auto_submitted) {
    throw new SecurityError(
      'This block was auto-submitted due to a violation and is locked.',
      403
    );
  }
  ...
  const { data: questions, error } = await supabase
    .from('quiz_questions')
    .select(
      'id, question_text, option_a, option_b, option_c, option_d, difficulty, image_url'
    )
    .eq('unit_id', unit.id)
    .eq('is_active', true)
  ...
  if (error) {
    throw new SecurityError('Failed to fetch questions', 500);
  }
  ...
  const timeLeft = Math.max(
    0,
    BLOCK_TIME_LIMIT_SECONDS - elapsed
  );
 ...
 async function startSession(body, res, ctx) {
  ...
  if (existing) {
    if (existing.auto_submitted) {
      throw new SecurityError(
        'Block locked due to integrity violation',
        403
      );
    }
    ...
    if (
      elapsed >
      BLOCK_TIME_LIMIT_SECONDS + BLOCK_TIME_GRACE_SECONDS
    ) {
      await autoSubmitForTimeout(
        ctx,
        unit,
        blockNum,
        existing
  ...
  if (insertError) {
    if (insertError.code === '23505') {
      const concurrentSession = await getSession(
        ctx,
        unit,
        blockNum
      );
      ...
      if (concurrentSession) {
        if (concurrentSession.auto_submitted) {
          throw new SecurityError(
            'Block locked due to integrity violation',
            403
          );
        ...
        return res.status(200).json({
          ...
          resumed: true,
          tab_switches:
            concurrentSession.tab_switches || 0,
          max_allowed:
            concurrentSession.max_tab_switches || 3,
          time_left: Math.max(
            0,
            BLOCK_TIME_LIMIT_SECONDS - elapsed
          ),
...
async function checkAnswer(body, res, ctx) {
  ...
  const userOption = String(selected_option)
    .trim()
    .toUpperCase();

  if (!['A', 'B', 'C', 'D'].includes(userOption)) {
  ...
  const { data: questions, error } = await supabase
    .from('quiz_questions')
    .select(
      'id, option_a, option_b, option_c, option_d, correct_option, times_answered, times_correct'
    )
    .eq('unit_id', unit.id)
    .eq('is_active', true)
    .order('id', { ascending: true })
  ...
  const question = (questions || []).find(
    q => String(q.id) === String(question_id)
  );

  if (!question) {
    throw new SecurityError(
      'Question not found in this block',
      404
    );
  }
  ...
  if (existingAnswer) {
    const correctOptKey = `option_${String(
  ...
  if (questionUpdateError) {
    throw new SecurityError(
      'Failed to record question statistics',
      500
    );
  }
