 import { supabase, addXp, updateTopicPerformance, enforceClassAccess, canAccessLevel } from './core.js';
...
function validateLevel(level) {
  ...
  if (!VALID_LEVELS.includes(level)) throw new SecurityError('Invalid level', 400);
}
...
async function getQuizTopics(req, res, userId) {
  ...
  // If user is authenticated, check if they can access this level
  if (userId) {
    const canAccess = await canAccessLevel(userId, level);
    // Only enforce if they're not admin
    const isSuperAdminUser = await isSuperAdmin(userId);
    if (!isSuperAdminUser && !canAccess) {
  ...
  const { data: topics, error } = await supabase
    .from('quiz_topics')
    .select('id, topic_name, level, display_order, class_name')
    .eq('level', level)
    .order('display_order', { ascending: true });
  if (error) throw new SecurityError('Failed to fetch topics', 500);

  const { count: levelTotalQuestions, error: levelQuestionError } = await supabase
    .from('quiz_questions')
    .select('id', { count: 'exact', head: true })
    .eq('level', level);
  if (levelQuestionError) throw new SecurityError('Failed to fetch level question count', 500);

  const levelTotalTopics = topics ? topics.length : 0;
  ...
  return res.status(200).json({
    level,
    total_questions: levelTotalQuestions || 0,
    total_topics: levelTotalTopics,
    topics: enriched
  });
}
...
async function getQuizBlock(req, res, userId) {
  ...
  validateLevel(level);
  if (!topic || block_number === undefined) throw new SecurityError('topic and block_number are required', 400);
...
async function startQuizSession(body, res, userId) {
  ...
  if (!topic || block_number === undefined) {
    throw new SecurityError('topic and block_number are required', 400);
  }
...
async function submitQuizWithSession(body, res, userId) {
  ...
  if (!topic || block_number === undefined || !answers) {
    throw new SecurityError('topic, block_number and answers are required', 400);
  }
...
async function addQuizQuestionsBatch(body, res) {
  ...
  validateLevel(level);
  if (!topic || !questions || !Array.isArray(questions)) throw new SecurityError('topic and questions array are required', 400);
