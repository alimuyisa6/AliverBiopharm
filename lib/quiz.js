...
async function startQuizSession(body, res, userId) {
  ...
  const { data: existing, error: fetchError } = await supabase
    ...
    .select('user_id, state, tab_switches, max_tab_switches, auto_submitted, updated_at')
    .eq('user_id', userId)
    .maybeSingle();
  ...
  if (existing) {
    if (existing.auto_submitted) {
      return res.status(200).json({
        success: false,
        auto_submitted: true,
        message: 'This quiz was auto-submitted due to excessive tab switching. Please start a new one.',
...
async function trackTabSwitch(body, res, userId) {
  ...
  const { data: session, error: fetchError } = await supabase
    .from('user_quiz_sessions')
    .select('tab_switches, max_tab_switches, auto_submitted, state')
    .eq('user_id', userId)
    .maybeSingle();
  ...
  if (session.auto_submitted) {
    return res.status(200).json({
      success: false,
      auto_submitted: true,
      message: 'This quiz was already auto-submitted due to excessive tab switching.'
    });
  }
  ...
  let timestamps = [];
  try {
    timestamps = typeof session.state === 'object' && session.state?.tab_switch_timestamps 
...
async function submitQuizWithSession(body, res, userId) {
  ...
  const { data: session, error: fetchError } = await supabase
    .from('user_quiz_sessions')
    .select('tab_switches, max_tab_switches, auto_submitted')
    .eq('user_id', userId)
    .maybeSingle();
  ...
  if (session?.auto_submitted) {
    return res.status(200).json({
      success: false,
      auto_submitted: true,
      message: 'This quiz was auto-submitted due to excessive tab switching.',
...
async function getQuizSessionStatus(req, res, userId) {
  const { data: session, error } = await supabase
    .from('user_quiz_sessions')
    .select('tab_switches, max_tab_switches, auto_submitted, updated_at')
    .eq('user_id', userId)
    .maybeSingle();
