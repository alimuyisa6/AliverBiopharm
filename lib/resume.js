...
async function getResume(req, res, ctx) {
  ...
  const [quizResult, flashcardResult, recallResult, readingResult] = await Promise.all([
    supabase
      ...
      .select('id, level, topic, unit_id, block_number, updated_at')
      .eq('user_id', userId)
      .eq('status', 'active')
  ...
  if (quizResult.data?.updated_at) {
    candidates.push({
      module: 'quiz',
      updated_at: quizResult.data.updated_at,
      payload: {
        session_id: quizResult.data.id,
        unit_id: quizResult.data.unit_id,
        level: quizResult.data.level,
        topic: quizResult.data.topic,
        block_number: quizResult.data.block_number
      }
    });
  }
