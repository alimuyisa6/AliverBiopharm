...
const ALLOWED_CURRENCIES = new Set(['GHS', 'USD']);
...
async function createEscrowHold(body, res, ctx) {
  const { tutor_id, currency, session_date, payment_reference } = body;

  // NOTE: `amount` is deliberately NOT read from the client body. It must
  // come only from the verified payment row below — otherwise a student
  // could reference their own real (small) payment but claim an arbitrary
  // escrow amount, which is exactly the kind of discrepancy a dispute
  // resolution or payout process would trust as ground truth.
  if (!tutor_id || !currency || !payment_reference) {
    throw new SecurityError('tutor_id, currency, and payment_reference required', 400);
  }
  if (tutor_id === ctx.userId) throw new SecurityError('Cannot book a session with yourself', 400);
  if (!ALLOWED_CURRENCIES.has(currency)) throw new SecurityError('Unsupported currency', 400);
  ...
  // FIX: this table's actual status value is 'completed', never 'success' —
  // the previous check rejected every real payment, 100% of the time
  // (confirmed against live data: zero rows had ever made it into
  // escrow_transactions through this endpoint).
  if (payment.status !== 'completed') throw new SecurityError('Payment not completed', 400);

  // FIX: amount now comes from the verified payment record, not the
  // client-supplied body, closing an amount-spoofing hole.
  const verifiedAmount = Number(payment.amount);

  if (!Number.isFinite(verifiedAmount) || verifiedAmount <= 0) {
    throw new SecurityError('Invalid payment amount on record', 400);
  }
  ...
  await createNotification(tutor_id, 'session_payment_held', { amount: verifiedAmount, currency });
  ...
  await auditLog({
    ...
    targetId: escrow.id,
    metadata: { amount: verifiedAmount, currency },
  });
