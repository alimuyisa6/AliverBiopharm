...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      ...
      // SECURITY FIX: feature toggles were previously reachable by ANY admin role
      // (content_manager, resource_manager), inconsistent with how tightly everything
      // else is scoped. Site-wide feature flags now require super_admin.
      ...
      // New: admin MFA enrollment (TOTP). See core.js for the TOTP implementation.
...
async function getAdminUsers(req, res) {
  ...
  return res.status(200).json((data || []).map(a => ({ admin_id: a.admin_id, admin_email: a.admin_email, admin_role: a.admin_role, permissions: a.permissions, is_active: a.is_active, is_locked: a.is_locked || false, last_login: a.last_login || null, mfa_enabled: a.mfa_enabled || false })));
}
...
async function listUsers(req, res) {
  ...
  const [{ data: admins }, { data: restrictions }, { data: presence }] = await Promise.all([
  ...
  // Batch profile-name lookups instead of one auth.admin call per user (N+1).
...
async function updateUserRole(body, res, ctx) {
  ...
  // SECURITY FIX: previously an unrecognized role string silently no-op'd and still
  // returned { success: true }, which is misleading and could mask a client bug or a
  // probing attempt. Now validated against an explicit allow-list.
...
async function deleteQuizTopic(body, res, ctx) {
  ...
  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: 'delete_quiz_topic', targetType: 'quiz_topic', targetId: `${level}:${topic}`, metadata: {} });
  return res.status(200).json({ success: true });
}
...
async function confirmMfa(body, res, ctx) {
  ...
  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: 'mfa_enabled', targetType: 'admin', targetId: ctx.userId, metadata: {} });
  return res.status(200).json({ success: true });
}
...
async function disableMfa(body, res, ctx) {
  const { userId } = body;
  if (!userId) throw new SecurityError('userId required', 400);
  await supabase.from('admin_master').update({ mfa_enabled: false, mfa_secret: null, mfa_secret_pending: null }).eq('admin_id', userId);
  await auditLog({ actorId: ctx.userId, actorRole: ctx.adminData?.admin_role, action: 'mfa_disabled', targetType: 'admin', targetId: userId, metadata: {} });
  return res.status(200).json({ success: true });
}
