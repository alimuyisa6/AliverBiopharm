...
const MFA_EXEMPT_PATHS = new Set(['setup_mfa', 'confirm_mfa']);

export async function handler(req, res, path, ctx) {
  requireAdmin(ctx);
  if (!MFA_EXEMPT_PATHS.has(path)) requireMfaEnrolled(ctx);
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
   ...
   case 'approve_resource': requireAdmin(ctx); return approveResource(body, res, ctx);
