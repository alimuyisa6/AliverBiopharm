...
const CONTENT_TYPE_CONFIG = {
  note: {
    table: 'notes',
    select: '*',
    activeCondition: (q) => q.eq('is_active', true),
    idColumn: 'id',
    slugColumn: 'slug',
  },
  article: {
    table: 'articles',
    select: 'id, title, slug, excerpt, content, featured_image_url, category, tags, author_id, author_name, status, is_featured, is_premium, read_time_minutes, view_count, published_at, created_at, updated_at, unit_id',
    activeCondition: (q) => q.eq('status', 'published'),
    idColumn: 'id',
    slugColumn: 'slug',
  },
  video: {
    table: 'videos',
    select: 'id, title, description, slug, provider, video_url, embed_url, thumbnail_url, duration_seconds, transcript, is_premium, is_active, view_count, display_order, created_at, updated_at, unit_id',
    activeCondition: (q) => q.eq('is_active', true),
    idColumn: 'id',
    slugColumn: 'slug',
  },
...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'links':       return getInternalLinks(req, res, ctx);
    case 'related':     return getRelatedContent(req, res, ctx);
...
async function getContentDetail(req, res, ctx) {
  ...
  if (!config) throw new SecurityError('Unsupported content type', 400);

  let query = supabase
    .from(config.table)
    .select(config.select)
    .eq(config.idColumn, id || slug)
    .maybeSingle();

  const { data: item, error } = await query;
  if (error || !item) throw new SecurityError('Content not found', 404);
...
async function getInternalLinks(req, res, ctx) {
  ...
  const { data: links } = await supabase
    .from('content_links')
    .select(
      'link_text, target_reference_id, content_references!inner(slug, path, title, content_type)'
    )
    .eq('source_type', type)
    .eq('source_id', id)
    .order('position');
...
async function getRelatedContent(req, res, ctx) {
  ...
  if (ref) {
    const { data: explicit } = await supabase
      ...
      .select(
        'relationship_type, target_reference_id, content_references!inner(slug, path, title, content_type)'
      )
      ...
      if (ctx.authenticated) {
        ...
        if (scope?.active_group_id) {
          ...
          if (allowedIds.length) {
            query = query.in('unit_id', allowedIds);
          }
        }
      }

      const { data: sameUnit } = await query;
      return res.status(200).json(
        (sameUnit || []).map(n => ({
...
async function getCollection(req, res) {
  ...
  const { data: items } = await supabase
    ...
    .select(
      'position, reference_id, content_references(slug, path, title, content_type)'
    )
