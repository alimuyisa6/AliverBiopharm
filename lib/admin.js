...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'messages':                requireSuperAdmin(ctx); return getMessages(req, res);
    case 'get_admin_users':         requireSuperAdmin(ctx); return getAdminUsers(req, res);
    case 'list_users':              requireSuperAdmin(ctx); return listUsers(req, res);
    ...
    case 'get_newsletter_subscribers':requireSuperAdmin(ctx); return getNewsletterSubscribers(req, res);
    case 'get_donations':           requireSuperAdmin(ctx); return getDonations(req, res);
    case 'get_page_activity':       requireSuperAdmin(ctx); return getPageActivity(req, res, ctx);
    ...
    case 'get_user_activity_trace': requireSuperAdmin(ctx); return getUserActivityTrace(req, res, ctx);
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    ...
    case 'confirm_passkey':         return confirmPasskey(body, res, ctx);
    case 'disable_passkey':         requireSuperAdmin(ctx); return disablePasskey(body, res, ctx);
    case 'approve_teacher':         requireSuperAdmin(ctx); return approveTeacher(body, res, ctx);
    case 'approve_resource':        requireSuperAdmin(ctx); return approveResource(body, res, ctx);
    case 'reject_teacher':          requireSuperAdmin(ctx); return rejectTeacher(body, res, ctx);
    default: throw new SecurityError('Invalid action', 400);
  }
}
...
async function getStats(req, res) {
  ...
  return res.status(200).json({ resources, pendingSubmissions, messages, pendingTeachers });
}
...
async function getAdminUsers(req, res) {
  const { data } = await supabase.from('admin_master').select('admin_id, admin_email, admin_role, permissions, is_active, is_locked, last_login, mfa_enabled, passkey_enabled');
  return res.status(200).json(data || []);
}
...
async function getNewsletterSubscribers(req, res) {
  ...
  const emails = allUsers.filter(u => userIds.includes(u.id)).map(u => ({ email: u.email, created_at: u.created_at }));
...
async function getPageActivity(req, res, ctx) {
  ...
  for (const act of (data || [])) {
    const activity = { ...act };
    if (act.user_id) {
      activity.user_email = await getUserProfileName(act.user_id) || 'Unknown';
    }
...
async function getUserActivityTrace(req, res, ctx) {
  ...
  for (const [key, group] of groups.entries()) {
    ...
    for (let i = 0; i < timestamps.length; i++) {
      let count = 1;
      for (let j = i + 1; j < timestamps.length && timestamps[j] - timestamps[i] <= 60000; j++) {
        count++;
      }
      if (count >= 10) {
        burst = true;
...
async function updateUserLock(body, res, ctx) {
  ...
  const actorAdminId = resolveActorAdminId(ctx);

  if (lock) {
    await supabase.from('admin_master').update({
      is_locked: true,
      lock_reason: reason || 'Locked by admin',
      locked_by: actorAdminId,
...
async function disableMfa(body, res, ctx) {
  ...
  if (!userId) throw new SecurityError('userId required', 400);
  if (userId === ctx.userId) throw new SecurityError('You cannot disable your own MFA from here. Sign in with a different super admin account to disable it.', 400);
...
async function confirmPasskey(body, res, ctx) {
  const { data: passkeys, error } = await supabasePasskeyAdmin.auth.admin.passkey.listPasskeys({ userId: ctx.userId });
  ...
  return res.status(200).json({ success: true, passkey_count: passkeys.length });
}

async function disablePasskey(body, res, ctx) {
  const { userId, reason } = body;
  if (!userId) throw new SecurityError('userId required', 400);
  if (userId === ctx.userId) throw new SecurityError('You cannot disable your own passkey requirement from here. Sign in with a different super admin account to disable it.', 400);
  if (!reason || typeof reason !== 'string' || reason.trim().length < 5) {
    throw new SecurityError('A reason of at least 5 characters is required', 400);
  }

  const { data: passkeys } = await supabasePasskeyAdmin.auth.admin.passkey.listPasskeys({ userId });
  for (const pk of (passkeys || [])) {
    await supabasePasskeyAdmin.auth.admin.passkey.deletePasskey({ userId, passkeyId: pk.id });
  }
  ...
  await auditLog({
    ...
    targetType: 'admin',
    targetId: userId,
    metadata: { reason, revoked_count: (passkeys || []).length },
  });
  return res.status(200).json({ success: true });
}
...
async function approveResource(body, res, ctx) {
  ...
  const { data: submission } = await supabase.from('resource_submissions').select('*').eq('id', submissionId).maybeSingle();
