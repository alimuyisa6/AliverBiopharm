...
async function getContentCounts(unitIds) {
  ...
  const [quizRes, recallRes, notesWithFileRes, blocksRes, notesTotalRes] = await Promise.all([
    supabase
      .from('quiz_questions')
      .select('unit_id')
      .in('unit_id', unitIds)
  ...
  for (const row of notesWithFileRes.data || []) {
  ...
  for (const row of notesTotalRes.data || []) {
