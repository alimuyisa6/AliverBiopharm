...
async function getContentCounts(unitIds) {
  ...
  for (const row of notesRes.data || []) {
    if (counts[row.unit_id]) counts[row.unit_id].pdf_count += 1;
  }
...
async function getUnitProgress(userId, unitIds) {
  if (!userId || !unitIds.length) return {};
  ...
  for (const unitId of unitIds) {
    completedBlocks[unitId] = new Set();
  }

  for (const row of [...(legacyRes.data || []), ...(attemptsRes.data || [])]) {
    if (completedBlocks[row.unit_id]) {
      completedBlocks[row.unit_id].add(row.block_number);
    }
  }
  ...
  for (const unitId of unitIds) {
    progress[unitId] = completedBlocks[unitId].size;
  }
...
async function getUnits(req, res, ctx) {
  ...
  const [contentCounts, unitProgress] = await Promise.all([
    getContentCounts(unitIds),
    ctx?.authenticated ? getUnitProgress(ctx.userId, unitIds) : Promise.resolve({})
  ]);

  const units = (data || []).map((unit) => {
    const counts = contentCounts[unit.id] || { quiz_question_count: 0, recall_question_count: 0, pdf_count: 0, total_blocks: 0 };
    const completedBlocks = unitProgress[unit.id] || 0;
    const progressPercent = counts.total_blocks > 0
      ? Math.round((completedBlocks / counts.total_blocks) * 100)
      : 0;
