...
async function getUnits(req, res) {
  ...
  let query = supabase
    ...
    .select('*, curriculum_groups!inner(id, name, level_id)')
    .eq('is_active', true)
    .order('display_order', { ascending: true });

  if (group_id) query = query.eq('group_id', group_id);
  if (level_id) query = query.eq('curriculum_groups.level_id', level_id);
...
async function getUnit(req, res) {
  ...
  return res.status(200).json({
    ...
    level: level ? { id: level.id, display_name: level.display_name, group_label: level.group_label, unit_label: level.unit_label, kind: level.kind } : null
  });
}
...
export async function resolveUnitTitle(unitId) {
  ...
  return {
    ...
    full_title: level ? `${group?.name || ''} — ${level.unit_label}: ${data.name}` : data.name
  };
}
...
export async function getUserCurriculumScope(userId) {
  ...
  if (profile.role === 'teacher') {
    ...
    if (profile.approved_track === 'ALL') return { level: null, groupName: null, showAll: true, pending: false };
    return { level: profile.approved_track || profile.track, groupName: profile.class_name, showAll: false, pending: false };
  }

  return { level: profile.track, groupName: profile.class_name, showAll: false, pending: false };
}

export async function resolveGroupRank(levelId, groupName) {
  const { data, error } = await supabase
    .from('curriculum_groups')
...
export function getLevelRank(levelId) {
  const ranks = { 'O-Level': 0, 'A-Level': 1, 'Pharmacy': 2 };
  return ranks[levelId] ?? null;
}

export function resolveLevelChangeType(currentLevel, requestedLevel) {
  const currentRank = getLevelRank(currentLevel);
  const requestedRank = getLevelRank(requestedLevel);
  if (currentRank === null || requestedRank === null) return 'invalid';
  if (requestedRank > currentRank) return 'upgrade';
  return 'lateral_or_downgrade';
}
...
async function createGroup(body, res) {
  ...
  const { data, error } = await supabase
    .from('curriculum_groups')
    .insert({ id, level_id, name, description: description || null, icon: icon || null, sequence_order })
    .select()
    .single();
...
async function updateGroup(body, res) {
  const { id, name, description, icon, sequence_order, is_active } = body;
  if (!id) throw new SecurityError('id required', 400);
  const updates = {};
  ...
  const { error } = await supabase.from('curriculum_groups').update(updates).eq('id', id);
  if (error) throw new SecurityError('Failed to update group', 500);
...
async function deleteGroup(body, res) {
  const { id } = body;
  if (!id) throw new SecurityError('id required', 400);
  const { error } = await supabase.from('curriculum_groups').delete().eq('id', id);
  if (error) throw new SecurityError('Failed to delete group', 500);
  return res.status(200).json({ success: true });
}
...
async function createUnit(body, res) {
  ...
  const { data, error } = await supabase
    .from('curriculum_units')
    .insert({
      group_id, name, code: code || null, icon: icon || 'fa-book',
      display_order: display_order || 0, is_hard_topic: !!is_hard_topic
    })
    .select()
    .single();
...
async function updateUnit(body, res) {
  const { id, name, code, icon, display_order, is_hard_topic, is_active } = body;
  if (!id) throw new SecurityError('id required', 400);
  const updates = {};
  if (name !== undefined) updates.name = name;
  ...
  if (is_hard_topic !== undefined) updates.is_hard_topic = is_hard_topic;
  if (is_active !== undefined) updates.is_active = is_active;

  const { error } = await supabase.from('curriculum_units').update(updates).eq('id', id);
  if (error) throw new SecurityError('Failed to update unit', 500);
  return res.status(200).json({ success: true });
}
...
async function deleteUnit(body, res) {
  ...
  if (error) throw new SecurityError('Failed to delete unit', 500);
  return res.status(200).json({ success: true });
}

async function setUnitPremium(body, res) {
  const { unit_id, is_premium } = body;
  if (!unit_id || is_premium === undefined) throw new SecurityError('unit_id and is_premium required', 400);
  const { error } = await supabase.from('curriculum_units').update({ is_premium }).eq('id', unit_id);
  if (error) throw new SecurityError('Failed to update premium status', 500);
  return res.status(200).json({ success: true });
}
...
async function setBlockPremium(body, res) {
  ...
  if (error) throw new SecurityError('Failed to update block premium status', 500);
  return res.status(200).json({ success: true });
}

async function ensureBlocks(body, res) {
  const { unit_id, total_blocks } = body;
  if (!unit_id || !total_blocks) throw new SecurityError('unit_id and total_blocks required', 400);
  ...
  for (let i = 0; i < total_blocks; i++) {
    if (!existingNumbers.has(i)) rows.push({ unit_id, block_number: i, is_premium: false });
  }
