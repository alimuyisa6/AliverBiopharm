...
async function createGroup(body, res) {
  ...
  if (!id || !level_id || !name || sequence_order === undefined) throw new SecurityError('id, level_id, name, sequence_order required', 400);
  const { data, error } = await supabase.from('curriculum_groups').insert({ id, level_id, name, description, icon, sequence_order }).select().single();
...
async function createUnit(body, res) {
  ...
  const { data, error } = await supabase.from('curriculum_units').insert({ group_id, name, code, icon, display_order, is_hard_topic: !!is_hard_topic }).select().single();
...
async function setBlockPremium(body, res) {
  ...
  if (!unit_id || block_number === undefined || is_premium === undefined) throw new SecurityError('unit_id, block_number, is_premium required', 400);
  await supabase.from('curriculum_unit_blocks').update({ is_premium }).eq('unit_id', unit_id).eq('block_number', block_number);
  return res.status(200).json({ success: true });
}
...
async function ensureBlocks(body, res) {
  ...
  const { data: existing } = await supabase.from('curriculum_unit_blocks').select('block_number').eq('unit_id', unit_id);
