...
async function getContentCounts(unitIds) {
  ...
  const [quizRes, recallRes, notesRes] = await Promise.all([
    supabase
      .from('quiz_questions')
      .select('unit_id')
      .in('unit_id', unitIds)
  ...
  for (const unitId of unitIds) {
    counts[unitId] = { quiz_question_count: 0, recall_question_count: 0, pdf_count: 0 };
  }
...
async function getUnits(req, res) {
  ...
  const units = (data || []).map((unit) => ({
    ...
    quiz_question_count: contentCounts[unit.id]?.quiz_question_count || 0,
    recall_question_count: contentCounts[unit.id]?.recall_question_count || 0,
    pdf_count: contentCounts[unit.id]?.pdf_count || 0
  }));
