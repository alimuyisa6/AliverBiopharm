...
// Presentation-only labels. Not a source of truth for valid classes/programmes —
// that always comes from the DB via getValidClassNames().
const LEVEL_CONFIG = {
  'O-Level': { display_name: 'Secondary School Biology', class_label: 'Class' },
  'A-Level': { display_name: 'Advanced Secondary Biology', class_label: 'Class' },
  'Pharmacy': { display_name: 'Pharmacy & Pharmaceutical Sciences', class_label: 'Programme' }
};
...
// Single source of truth for which class_name/programme values are valid for a track.
// O-Level/A-Level pull from class_sequence; Pharmacy pulls from pharmacy_programs.
async function getValidClassNames(track) {
  if (track === 'Pharmacy') {
    ...
    if (error) throw new SecurityError('Failed to load pharmacy programs', 500);
    return (data || []).map(p => p.program_name);
  }
  ...
  if (error) throw new SecurityError('Failed to load class sequence', 500);
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'GET' && path === 'get_profile') {
    ...
    const classOptions = base.track ? await getValidClassNames(base.track) : [];
  ...
  if (req.method === 'POST' && path === 'save_onboarding') {
    ...
    if (!validClassNames.includes(class_name)) {
      throw new SecurityError('Invalid track/class combination', 400);
    }
  ...
  if (req.method === 'POST' && path === 'request_level_change') {
    ...
    if (!validClassNames.includes(requested_class)) {
      throw new SecurityError('Invalid requested_class for the selected track', 400);
    }
  ...
  if (req.method === 'POST' && path === 'review_level_change') {
    ...
    if (action === 'approve') {
      // Re-validate at approval time in case class_sequence/pharmacy_programs changed
      // since the request was submitted.
      const validClassNames = await getValidClassNames(reqRow.requested_track);
      if (!validClassNames.includes(reqRow.requested_class)) {
        throw new SecurityError('Requested class is no longer valid for this track', 409);
      }
...
export async function canAccessContent(userId, contentTrack, contentClassName) {
  ...
  if (!profile.onboarding_completed && profile.role !== 'teacher') {
