...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST' && path === 'save_onboarding') {
    ...
    if (existing?.onboarding_completed) {
      throw new SecurityError('Onboarding already completed. Use update_class to change your class, or request_level_change to change level.', 400);
    }

    if (!role || !track || !class_name) throw new SecurityError('role, track and class_name required', 400);
  ...
  if (req.method === 'POST' && path === 'update_class') {
    ...
    if (!profile || !profile.onboarding_completed) throw new SecurityError('Complete onboarding first', 400);

    const updates = { updated_at: new Date().toISOString() };

    if (class_name !== undefined) {
      const validClassNames = await getValidClassNames(profile.track);
      if (!validClassNames.includes(class_name)) throw new SecurityError('Invalid class for your level', 400);
      const { data: group } = await supabase
        .from('curriculum_groups')
        .select('id')
        .eq('level_id', profile.track)
    ...
    if (profile.role === 'teacher') {
      if (contribute_class_name !== undefined) {
        ...
        if (!validNames.includes(contribute_class_name)) throw new SecurityError('Invalid class for contribute track', 400);
    ...
    if (Object.keys(updates).length === 1) throw new SecurityError('Nothing to update', 400);
  ...
  if (req.method === 'POST' && path === 'switch_class') {
    ...
    const { data: group } = await supabase
      ...
      .eq('id', group_id)
      .maybeSingle();
    if (!group || !group.is_active) throw new SecurityError('Class not found', 404);
    ...
    if (profile.role === 'student' && profile.track !== group.level_id) {
      throw new SecurityError('Cannot switch to a different level. Use request_level_change instead.', 403);
    }
    if (profile.role === 'teacher') {
      if (!profile.is_approved_teacher) throw new SecurityError('Teacher approval pending', 403);
      if (profile.approved_track !== 'ALL' && profile.approved_track !== group.level_id) {
        throw new SecurityError('Not approved for this level', 403);
      }
    }
