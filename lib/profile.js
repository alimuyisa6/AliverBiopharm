...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST') {
    if (path === 'track_download' || path === 'toggle_bookmark' || path === 'track_view' || path === 'rate_paper' || path === 'delete_review' || path === 'save_preset' || path === 'delete_preset') {
      requireAuth(ctx);
...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'get_presets':        return getFilterPresets(req, res, ctx);
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    ...
    case 'save_preset':       return saveFilterPreset(body, res, ctx);
    case 'delete_preset':     return deleteFilterPreset(body, res, ctx);
    default: throw new SecurityError('Invalid action', 400);
  }
}
...
async function getPapers(req, res, ctx) {
  ...
  const { unit_id, subject, year, exam_board, paper_type, search, sort, page = 1, limit = 20 } = req.query;
  ...
  if (search) {
    ...
    if (safeSearch) {
      query = query.or(`title.ilike.%${safeSearch}%,subject.ilike.%${safeSearch}%`);
    }
  }
  ...
  switch (sort) {
    case 'year_asc':
      ...
      break;
    case 'downloads':
      query = query.order('download_count', { ascending: false, nullsLast: true });
      break;
    case 'rating':
  ...
  query = query.order('title', { ascending: true }).range(offset, offset + parseInt(limit) - 1);
...
async function getFilterPresets(req, res, ctx) {
  const { data, error } = await supabase
    .from('paper_filter_presets')
  ...
  if (error) {
    console.error('[PAST_PAPER_PRESETS_FETCH_ERROR]', JSON.stringify({
      user_id: ctx.userId,
      error: error.message
    }));
    throw new SecurityError('Failed to fetch presets', 500);
  }

  return res.status(200).json(data || []);
}

async function saveFilterPreset(body, res, ctx) {
  const { name, filters } = body;

  if (!name || !filters) {
    throw new SecurityError('name and filters required', 400);
  }

  const trimmedName = name.trim();

  if (trimmedName.length < 2 || trimmedName.length > 50) {
    throw new SecurityError('Preset name must be 2-50 characters', 400);
  }

  if (typeof filters !== 'object' || Array.isArray(filters)) {
    throw new SecurityError('filters must be an object', 400);
  }

  const sanitizedFilters = {
    subject: typeof filters.subject === 'string' ? filters.subject.trim() : '',
    year: typeof filters.year === 'string' ? filters.year.trim() : '',
    exam_board: typeof filters.exam_board === 'string' ? filters.exam_board.trim() : '',
    paper_type: typeof filters.paper_type === 'string' ? filters.paper_type.trim() : ''
  };
  ...
  if (error) {
    console.error('[PAST_PAPER_PRESET_SAVE_ERROR]', JSON.stringify({
      user_id: ctx.userId,
      name: trimmedName,
      error: error.message
    }));
    throw new SecurityError('Failed to save preset', 500);
  }

  return res.status(200).json(data);
}

async function deleteFilterPreset(body, res, ctx) {
  const { preset_id } = body;

  if (!preset_id) throw new SecurityError('preset_id required', 400);
  ...
  if (error) {
    console.error('[PAST_PAPER_PRESET_DELETE_ERROR]', JSON.stringify({
      user_id: ctx.userId,
      preset_id,
      error: error.message
    }));
    throw new SecurityError('Failed to delete preset', 500);
  }
