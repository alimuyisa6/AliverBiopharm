...
async function getUserCurriculumScope(userId) {
  ...
  if (!profile || !profile.active_level_id || !profile.active_group_id) {
    return null;
  }
  ...
  if (!level || !group || group.level_id !== level.id) {
    return null;
  }
  ...
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'get_profile': {
        ...
        let classOptions = [];
        if (scope) {
          classOptions = await getValidGroupNames(scope.active_level_id);
        }
        ...
        return res.status(200).json({
          ...
          track_id: scope?.active_level_id ?? profile.track,
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      ...
      case 'switch_class': {
        ...
        const scope = await getUserCurriculumScope(ctx.userId);
        if (!scope) {
          throw new SecurityError('Failed to retrieve curriculum scope after switching.', 500);
        }
