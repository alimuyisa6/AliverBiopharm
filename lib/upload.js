...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST') {
    return handlePost(path, req, res, ctx);
  }
  if (req.method === 'DELETE') {
    return handleDelete(path, req, res, ctx);
  }
...
async function handlePost(path, req, res, ctx) {
  if (path === 'file') {
...
async function handleDelete(path, req, res, ctx) {
  if (path === 'file') {
...
async function deleteFile(req, res, ctx) {
  ...
  await supabase.from('user_files').update({ is_active: false }).eq('id', file_id);
