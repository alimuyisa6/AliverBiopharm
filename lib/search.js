...
async function unitIdsForGroup(groupId) {
...
async function globalSearch(req, res, ctx) {
  ...
  let levelUnitIds = null;
  let groupUnitIds = null;

  if (groupId) {
    const ids = await unitIdsForGroup(groupId);
    groupUnitIds = ids.length ? ids : ['__none__'];
  }

  if (!showAll && userLevel) {
    levelUnitIds = await unitIdsForLevel(userLevel);
  }

  const allowedUnitIds = groupUnitIds ? groupUnitIds : levelUnitIds;
  ...
  if (allowedUnitIds) {
    const { data: papers } = await commonQuery('past_papers', 'id, title, subject, year', 'title', q => q.eq('is_active', true).in('unit_id', allowedUnitIds));
    if (papers) results.push(...papers.map(p => ({ type: 'past_paper', id: p.id, title: p.title, preview: `${p.subject} ${p.year}` })));

    const { data: quizzes } = await commonQuery('curriculum_units', 'id, name', 'name', q => q.eq('is_active', true).in('id', allowedUnitIds));
    if (quizzes) results.push(...quizzes.map(u => ({ type: 'curriculum_unit', id: u.id, title: u.name, preview: 'Quiz topic' })));

    const { data: flashcards } = await commonQuery('flashcard_decks', 'id, title, description', 'title', q => q.eq('is_active', true).in('unit_id', allowedUnitIds));
