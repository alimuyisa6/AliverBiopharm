...
import { createIdempotencyKey, getStoredIdempotencyResponse } from './idempotency.js';
...
export async function getQuizTopics(req, res, ctx) {
  ...
  const { data: activity } = await supabase
    .from('quiz_attempts')
    .select('block_number, passed, submitted_at, auto_submitted')
...
export async function listQuizTopics(req, res, ctx) {
  ...
  for (const row of countRows || []) {
    countMap.set(row.unit_id, Number(row.count) || 0);
  }
...
export async function startSession(body, res, ctx) {
  ...
  await supabase.rpc('atomic_create_quiz_attempt', {
    p_user_id: ctx.userId,
    p_unit_id: unit.id,
    p_group_id: unit.groupId,
    p_level_id: unit.levelId || null,
    p_session_id: insertedSession.session_id,
