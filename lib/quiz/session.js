...
export async function getQuizTopics(req, res, ctx) {
  ...
  const {
    ...
    .from('quiz_attempts')
    .select(
      'block_number, submitted_at, status, auto_submitted'
    )
  ...
  return res.status(200).json({
    ...
    all_done:
      totalBlocks > 0 &&
      passedBlocks.length === totalBlocks
  });
}
...
export async function getQuizBlock(
  ...
  if (
    ...
    throw new SecurityError(
      'Some session questions are no longer available',
      409
    );
  }
...
export async function checkDailyRetry(
  ...
  if (integrity.locked) {
    return res.status(200).json({
      can_retry: false,
      reason:
        `Integrity lock: try again in ${integrity.hoursLeft} hour(s)`,
      integrity_lock: true,
      hours_left:
        integrity.hoursLeft,
      locked_until:
        integrity.locked_until
    });
  }
  ...
  const {
    ...
    .from('quiz_attempts')
    .select(
      'submitted_at, status, auto_submitted'
    )
    .eq('user_id', ctx.userId)
    .eq('unit_id', unit.id)
    .eq(
  ...
  if (error) {
    ...
    throw new SecurityError(
      'Unable to check retry availability',
      500
    );
  }
...
export async function startSession(
  ...
  if (existing) {
    if (
      ...
      const autoSubmittedAt =
        new Date(
          existing.state?.auto_submitted_at ||
          existing.updated_at
        ).getTime();
      ...
      if (
        Number.isFinite(autoSubmittedAt) &&
        Date.now() - autoSubmittedAt <
          lockDurationMs
      ) {
        throw new SecurityError(
          'This block was auto-submitted due to an integrity violation.',
          403
        );
      }
      ...
      if (
        ...
        throw new SecurityError(
          'This quiz block has already been submitted.',
          409
        );
      }
...
async function getIntegrityLock(
  userId,
  unitId,
  blockNumber
) {
  ...
  if (error) {
    console.error(
      '[QUIZ_INTEGRITY_LOCK_LOOKUP_ERROR]',
      JSON.stringify({
        user_id: userId,
        unit_id: unitId,
        block_number:
    ...
    throw new SecurityError(
      'Unable to verify quiz integrity status',
      500
    );
  }
...
async function expireSession(
  ...
  try {
    ...
    console.error(
      '[QUIZ_AUTO_SUBMIT_NOTIFICATION_ERROR]',
      JSON.stringify({
        user_id:
          ctx.userId,
        error:
...
async function getNextAttemptNumber(
  ...
  if (error) {
    ...
    throw new SecurityError(
      'Unable to determine quiz attempt number',
      500
    );
  }
