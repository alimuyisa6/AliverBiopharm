...
export async function listQuizTopics(req, res, ctx) {
  ...
  if (!scope?.active_group_id) {
    console.error('[QUIZ_TOPICS] No active curriculum group', ctx.userId);
  ...
  if (!units?.length) {
    console.error('[QUIZ_TOPICS] No units found for group', scope.active_group_id);
  ...
  if (questionError) {
    console.error('[QUIZ_TOPICS] question count error', questionError.message);
    return res.status(200).json({ level: scope.active_level_name || null, topics: [] });
  }
  ...
  console.error('[QUIZ_TOPICS] result', JSON.stringify({
    user_id: ctx.userId,
    group_id: scope.active_group_id,
    unit_count: units.length,
    topic_count: topics.length,
    question_total: countMap.size
  }));
