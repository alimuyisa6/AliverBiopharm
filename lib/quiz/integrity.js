...
export async function trackTabSwitch(body, res, ctx) {
  ...
  if (idempotency_key) {
    const stored = await getStoredIdempotencyResponse(ctx.userId, 'quiz_tab_switch', idempotency_key);
  ...
  if (newCount >= maxAllowed) {
    ...
    if (idempotency_key) {
      await createIdempotencyKey(ctx.userId, 'quiz_tab_switch', idempotency_key, response, 200);
    }
...
export async function recordHeartbeat(body, res, ctx) {
  ...
  const { session_id, client_timestamp, idempotency_key } = body;
  ...
  if (idempotency_key) {
    const stored = await getStoredIdempotencyResponse(ctx.userId, 'quiz_heartbeat', idempotency_key);
  ...
  if (!session) throw new SecurityError('Quiz session not found', 404);

  await supabase.rpc('atomic_record_quiz_heartbeat', {
  ...
  if (idempotency_key) {
    await createIdempotencyKey(ctx.userId, 'quiz_heartbeat', idempotency_key, response, 200);
  }
