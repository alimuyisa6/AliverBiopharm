  ...
  if (mappings && mappings.length > 0) {
    ...
    if (quizIds.length > 0) {
      const { data: quizzes, error: quizError } = await supabase
        ...
        .eq('is_active', true)
        .limit(5);
      if (!quizError) content.quizzes = quizzes || [];
    }
    ...
    if (noteIds.length > 0) {
      const { data: notes, error: noteError } = await supabase
        ...
        .eq('level', level)
        .limit(5);
      if (!noteError) content.notes = notes || [];
    }
    ...
    if (flashcardIds.length > 0) {
      const { data: decks, error: deckError } = await supabase
        ...
        .eq('level', level)
        .limit(5);
      if (!deckError) content.flashcards = decks || [];
    }
    ...
    if (paperIds.length > 0) {
      const { data: papers, error: paperError } = await supabase
        ...
        .eq('is_active', true)
        .limit(5);
      if (!paperError) content.past_papers = papers || [];
    }
    ...
    if (recallIds.length > 0) {
      ...
      if (numericIds.length > 0) {
        const { data: recall, error: recallError } = await supabase
          ...
          .eq('is_active', true)
          .limit(5);
        if (!recallError) content.recall_questions = recall || [];
      }
    }
  }
