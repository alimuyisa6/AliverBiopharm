...
async function getTerm(req, res) {
  ...
  if (mappings && mappings.length > 0) {
    // Map old content types to new tables where needed
    const typeMap = {
      'quiz': 'quizzes',
    ...
    // Distribute results back to content categories
    // We need to know which table each query came from. We'll re-iterate tableIds.
    const tableResults = {};
    let idx = 0;
    for (const [table, ids] of Object.entries(tableIds)) {
      if (ids.length === 0) continue;
