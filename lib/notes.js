...
async function getNoteDetail(req, res, ctx) {
  ...
  const { data: { user } } = ctx.authenticated ? await supabase.auth.admin.getUserById(ctx.userId) : { data: { user: null } };
...
async function getRelatedNotes(req, res) {
  ...
  const { data: source } = await supabase.from('notes').select('unit_id, display_order').eq('id', note_id).maybeSingle();
  if (!source) return res.status(200).json(explicitResults);

  const { data: sameUnit } = await supabase
    .from('notes')
    .select('id, slug, title, content_preview, read_time, display_order')
...
async function getToc(req, res) {
  const { note_id } = req.query;
  if (!note_id) throw new SecurityError('note_id required', 400);
  const { data, error } = await supabase.from('notes').select('toc').eq('id', note_id).maybeSingle();
  if (error) throw new SecurityError('Failed to fetch table of contents', 500);
...
async function getReadingProgress(req, res, ctx) {
  ...
  const { data, error } = await supabase
    ...
    .filter('metadata->>kind', 'eq', 'reading_progress')
    .maybeSingle();
...
async function saveReadingProgress(body, res, ctx) {
  ...
  const wasCompleted = existing?.metadata?.completed;
  ...
  if (existing) {
    ...
    await supabase.from('user_interactions').insert({
      ...
      resource_id: null,
      value: scroll_percentage,
      metadata: { note_id, kind: 'reading_progress', scroll_position: scroll_position || 0, time_spent: time_spent || 0, completed: nowCompleted }
    });
  }
...
async function updateReadingStreak(userId) {
  ...
  if (stats.last_read_date === today) {
    await supabase.from('note_reading_stats').update({ notes_read_count: stats.notes_read_count + 1 }).eq('user_id', userId);
    return;
  }
...
async function getDownloadUrl(req, res, ctx) {
  ...
  const { data: note } = await supabase.from('notes').select('id, file_url, is_premium, unit_id, title').eq('id', id).maybeSingle();
  if (!note || !note.file_url) throw new SecurityError('Note not found or has no downloadable file', 404);
  ...
  if (!access.allowed) {
    if (access.reason === 'premium_locked') await recordAbuseProbe(ctx.userId, 'premium_probe_note_download');
    throw new SecurityError(access.reason === 'premium_locked' ? 'This note requires premium access' : 'Access denied', 403);
  }

  await supabase.from('notes').update({ download_count: note.download_count + 1 }).eq('id', id);
  await createNotification(ctx.userId, 'note_downloaded', {}).catch(() => {});
