...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'nav_list': return getNavList(req, res, ctx);
...
async function getDownloadUrl(req, res, ctx) {
  ...
  const { data: note } = await supabase.from('notes').select('id, file_url, is_premium, unit_id, download_count').eq('id', id).maybeSingle();
