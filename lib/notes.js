...
const debugLog = (section, data) => {
  if (DEBUG) {
    console.log(`[NOTES_DEBUG][${section}]`, JSON.stringify(data, null, 2));
  }
};

export async function handler(req, res, path, ctx) {
  debugLog('HANDLER', { method: req.method, path, userId: ctx?.userId });
...
async function handleGet(path, req, res, ctx) {
  debugLog('HANDLE_GET', { path, query: req.query, userId: ctx?.userId });
...
async function handlePost(path, body, req, res, ctx) {
  debugLog('HANDLE_POST', { path, body, userId: ctx?.userId });
...
async function getActiveUnitIds(ctx) {
  debugLog('GET_ACTIVE_UNITS_START', { userId: ctx.userId });
  
  const scope = await getUserCurriculumScope(ctx.userId);
  debugLog('USER_SCOPE', { scope });

  if (!scope?.active_group_id) {
    debugLog('NO_ACTIVE_GROUP', { userId: ctx.userId });
    return [];
  }
  ...
  debugLog('ACTIVE_UNITS_QUERY', { 
    groupId: scope.active_group_id,
    units: units,
    error: error?.message 
  });

  if (error) {
    console.error('[NOTES_DEBUG][ACTIVE_UNITS_ERROR]', error);
    return [];
  }

  const unitIds = (units || []).map((unit) => unit.id);
  debugLog('ACTIVE_UNIT_IDS', { unitIds });
  
  return unitIds;
}

async function assertNoteInScope(ctx, noteUnitId) {
  debugLog('SCOPE_CHECK_START', { 
    userId: ctx.userId, 
    noteUnitId 
  });
  ...
  debugLog('SCOPE_CHECK_COMPARISON', {
    noteUnitId,
    allowedIds,
    isAllowed: allowedIds.includes(noteUnitId)
  });

  if (!allowedIds.length || !allowedIds.includes(noteUnitId)) {
    console.error('[NOTES_DEBUG][SCOPE_CHECK_FAILED]', {
      userId: ctx.userId,
      noteUnitId,
      allowedIds,
      message: 'Note unit not in user\'s allowed units'
    });
    throw new SecurityError('Note not available in your curriculum', 403);
  }
  
  debugLog('SCOPE_CHECK_PASSED', { noteUnitId });
}
...
async function listNotes(req, res, ctx) {
  ...
  debugLog('LIST_NOTES_START', { 
    userId: ctx.userId, 
    query: { unit_id, group_id } 
  });
  
  const allowedUnitIds = await getActiveUnitIds(ctx);

  if (!allowedUnitIds.length) {
    debugLog('LIST_NOTES_NO_UNITS', { userId: ctx.userId });
  ...
  if (unit_id && unit_id !== 'undefined' && unit_id !== 'null') {
    if (!allowedUnitIds.includes(unit_id)) {
      debugLog('LIST_NOTES_UNIT_NOT_ALLOWED', { 
    ...
    if (group_id !== scope?.active_group_id) {
      debugLog('LIST_NOTES_GROUP_MISMATCH', { 
        requestedGroup: group_id,
        userGroup: scope?.active_group_id 
      });
      return res.status(200).json([]);
    }
  ...
  debugLog('LIST_NOTES_FILTER', { unitFilterIds });
  ...
  if (error) {
    console.error('[NOTES_DEBUG][LIST_NOTES_ERROR]', error);
    throw new SecurityError('Failed to fetch notes', 500);
  }

  debugLog('LIST_NOTES_RESULT', { 
    count: data?.length,
    notes: data?.map(n => ({ id: n.id, title: n.title, unit_id: n.unit_id }))
  });
...
async function getNoteDetail(req, res, ctx) {
  ...
  debugLog('GET_NOTE_DETAIL_START', { 
    userId: ctx.userId,
    id, 
    slug,
    idType: typeof id 
  ...
  if (!id && !slug) {
    console.error('[NOTES_DEBUG][GET_NOTE_DETAIL_NO_ID]', 'id or slug required');
    throw new SecurityError('id or slug required', 400);
  }

  // First, check if note exists without is_active filter
  let checkQuery = supabase
    .from('notes')
    .select('id, slug, title, unit_id, is_active');
  
  if (id) {
    checkQuery = checkQuery.eq('id', id);
  } else {
    checkQuery = checkQuery.eq('slug', slug);
  }

  const { data: noteCheck, error: checkError } = await checkQuery.maybeSingle();
  
  debugLog('NOTE_EXISTENCE_CHECK', { 
    noteCheck,
    checkError: checkError?.message 
  });

  if (checkError) {
    console.error('[NOTES_DEBUG][NOTE_CHECK_ERROR]', checkError);
    throw new SecurityError('Failed to fetch note', 500);
  }

  if (!noteCheck) {
    console.error('[NOTES_DEBUG][NOTE_NOT_FOUND_IN_DB]', { 
      id, 
      slug,
  ...
  if (!noteCheck.is_active) {
    console.error('[NOTES_DEBUG][NOTE_INACTIVE]', { 
      noteId: noteCheck.id,
      title: noteCheck.title,
      is_active: noteCheck.is_active 
    });
    throw new SecurityError('Note is not available', 404);
  }

  // Now check scope
  debugLog('CHECKING_SCOPE', { 
    noteId: noteCheck.id,
    noteUnitId: noteCheck.unit_id 
  });
  
  try {
    await assertNoteInScope(ctx, noteCheck.unit_id);
  } catch (scopeError) {
    console.error('[NOTES_DEBUG][SCOPE_ERROR]', {
      noteId: noteCheck.id,
      noteTitle: noteCheck.title,
      noteUnitId: noteCheck.unit_id,
      error: scopeError.message,
      status: scopeError.status || scopeError.statusCode
    });
    throw scopeError;
  }

  // Get full note data
  let query = supabase.from('notes').select('*').eq('id', noteCheck.id);
  const { data: note, error } = await query.maybeSingle();

  if (error) {
    console.error('[NOTES_DEBUG][FULL_NOTE_FETCH_ERROR]', error);
    throw new SecurityError('Failed to fetch note', 500);
  }

  debugLog('NOTE_FOUND_SUCCESS', { 
    noteId: note.id,
    title: note.title,
    unitId: note.unit_id 
  });
...
async function getRelatedNotes(req, res, ctx) {
  ...
  debugLog('GET_RELATED_NOTES', { note_id, userId: ctx.userId });
