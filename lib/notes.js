...
import { resolveBreadcrumb, resolveUnitTitle, getUserCurriculumScope } from './curriculum.js';
...
// ---------- helper: enforce curriculum scope on notes ----------
...
async function listNotes(req, res, ctx) {
  ...
  if (unit_id) {
    ...
    if (group_id !== (await getUserCurriculumScope(ctx.userId))?.active_group_id) {
  ...
  const { data, error } = await supabase
    .from('notes')
    .select(
      'id, slug, title, content_preview, author, file_url, category, tag, section_type, read_time, word_count, is_premium, display_order, unit_id'
    )
    .eq('is_active', true)
...
async function getRelatedNotes(req, res, ctx) {
  ...
  // ensure the source note is in user scope
  ...
  const { data: explicit } = await supabase
    .from('note_links')
    .select(
      'link_type, target_note_id, notes!note_links_target_note_id_fkey(id, slug, title, content_preview, read_time)'
    )
...
async function getDownloadUrl(req, res, ctx) {
  ...
  await supabase.from('notes').update({ download_count: (note.download_count || 0) + 1 }).eq('id', id);
