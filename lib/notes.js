...
async function autoLinkGlossaryTerms(html, levelId) {
  if (!html) return html;
  const { data: terms } = await supabase.from('glossary_terms').select('term, slug').contains('levels', [levelId]);
...
async function listNotes(req, res, ctx) {
  ...
  let query = supabase.from('notes').select('id, slug, title, content_preview, author, file_url, category, tag, section_type, read_time, word_count, is_premium, display_order, unit_id').eq('is_active', true).order('display_order', { ascending: true });
...
async function getRelatedNotes(req, res) {
  ...
  if (explicitResults.length >= 3) return res.status(200).json(explicitResults.slice(0, 5));
...
async function getReadingProgress(req, res, ctx) {
  const { note_id } = req.query;
  if (!note_id) throw new SecurityError('note_id required', 400);
  const { data } = await supabase.from('reading_progress').select('*').eq('user_id', ctx.userId).eq('note_id', note_id).maybeSingle();
...
async function getContinueReading(req, res, ctx) {
  ...
  const { data } = await supabase.from('reading_progress').select('*').eq('user_id', ctx.userId).neq('completed', true).gt('scroll_percentage', 5).order('last_accessed', { ascending: false }).limit(limit * 2);
  ...
  for (const prog of data) {
    ...
    results.push({
      note_id: note.id, slug: note.slug, title: note.title,
      topic: note.curriculum_units?.name, progress_percentage: prog.scroll_percentage,
...
async function saveReadingProgress(body, res, ctx) {
  ...
  await supabase.from('reading_progress').upsert({
    user_id: ctx.userId, note_id,
    scroll_percentage, scroll_position: scroll_position || 0,
...
async function getDownloadUrl(req, res, ctx) {
  ...
  const access = await checkDownloadAccess(user?.email, ctx.userId, 'note', note.id, note.is_premium || unitInfo?.is_premium);
  if (!access.allowed) throw new SecurityError('Access denied', 403);
...
async function createNote(body, res, ctx) {
  ...
  const readTime = wordCount ? `${Math.max(1, Math.ceil(wordCount / 200))} min read` : null;
  ...
  await supabase.from('content_references').upsert({
    content_type: 'note', content_id: data.id.toString(),
    slug: data.slug, path: `/notes/${data.slug}`, title: data.title
  });
...
async function updateNote(body, res, ctx) {
  ...
  if (updates.content !== undefined) {
    updates.toc = generateToc(updates.content);
    ...
    updates.read_time = updates.word_count ? `${Math.max(1, Math.ceil(updates.word_count / 200))} min read` : null;
  }
  updates.updated_at = new Date().toISOString();
  await supabase.from('notes').update(updates).eq('id', id);
  if (updates.title) {
    await supabase.from('content_references').update({ title: updates.title }).eq('content_type', 'note').eq('content_id', id.toString());
  }
  return res.status(200).json({ success: true });
}
...
async function linkNotes(body, res, ctx) {
  ...
  await supabase.from('note_links').insert({ source_note_id, target_note_id, link_type });
  return res.status(200).json({ success: true });
}
