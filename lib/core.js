...
export async function verifyTurnstile(token, ip) {
 ...
 async function findSessionRowByToken(hashedToken, selectCols) {
  ...
  const { data, error } = await supabase
    ...
    .select(selectCols)
    .or(
      `session_token_hash.eq.${hashedToken},and(prev_session_token_hash.eq.${hashedToken},prev_token_grace_until.gt.${nowIso})`
    )
    .limit(1)
    .maybeSingle();
 ...
 export async function validateSession(token) {
  ...
  if (data.expires_at && new Date(data.expires_at).getTime() <= Date.now()) {
    await supabase
      ...
      .or(
        `session_token_hash.eq.${hashedToken},prev_session_token_hash.eq.${hashedToken}`
      );
...
export async function addXp(userId, amount, reason, source, metadata) {
...
export async function updateTopicXp(userId, topic, xpDelta) {
  if (!topic || !xpDelta) return null;
...
export async function updateDailyChallengeProgress(userId, groupId, requirementType, amount) {
  ...
  await supabase.from('user_challenge_progress').upsert({
    ...
    progress: Math.min(newProgress, challenge.requirement_target),
  ...
  if (justCompleted) {
    await Promise.all([
      supabase.from('user_daily_challenge_completions').upsert({
        user_id: userId,
