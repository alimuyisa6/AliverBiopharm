...
export function isValidUserAnswer(answer) { return typeof answer === 'string' && answer.length <= 500; }
...
export async function handleContinueSession(userId, { session_id }) {
  ...
  return {
    ...
    questions: ordered.map(q => ({ id: q.id, text: q.question_text, topic: q.topic, concepts: [q.correct_answer, ...(q.alternate_answers?.map(a => a.term) || [])] }))
  };
}
...
export async function handleSubmitAnswer(userId, params, ip) {
  ...
  if (result.strength === 'developing') {
    diffText = {
      ...
      was_common_mistake: result.isCommonMistake || false
    };
  }
  ...
  if (levelResult.leveled_up) {
    const bonusXp = 20;
    await addXp(userId, bonusXp, 'level_up');
  }
  ...
  if (newRecallLevel > oldLevel && !milestones.includes(`Level ${newRecallLevel}`)) {
  ...
  const { count: dailyCount } = await supabase
    ...
    .single() || { count: 0 };
  const questResult = await checkAndAwardDailyQuest(userId, (dailyCount || 0) + 1);

  return {
    strength: result.strength, xp: result.xp, matched: result.matched,
    time_taken_seconds: timeTakenSeconds,
    strength_meaning: strengthMeanings[result.strength],
    feedback: { correct_answer: questionBank.correct_answer, answer_explanation: questionBank.correct_explanation || null, related_concepts: [questionBank.correct_answer, ...(questionBank.alternate_answers?.map(a => a.term) || [])], common_mistakes: questionBank.common_mistakes || [] },
    common_mistake_explanation: result.mistakeExplanation || null, study_note: result.note || null,
    diff: diffText,
...
export async function handleCompleteSession(userId, { session_id }) {
  if (!isValidSessionId(session_id)) throw new Error('Invalid session ID');
  const { data: session } = await supabase.from('recall_sessions').select('user_answers, topic, is_active, created_at').eq('session_id', session_id).eq('user_id', userId).maybeSingle();
  ...
  if (!session.is_active) {
    ...
    return { success: true, xp_earned_total: totalXp, already_completed: true, session_time_seconds: totalTime };
  }
  const totalXp = session.user_answers?.reduce((sum, a) => sum + (a.xp_earned || 0), 0) || 0;
  ...
  const totalTime = session.created_at ? Math.round((new Date() - new Date(session.created_at)) / 1000) : 0;
  return { success: true, xp_earned_total: totalXp, session_time_seconds: totalTime };
}
...
export async function handleGetDashboard(userId) {
  ...
  if (stats.current_streak >= 30) gardenStage = 'tree';
  ...
  const { data: questCompleted } = await supabase
    .from('user_daily_challenge_completions')
    .select('completed_at')
    .eq('user_id', userId)
    .eq('completed_date', today)
  ...
  const result = {
    level: stats.recall_level || 1, xp: stats.total_xp || 0, xpToNext: 100 - (stats.total_xp % 100),
    progressPercent, streak: stats.current_streak || 0, bestStreak: stats.best_streak || 0,
    bestMastery: stats.best_mastery || 0, totalQuestions: stats.total_questions || 0,
    milestones: milestoneList,
    dailyChallenge: { completed: dailyQuestions, target: 10, progressPercent: (dailyQuestions / 10) * 100, isCompleted: !!questCompleted },
    gardenStage, subjectIllustration: subjectIllustration[stats.selected_level] || 'fa-flask',
    quote: randomQuote, brainEnergy: 100,
