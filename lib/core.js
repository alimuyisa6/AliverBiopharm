...
/**
 * The functions below (addXp through getUserFiles-adjacent recall helpers)
 * operate exclusively on rows owned by a single userId. They now take `db`
 * (an RLS-scoped client, i.e. ctx.db from security-middleware.js) as the
 * first argument instead of reaching for the module-level service-role
 * `supabase` client. Callers must pass ctx.db.
 */

export async function addXp(db, userId, amount, reason, source, metadata) {
  const { data: current } = await db.from('user_platform_stats').select('total_xp').eq('user_id', userId).maybeSingle();
  const newTotal = (current?.total_xp || 0) + amount;
  await db.from('user_platform_stats').upsert({
    user_id: userId,
    total_xp: newTotal,
    platform_level: computeRecallLevel(newTotal),
    updated_at: new Date().toISOString()
  }, { onConflict: 'user_id' });
  await db.from('platform_xp_log').insert({
    user_id: userId,
    amount,
    reason,
...
export async function getPlatformStats(db, userId) {
  const { data } = await db.from('user_platform_stats').select('*').eq('user_id', userId).maybeSingle();
  return data || { total_xp: 0, platform_level: 1, current_streak: 0, longest_streak: 0, last_activity_date: null };
}

export async function recordPlatformActivity(db, userId) {
...
export async function updateTopicPerformance(db, userId, level, topic, sessionMasteryScore) {
  ...
  await db.from('user_recall_stats').upsert({
    user_id: userId,
    mastery,
...
export async function getMasteryTopics(db, userId) {
  const { data, error } = await db.from('user_recall_stats').select('mastery').eq('user_id', userId).maybeSingle();
...
export async function recordSessionActivity(db, userId, sessionAnswers) {
  const { data: stats } = await db
    .from('user_recall_stats')
...
export async function getPlatformLeaderboard(levelId, limit) {
  ...
  const names = await Promise.all(data.map(async (row) => [row.user_id, await getUserProfileName(row.user_id)]));
  ...
  return data
    .map(row => ({
      ...
      platform_level: computeRecallLevel(row.total_xp || 0),
...
export async function updateSpacedRepetition(db, userId, questionId, level, topic, strength) {
  ...
  await db.from('user_spaced_repetition').upsert({
    user_id: userId,
...
export async function getDueReviewQuestions(db, userId, limit) {
  const lim = Math.min(Math.max(parseInt(limit, 10) || 10, 1), 50);
...
export async function checkAndAwardAchievements(db, userId, stats) {
  const { data: allAchievements } = await db.from('achievements').select('*');
  if (!allAchievements || allAchievements.length === 0) return [];
  const { data: earned } = await db.from('user_achievements').select('achievement_id').eq('user_id', userId);
...
export async function handleGetSession(db, userId, { level, topic, class_name }) {
  let query = db
...
export async function handleSessionCheck(db, userId, { level, topic, class_name }) {
  const session = await handleGetSession(db, userId, { level, topic, class_name });
...
export async function handleContinueSession(db, userId, body) {
  ...
  if (currentQuestionId) {
    const { data: q } = await db.from('recall_questions_bank').select('*').eq('id', currentQuestionId).single();
    currentQuestion = q;
  }
  return {
...
export async function handleSubmitAnswer(db, userId, body, fingerprint) {
  ...
  const { data: question } = await db.from('recall_questions_bank').select('*').eq('id', question_id).single();
  ...
  await db.from('recall_sessions').update({
  ...
  if (strength.xp > 0) await addXp(db, userId, strength.xp, 'recall_answer');
...
export async function handleCompleteSession(db, userId, body) {
  const { session_id } = body;
  const { data: session } = await db
    .from('recall_sessions')
    .select('*')
  ...
  await updateTopicPerformance(db, userId, session.level, session.topic, report.mastery_score);
  const streakInfo = await recordSessionActivity(db, userId);
  return { success: true, report, streak_info: streakInfo };
}

export async function handleGetStats(db, userId) {
  const { data } = await db.from('user_recall_stats').select('*').eq('user_id', userId).maybeSingle();
  return data || { total_xp: 0, recall_level: 1, current_streak: 0, longest_streak: 0, total_sessions: 0, total_questions: 0 };
}

export async function handleGetAchievements(db, userId) {
  const { data } = await db.from('user_achievements').select('achievement_id, earned_at').eq('user_id', userId);
  const achievements = [];
  for (const a of (data || [])) {
    const { data: ach } = await db.from('achievements').select('*').eq('id', a.achievement_id).single();
...
export async function handleGetDashboard(db, userId) {
  const [stats, xp] = await Promise.all([
    ...
    db.from('user_xp').select('total_xp, rank_title').eq('user_id', userId).maybeSingle()
  ]);
...
export async function handleSetLevel(db, userId, level) {
  await db.from('user_recall_stats').upsert({
    user_id: userId,
    selected_level: level,
    updated_at: new Date().toISOString()
...
export async function handleGetLevel(db, userId) {
  const { data } = await db.from('user_recall_stats').select('selected_level').eq('user_id', userId).maybeSingle();
