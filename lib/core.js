// /lib/core.js – Shared utilities and Supabase clients
...
export async function isAdmin(userId, ip) {
  ...
  const { data } = await supabase.from('admin_master').select('admin_role, permissions, is_active, is_locked, ip_whitelist, id, is_online, is_busy').eq('admin_id', userId).eq('is_active', true).maybeSingle();
  if (!data) return null;
  if (data.ip_whitelist && data.ip_whitelist.length > 0 && !data.ip_whitelist.includes(ip || '')) return null;
  return data;
}

export const rateLimitMemory = new Map();
const globalRateLimit = { count: 0, resetTime: Date.now() + 1000 };
const MAX_GLOBAL_REQUESTS_PER_SECOND = 1000;

export async function checkRateLimit(ip, userId) {
  const now = Date.now();
  ...
  if (globalRateLimit.count > MAX_GLOBAL_REQUESTS_PER_SECOND) return false;
  ...
  const checkAndUpdate = (key, limit, windowMs) => {
    ...
    if (!record) { rateLimitMemory.set(key, { timestamps: [now] }); return true; }
...
export async function handleSubmitAnswer(userId, params, ip) {
  ...
  supabase.from('recall_xp_log').insert({ user_id: userId, amount: result.xp, reason: result.streak, session_id, question_id }).catch(() => {});
