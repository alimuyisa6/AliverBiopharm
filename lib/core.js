...
// Refactored to use user_topic_stats (still exists)
export async function updateTopicPerformance(userId, level, topic, percentage) {
  await supabase.from('user_topic_stats').upsert({
    user_id: userId,
    topic,
    xp: percentage,  // store avg score as xp? This table has xp, streak, last_activity_date. We'll store the score in xp for now.
...
// Refactored to use recall_questions_bank with unit_id instead of level/class_name
...
// Refactored: enforceClassAccess now uses curriculum_groups instead of class_sequence
...
export async function enforceClassAccess(userId, requestedLevel, requestedClassName) {
  ...
  if (profile.role === 'teacher') return true; // teachers can access any class content (already controlled by approved_track)
  ...
  if (requestedRank < ownRank) return true;   // lower level (e.g., O-Level vs A-Level) -> allowed
  if (requestedRank > ownRank) return false;  // higher level -> deny

  // Same level, check class sequence using curriculum_groups
  if (profile.class_name === requestedClassName) return true; // current class
  ...
  if (!ownSeq || !requestedSeq) return true; // if we can't determine sequence, allow access (safe)
