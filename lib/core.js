...
export async function handleSessionCheck(userId, { level, topic }) {
  ...
  if (data?.last_completed === today) return { available: false, message: 'You already completed this topic today. Come back tomorrow.' };
...
function sm2(easeFactor, intervalDays, repetitions, quality) {
  ...
  return { easeFactor: newEase, intervalDays: newInterval, repetitions: newReps, nextReviewDate: nextReview.toISOString() };
}

function qualityFromStrength(strength) {
  if (strength === 'excellent') return 5;
  if (strength === 'strong') return 4;
  return 2;
}

async function updateSpacedRepetition(userId, questionId, level, topic, strength) {
  try {
    ...
    const updated = sm2(base.ease_factor, base.interval_days, base.repetitions, quality);
    await supabase.from('user_spaced_repetition').upsert({
      user_id: userId, question_id: questionId, level, topic,
      ease_factor: updated.easeFactor, interval_days: updated.intervalDays,
      repetitions: updated.repetitions, next_review_date: updated.nextReviewDate,
      last_reviewed_at: new Date().toISOString()
    }, { onConflict: 'user_id,question_id' });
  } catch (e) {
    console.error('[SR] updateSpacedRepetition failed:', e.message);
  }
}
...
export async function handleGetSession(userId, { level, topic }) {
  ...
  if (existing && existing.question_ids?.length) {
    const questionIds = existing.question_ids.map(id => Number(id)).filter(id => !isNaN(id));
    if (!questionIds.length) throw new Error('No questions available for this topic yet.');
    const { data: questions } = await supabase.from('recall_questions_bank').select('id, question_text, topic').in('id', questionIds);
    const ordered = questionIds.map(id => questions?.find(q => Number(q.id) === id)).filter(Boolean);
    return {
      session_id: existing.session_id,
      questions: ordered.map(q => ({ id: q.id, text: q.question_text, topic: q.topic })),
  ...
  const allQuestionIds = allSelected.map(q => Number(q.id));
  const firstBatchIds = allQuestionIds.slice(0, 5);
  const { data: newSession, error: sessionError } = await supabase
    .from('recall_sessions')
    .insert({ user_id: userId, level, topic: topic || null, question_ids: firstBatchIds, all_question_ids: allQuestionIds, current_index: 0, user_answers: [], is_active: true })
    .select('session_id')
    .single();
  if (sessionError) throw new Error(`Failed to create session: ${sessionError.message}`);
  const firstQuestions = allSelected.slice(0, 5).map(q => ({ id: q.id, text: q.question_text, topic: q.topic }));
  return { session_id: newSession.session_id, questions: firstQuestions, has_more: allQuestionIds.length > 5 };
}

export async function handleContinueSession(userId, { session_id }) {
  if (!isValidSessionId(session_id)) throw new Error('Invalid session ID');
  const { data: session, error: sessionError } = await supabase.from('recall_sessions').select('question_ids, all_question_ids, level, topic').eq('session_id', session_id).eq('user_id', userId).maybeSingle();
  if (sessionError || !session) throw new Error('Session not found');
  const currentIds = (session.question_ids || []).map(id => Number(id));
  const allIds = (session.all_question_ids || []).map(id => Number(id));
  ...
  const { data: questions } = await supabase.from('recall_questions_bank').select('id, question_text, topic').in('id', nextBatch);
  const ordered = nextBatch.map(id => questions?.find(q => Number(q.id) === id)).filter(Boolean);
  return {
...
async function checkAndAwardDailyQuest(userId, dailyQuestionsToday) {
  const today = new Date().toISOString().split('T')[0];
  const { data: completed } = await supabase.from('user_daily_challenge_completions').select('completed_at').eq('user_id', userId).eq('completed_date', today).maybeSingle();
  if (completed) return null;
  if (dailyQuestionsToday >= 10) {
    await supabase.from('user_daily_challenge_completions').insert({ user_id: userId, completed_date: today, completed_at: new Date().toISOString() });
    await addXp(userId, 50, 'daily_challenge');
    await createNotification(userId, 'daily_challenge_complete', { xp_earned: 50 });
    return { completed: true, reward_xp: 50 };
  }
  return null;
}

async function detectAndRecordLevelUp(userId, newLevel, oldLevel) {
  if (newLevel > oldLevel) {
    await supabase.from('user_milestones').insert({ user_id: userId, milestone: `Level ${newLevel}` }).select();
    await createNotification(userId, 'level_up', { new_level: newLevel });
    return { leveled_up: true, new_level: newLevel };
  }
  return { leveled_up: false };
}

export async function handleSubmitAnswer(userId, params, ip) {
  const { session_id, question_id, user_answer, nonce, started_at } = params;
  if (!isValidSessionId(session_id)) throw new Error('Invalid session ID');
  ...
  const today = new Date().toISOString().split('T')[0];
  const { data: session, error: sessionError } = await supabase.from('recall_sessions').select('session_id, current_index, user_answers, question_ids, is_active, topic, version, level').eq('session_id', session_id).eq('user_id', userId).maybeSingle();
  if (sessionError || !session) throw new Error('Session not found');
  ...
  const { data: questionBank } = await supabase.from('recall_questions_bank').select('correct_answer, correct_explanation, alternate_answers, common_mistakes, topic').eq('id', question_id).single();
  ...
  const timeTakenSeconds = started_at ? Math.round((new Date(answeredAt) - new Date(started_at)) / 1000) : null;
  let diffText = null;
  if (result.strength === 'developing') {
    diffText = { your_answer: user_answer, correct_answer: questionBank.correct_answer, was_common_mistake: result.isCommonMistake || false };
  }
  const newUserAnswers = [
    ...(session.user_answers || []),
    { question_id, answer: user_answer, strength: result.strength, xp_earned: result.xp, answered_at: answeredAt, started_at: started_at || null, time_taken_seconds: timeTakenSeconds, topic: questionBank.topic || session.topic || 'General' }
  ];
  const newIndex = session.current_index + 1;
  const completed = newIndex >= session.question_ids.length;
  const { error: updateError } = await supabase
    .from('recall_sessions')
    .update({ user_answers: newUserAnswers, current_index: newIndex, is_active: !completed, completed_at: completed ? new Date().toISOString() : null, version: (session.version || 0) + 1 })
  ...
  const oldLevel = statsData.recall_level || 1;
  const newTotalXp = (statsData.total_xp || 0) + result.xp;
  const newRecallLevel = computeRecallLevel(newTotalXp);
  const levelResult = await detectAndRecordLevelUp(userId, newRecallLevel, oldLevel);
  if (levelResult.leveled_up) await addXp(userId, 20, 'level_up');
  ...
  if (statsData.last_session_date !== today) {
    if (statsData.last_session_date === new Date(Date.now() - 86400000).toISOString().split('T')[0]) newStreak++;
    else newStreak = 1;
  }
  ...
  if (newRecallLevel > oldLevel && !milestones.includes(`Level ${newRecallLevel}`)) milestones.push(`Level ${newRecallLevel}`);
  if (newStreak === 7 && !milestones.includes('7 Day Streak')) { milestones.push('7 Day Streak'); await supabase.from('user_milestones').insert({ user_id: userId, milestone: '7 Day Streak' }); await createNotification(userId, 'streak_milestone', { streak_days: 7 }); }
  if (newStreak === 30 && !milestones.includes('30 Day Streak')) { milestones.push('30 Day Streak'); await supabase.from('user_milestones').insert({ user_id: userId, milestone: '30 Day Streak' }); await createNotification(userId, 'streak_milestone', { streak_days: 30 }); }
  if (newStreak === 100 && !milestones.includes('100 Day Streak')) { milestones.push('100 Day Streak'); await supabase.from('user_milestones').insert({ user_id: userId, milestone: '100 Day Streak' }); await createNotification(userId, 'streak_milestone', { streak_days: 100 }); }
  ...
  const masteryAverage = Math.round(avgMastery);
  const topicMastery = newMastery[topicName] || 0;
  if (topicMastery >= 80 && (currentMastery[topicName] || 0) < 80) {
    await createNotification(userId, 'mastery_milestone', { topic_name: topicName, mastery_percent: Math.round(topicMastery) });
  }
  await supabase.from('user_recall_stats').upsert({
    user_id: userId, total_xp: newTotalXp, recall_level: newRecallLevel, current_streak: newStreak,
    last_session_date: today, total_questions: (statsData.total_questions || 0) + 1,
    excellent_count: (statsData.excellent_count || 0) + (result.strength === 'excellent' ? 1 : 0),
    strong_count: (statsData.strong_count || 0) + (result.strength === 'strong' ? 1 : 0),
    developing_count: (statsData.developing_count || 0) + (result.strength === 'developing' ? 1 : 0),
    mastery: newMastery, best_streak: bestStreak, best_mastery: bestMastery, milestones
  }, { onConflict: 'user_id' });
  try { await supabase.from('recall_xp_log').insert({ user_id: userId, amount: result.xp, reason: result.strength, session_id, question_id }); } catch (_) {}
  ...
  if (completed) {
    ...
    await createNotification(userId, 'session_complete', { question_count: session.question_ids.length, xp_earned: newTotalXp - (statsData.total_xp || 0) });
  }
  await updateSpacedRepetition(userId, question_id, session.level, questionBank.topic || session.topic || 'General', result.strength);
  const { data: dailyRow } = await supabase.from('user_daily_activity').select('count').eq('user_id', userId).eq('activity_date', today).maybeSingle();
  const questResult = await checkAndAwardDailyQuest(userId, (dailyRow?.count || 0) + 1);
  const xpProgress = computeXpProgress(newTotalXp);
  return {
    strength: result.strength,
    xp: result.xp,
    matched: result.matched,
    time_taken_seconds: timeTakenSeconds,
    time_taken_formatted: timeTakenSeconds !== null ? formatTimeServer(timeTakenSeconds) : null,
    strength_meaning: STRENGTH_MEANINGS[result.strength],
    feedback: {
      correct_answer: questionBank.correct_answer,
      answer_explanation: questionBank.correct_explanation || null,
      related_concepts: [questionBank.correct_answer, ...(questionBank.alternate_answers?.map(a => a.term) || [])],
    ...
    new_level: levelResult.leveled_up ? newRecallLevel : null,
    xp_progress: xpProgress,
    rank_title: computeRankTitle(newTotalXp),
    mastery_average: masteryAverage,
    daily_quest_completed: questResult?.completed || false,
    daily_quest_reward_xp: questResult?.reward_xp || 0
  };
}

export async function handleCompleteSession(userId, { session_id }) {
  if (!isValidSessionId(session_id)) throw new Error('Invalid session ID');
  const { data: session } = await supabase.from('recall_sessions').select('user_answers, topic, is_active, created_at, question_ids').eq('session_id', session_id).eq('user_id', userId).maybeSingle();
  if (!session) throw new Error('Session not found');
  const totalXp = session.user_answers?.reduce((sum, a) => sum + (a.xp_earned || 0), 0) || 0;
  const totalTime = session.created_at ? Math.round((new Date() - new Date(session.created_at)) / 1000) : 0;
  const report = computeSessionReport(session.user_answers || [], session.question_ids?.length || 0);
  if (!session.is_active) {
    return { success: true, xp_earned_total: totalXp, already_completed: true, session_time_seconds: totalTime, session_time_formatted: formatTimeServer(totalTime), report };
  }
  await supabase.from('recall_sessions').update({ is_active: false, completed_at: new Date().toISOString() }).eq('session_id', session_id);
  const today = new Date().toISOString().split('T')[0];
  const topicKey = session.topic || 'all';
  await supabase.from('user_topic_completion').upsert({ user_id: userId, topic_key: topicKey, last_completed: today }, { onConflict: 'user_id,topic_key' });
  const { data: stats } = await supabase.from('user_recall_stats').select('total_sessions, current_streak').eq('user_id', userId).maybeSingle();
  if (stats) await supabase.from('user_recall_stats').update({ total_sessions: (stats.total_sessions || 0) + 1 }).eq('user_id', userId);
  return {
    success: true,
    xp_earned_total: totalXp,
    session_time_seconds: totalTime,
    session_time_formatted: formatTimeServer(totalTime),
...
export async function handleGetStats(userId) {
  ...
  if (!stats) {
    const empty = {
      total_xp: 0, streak_days: 0, mastery_percent: 0, excellent: 0, strong: 0, developing: 0,
      topic_mastery: {}, heatmap: [], weak_concepts: [], recall_level: 1, achievements: [],
      topic_xp: {}, topic_streak: {}, selected_level: null, milestones: [], best_streak: 0,
      best_mastery: 0, total_questions: 0, xp_progress: computeXpProgress(0),
      rank_title: 'Beginner', mastery_average: 0
    };
  ...
  const heatmap = (daily || []).map(d => ({
    ...
    count: d.count,
    intensity: computeHeatmapIntensity(d.count)
  }));
  ...
  const result = {
    ...
    mastery_average: masteryPercent,
    ...
    xp_progress: computeXpProgress(stats.total_xp || 0),
    rank_title: computeRankTitle(stats.total_xp || 0)
  };
...
export async function handleGetAchievements(userId) {
  const { data: stats } = await supabase.from('user_recall_stats').select('total_xp, current_streak, mastery, total_questions').eq('user_id', userId).maybeSingle();
  if (!stats) return [];
...
export async function handleGetDashboard(userId) {
  ...
  const cached = getCached(cacheKey);
  if (cached) return cached;
  const { data: stats } = await supabase.from('user_recall_stats').select('total_xp, recall_level, current_streak, best_streak, best_mastery, total_questions, selected_level, mastery').eq('user_id', userId).maybeSingle();
  if (!stats) {
    const empty = {
      level: 1, xp: 0, xpToNext: 100, progressPercent: 0, streak: 0, bestStreak: 0,
      bestMastery: 0, totalQuestions: 0, milestones: [],
      dailyChallenge: { completed: 0, target: 10, progressPercent: 0, isCompleted: false },
      gardenStage: 'seedling', subjectIllustration: 'fa-flask',
      quote: 'Welcome! Start your first recall session.', brainEnergy: 100,
      dueForReview: 0, rank_title: 'Beginner',
      xp_progress: computeXpProgress(0), mastery_average: 0,
      spin_messages: LEVEL_SPIN_MESSAGES['O-Level'],
      floating_concepts: FLOATING_CARD_CONCEPTS['O-Level']
    };
    setCached(cacheKey, empty, 30000);
    return empty;
  }
  ...
  const xpProgress = computeXpProgress(stats.total_xp || 0);
  const gardenStage = (stats.current_streak || 0) >= 30 ? 'tree' : 'seedling';
  const { count: dueCount } = await supabase
    ...
    .maybeSingle() || { count: 0 };
  const { data: questCompleted } = await supabase.from('user_daily_challenge_completions').select('completed_at').eq('user_id', userId).eq('completed_date', today).maybeSingle();
  ...
  const selectedLevel = stats.selected_level || 'O-Level';
  const masteryValues = Object.values(stats.mastery || {});
  const masteryAverage = masteryValues.length ? Math.round(masteryValues.reduce((a, b) => a + b, 0) / masteryValues.length) : 0;
  if (dueCount > 0) {
    await createNotification(userId, 'recall_due', { due_count: dueCount });
  }
  const result = {
    ...
    dailyChallenge: {
      completed: dailyQuestions,
      target: 10,
      progressPercent: Math.min((dailyQuestions / 10) * 100, 100),
      isCompleted: !!questCompleted
    },
    gardenStage,
    subjectIllustration: SUBJECT_ILLUSTRATIONS[selectedLevel] || 'fa-flask',
    quote: randomQuote,
    brainEnergy: 100,
    ...
    xp_progress: xpProgress,
    mastery_average: masteryAverage,
    spin_messages: LEVEL_SPIN_MESSAGES[selectedLevel] || LEVEL_SPIN_MESSAGES['O-Level'],
    floating_concepts: FLOATING_CARD_CONCEPTS[selectedLevel] || FLOATING_CARD_CONCEPTS['O-Level']
  };
  setCached(cacheKey, result, 30000);
  return result;
}
...
export async function handleSetLevel(userId, level) {
  ...
  const isUserAdmin = admin?.admin_role === 'super_admin';
  if (existing?.selected_level && !isUserAdmin) throw new Error('Level already set and cannot be changed');
...
export async function addXp(userId, amount, reason) {
  const { data: current } = await supabase.from('user_xp').select('total_xp, level').eq('user_id', userId).maybeSingle();
  const newTotal = (current?.total_xp || 0) + amount;
  const newLevel = computeRecallLevel(newTotal);
  const rankTitle = computeRankTitle(newTotal);
  ...
  if (current && current.level < newLevel) await supabase.from('user_milestones').insert({ user_id: userId, milestone: `Level ${newLevel}` });
}

export async function updateTopicPerformance(userId, level, topic, percentage) {
  const { data: existing } = await supabase.from('user_topic_performance').select('id, attempt_count, avg_score').eq('user_id', userId).eq('level', level).eq('topic', topic).maybeSingle();
  if (existing) {
