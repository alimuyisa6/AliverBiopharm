...
async function findSessionRowByToken(hashedToken, selectCols) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      console.error('[findSessionRowByToken]', error);
      return null;
    }
    if (!data || data.is_active !== true) return null;
    return data;
  } catch (err) {
    console.error('[findSessionRowByToken]', err);
    return null;
  }
}
...
export async function validateSession(token) {
  ...
  const data = await findSessionRowByToken(hashedToken, 'user_id, expires_at, is_active, fingerprint, created_at, session_token_hash, prev_session_token_hash, prev_token_grace_until');
  if (!data) return null;
  if (data.expires_at && new Date(data.expires_at).getTime() <= Date.now()) {
    try {
      await supabase.from('user_sessions').update({ is_active: false, terminated_reason: 'expired' }).or(`session_token_hash.eq.${hashedToken},prev_session_token_hash.eq.${hashedToken}`);
    } catch (err) {
      console.error('[validateSession expire]', err);
    }
    return null;
  }
  return data;
...
export async function isAdmin(userId, ip) {
  ...
  try {
    const { data } = await supabase.from('admin_master').select('admin_role, permissions, is_active, is_locked, ip_whitelist, id, is_online, is_busy, admin_email, mfa_enabled, mfa_secret').eq('admin_id', userId).eq('is_active', true).maybeSingle();
    if (!data) return null;
    if (data.is_locked) return { ...data, admin_role: null };
    if (data.ip_whitelist && data.ip_whitelist.length > 0) {
      const clientIp = (ip || '').split(',')[0].trim();
      if (!data.ip_whitelist.includes(clientIp)) return { ...data, admin_role: null, ip_rejected: true };
    }
    return data;
  } catch (err) {
...
export function calculateRecallStrength(userAnswer, correctAnswer, alternateAnswers, commonMistakes) {
  ...
  const acceptedAnswers = [{ term: correctAnswer, explanation: null, isPrimary: true }, ...(alternateAnswers || []).map(a => ({ term: a.term, explanation: a.explanation || null, isPrimary: false }))];
...
export function computeSessionReport(userAnswers, totalQuestions) {
  ...
  return { excellent, strong, developing, mastery_score: masteryScore, total_time_seconds: totalTime, total_time_formatted: formatTimeServer(totalTime), avg_time_seconds: avgTime, avg_time_formatted: formatTimeServer(avgTime), top_topic: topTopic };
}
...
export async function addXp(userId, amount, reason, source, metadata, unitId, groupId, levelId) {
  try {
    const { data: current } = await supabase.from('user_platform_stats').select('total_xp').eq('user_id', userId).maybeSingle();
    const newTotal = (current?.total_xp || 0) + amount;
    await supabase.from('user_platform_stats').upsert({ user_id: userId, total_xp: newTotal, platform_level: computeRecallLevel(newTotal), updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    await supabase.from('platform_xp_log').insert({
      user_id: userId,
      amount,
      reason,
...
export async function getPlatformStats(userId) {
  try {
    const { data } = await supabase.from('user_platform_stats').select('*').eq('user_id', userId).maybeSingle();
    return data || { total_xp: 0, platform_level: 1, current_streak: 0, longest_streak: 0, last_activity_date: null };
  } catch (err) {
    console.error('[getPlatformStats]', err);
    return { total_xp: 0, platform_level: 1, current_streak: 0, longest_streak: 0, last_activity_date: null };
  }
}

export async function recordPlatformActivity(userId) {
  try {
    const { data: stats } = await supabase.from('user_platform_stats').select('current_streak, longest_streak, last_activity_date').eq('user_id', userId).maybeSingle();
    const { streak, isNewDay } = computeStreak(stats?.last_activity_date, stats?.current_streak);
    const longestStreak = Math.max(streak, stats?.longest_streak || 0);
    await supabase.from('user_platform_stats').upsert({ user_id: userId, current_streak: streak, longest_streak: longestStreak, last_activity_date: new Date().toISOString().slice(0, 10), updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    return { current_streak: streak, longest_streak: longestStreak, is_new_day: isNewDay };
  } catch (err) {
    console.error('[recordPlatformActivity]', err);
    throw new Error('Failed to record platform activity');
  }
}

export async function updateTopicXp(userId, unitId, topic, xpDelta) {
  if (!topic || !xpDelta || !unitId) return null;
  try {
    const { data: existing } = await supabase.from('user_topic_stats').select('xp, streak, last_activity_date').eq('user_id', userId).eq('unit_id', unitId).maybeSingle();
    const { streak } = computeStreak(existing?.last_activity_date, existing?.streak);
    const newXp = (existing?.xp || 0) + xpDelta;
    await supabase.from('user_topic_stats').upsert({ user_id: userId, unit_id: unitId, topic, xp: newXp, streak, last_activity_date: new Date().toISOString().slice(0, 10) }, { onConflict: 'user_id,unit_id' });
    return { xp: newXp, streak };
  } catch (err) {
    console.error('[updateTopicXp]', err);
...
export async function updateUserRecords(userId, { score, timeTakenSeconds, isPerfect }) {
  try {
    const { data: existing } = await supabase.from('user_records').select('*').eq('user_id', userId).maybeSingle();
    const highestScore = Math.max(existing?.highest_score || 0, score || 0);
    let fastestCompletion = existing?.fastest_completion || 0;
    if (timeTakenSeconds && (fastestCompletion === 0 || timeTakenSeconds < fastestCompletion)) fastestCompletion = timeTakenSeconds;
    const perfectBlocks = (existing?.perfect_blocks || 0) + (isPerfect ? 1 : 0);
    await supabase.from('user_records').upsert({ user_id: userId, highest_score: highestScore, fastest_completion: fastestCompletion, perfect_blocks: perfectBlocks }, { onConflict: 'user_id' });
    return { highest_score: highestScore, fastest_completion: fastestCompletion, perfect_blocks: perfectBlocks };
  } catch (err) {
    console.error('[updateUserRecords]', err);
...
export async function updateDailyChallengeProgress(userId, groupId, requirementType, amount) {
  ...
  try {
    ...
    const { data: challenge } = await supabase.from('daily_challenges').select('*').eq('group_id', groupId).eq('date', today).eq('requirement_type', requirementType).maybeSingle();
    if (!challenge) return null;
    const { data: existing } = await supabase.from('user_challenge_progress').select('progress, completed').eq('user_id', userId).eq('challenge_id', challenge.id).maybeSingle();
    if (existing?.completed) return { already_completed: true };
    const newProgress = (existing?.progress || 0) + amount;
    const justCompleted = newProgress >= challenge.requirement_target;
    await supabase.from('user_challenge_progress').upsert({ user_id: userId, challenge_id: challenge.id, progress: Math.min(newProgress, challenge.requirement_target), completed: justCompleted, completed_at: justCompleted ? new Date().toISOString() : null }, { onConflict: 'user_id,challenge_id' });
    if (justCompleted) {
      await Promise.all([
        supabase.from('user_daily_challenge_completions').upsert({ user_id: userId, completed_date: today, completed_at: new Date().toISOString() }, { onConflict: 'user_id,completed_date' }),
        addXp(userId, challenge.reward_xp || 0, 'daily_challenge', 'daily_challenge', { challenge_id: challenge.id })
      ]);
    }
    return { just_completed: justCompleted, progress: Math.min(newProgress, challenge.requirement_target), target: challenge.requirement_target };
  } catch (err) {
    console.error('[updateDailyChallengeProgress]', err);
    throw new Error('Failed to update daily challenge progress');
  }
}

export async function updateTopicPerformance(userId, level, topic, sessionMasteryScore) {
  try {
    const { data: existing } = await supabase.from('user_recall_stats').select('mastery, topic_completion, last_topic_date, best_mastery').eq('user_id', userId).maybeSingle();
    ...
    await supabase.from('user_recall_stats').upsert({ user_id: userId, mastery, topic_completion: topicCompletion, last_topic_date: lastTopicDate, best_mastery: bestMastery, selected_level: level, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    return newMastery;
  } catch (err) {
    console.error('[updateTopicPerformance]', err);
    throw new Error('Failed to update topic performance');
  }
}
...
export async function getMasteryTopics(userId) {
  try {
    ...
    if (error) {
      console.error('[getMasteryTopics]', error);
      return {};
    }
    return data?.mastery || {};
  } catch (err) {
    console.error('[getMasteryTopics]', err);
    return {};
  }
}
...
export async function recordSessionActivity(userId, sessionAnswers) {
  try {
    const { data: stats } = await supabase.from('user_recall_stats').select('current_streak, longest_streak, best_streak, last_session_date, total_sessions, total_questions, excellent_count, strong_count, developing_count').eq('user_id', userId).maybeSingle();
    ...
    await supabase.from('user_recall_stats').upsert({ user_id: userId, current_streak: streak, longest_streak: longestStreak, best_streak: bestStreak, last_session_date: new Date().toISOString().slice(0, 10), total_sessions: (stats?.total_sessions || 0) + 1, total_questions: (stats?.total_questions || 0) + answers.length, excellent_count: (stats?.excellent_count || 0) + excellentDelta, strong_count: (stats?.strong_count || 0) + strongDelta, developing_count: (stats?.developing_count || 0) + developingDelta, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    return { current_streak: streak, longest_streak: longestStreak, best_streak: bestStreak, is_new_day: isNewDay };
  } catch (err) {
    console.error('[recordSessionActivity]', err);
...
export async function getPlatformLeaderboard(levelId, limit) {
  try {
    ...
    if (levelId) {
      const { data: profiles } = await supabase.from('user_profiles').select('user_id').eq('active_level_id', levelId);
      userIdFilter = (profiles || []).map(p => p.user_id);
      if (userIdFilter.length === 0) return [];
    }
    let query = supabase.from('user_platform_stats').select('user_id, total_xp, current_streak').order('total_xp', { ascending: false }).limit(lim);
    if (userIdFilter) query = query.in('user_id', userIdFilter);
    const { data, error } = await query;
    if (error) {
      console.error('[getPlatformLeaderboard]', error);
      return [];
    }
    if (!data || data.length === 0) return [];
    const names = await Promise.all(data.map(async (row) => [row.user_id, await getUserProfileName(row.userId)]));
    const nameMap = new Map(names);
    return data.map(row => ({ user_id: row.user_id, display_name: nameMap.get(row.user_id) || 'Anonymous Learner', total_xp: row.total_xp || 0, rank_title: computeRankTitle(row.total_xp || 0), platform_level: computeRecallLevel(row.total_xp || 0), current_streak: row.current_streak || 0 })).sort((a, b) => b.total_xp - a.total_xp);
  } catch (err) {
    console.error('[getPlatformLeaderboard]', err);
    return [];
  }
}
...
export async function getCurriculumLevelMeta(levelId) {
  ...
  try {
    const { data } = await supabase.from('curriculum_levels').select('id, display_name, kind, group_label, unit_label, icon, color, display_order').eq('id', levelId).maybeSingle();
    return data || null;
  } catch (err) {
    console.error('[getCurriculumLevelMeta]', err);
    return null;
  }
}
...
export async function getMotivationalQuote(levelId, unitId) {
  try {
    let query = supabase.from('recall_motivational_quotes').select('quote, author').eq('is_active', true).limit(20);
    ...
    console.error('[getMotivationalQuote]', err);
    return null;
  }
}
...
export async function updateSpacedRepetition(userId, questionId, level, topic, strength) {
  try {
    ...
    const { data: existing } = await supabase.from('user_spaced_repetition').select('ease_factor, interval_days, repetitions').eq('user_id', userId).eq('question_id', questionId).maybeSingle();
    ...
    await supabase.from('user_spaced_repetition').upsert({ user_id: userId, question_id: questionId, level, topic, ease_factor: easeFactor, interval_days: intervalDays, repetitions, next_review_date: nextReviewDate, last_reviewed_at: new Date().toISOString() }, { onConflict: 'user_id,question_id' });
    return { ease_factor: easeFactor, interval_days: intervalDays, repetitions, next_review_date: nextReviewDate };
  } catch (err) {
    console.error('[updateSpacedRepetition]', err);
...
export async function getDueReviewQuestions(userId, limit) {
  try {
    const lim = Math.min(Math.max(parseInt(limit, 10) || 10, 1), 50);
    const { data, error } = await supabase.from('user_spaced_repetition').select('question_id, level, topic, next_review_date, repetitions').eq('user_id', userId).lte('next_review_date', new Date().toISOString()).order('next_review_date', { ascending: true }).limit(lim);
    if (error) {
      console.error('[getDueReviewQuestions]', error);
      return [];
    }
    return data || [];
  } catch (err) {
    console.error('[getDueReviewQuestions]', err);
    return [];
  }
}
...
export async function checkAndAwardAchievements(userId, stats) {
  try {
    ...
    const statsMap = { total_xp: stats.total_xp || 0, current_streak: stats.current_streak || 0, longest_streak: stats.longest_streak || 0, total_sessions: stats.total_sessions || 0, total_questions: stats.total_questions || 0, excellent_count: stats.excellent_count || 0, strong_count: stats.strong_count || 0 };
    const newlyEarned = [];
    ...
    if (newlyEarned.length > 0) {
      await supabase.from('user_achievements').insert(newlyEarned.map(ach => ({ user_id: userId, achievement_id: ach.id, earned_at: new Date().toISOString() })));
    }
    return newlyEarned;
  } catch (err) {
...
export async function handleGetSession(userId, { level, topic, class_name }) {
  try {
    let query = supabase.from('recall_sessions').select('*').eq('user_id', userId).eq('level', level).eq('is_active', true).order('created_at', { ascending: false }).limit(1);
    if (topic) query = query.eq('topic', topic);
    if (class_name) query = query.eq('class_name', class_name);
    const { data } = await query.maybeSingle();
    return data || null;
  } catch (err) {
    console.error('[handleGetSession]', err);
    return null;
  }
}
...
export async function handleSessionCheck(userId, { level, topic, class_name }) {
  try {
    ...
    return { exists: true, session_id: session.session_id, current_index: session.current_index, total_questions: (session.question_ids || session.all_question_ids || []).length, completed: !session.is_active };
  } catch (err) {
    console.error('[handleSessionCheck]', err);
    return { exists: false };
  }
}
...
export async function handleContinueSession(userId, body) {
  try {
    ...
    const { data } = await supabase.from('recall_sessions').select('*').eq('session_id', session_id).eq('user_id', userId).eq('is_active', true).single();
    ...
    return { session_id: data.session_id, current_index: currentIndex, total_questions: questionIds.length, current_question: currentQuestion, user_answers: data.user_answers || [], level: data.level, topic: data.topic };
  } catch (err) {
    console.error('[handleContinueSession]', err);
    throw err;
  }
}
...
export async function handleSubmitAnswer(userId, body, fingerprint) {
  try {
    ...
    const { data: session } = await supabase.from('recall_sessions').select('*').eq('session_id', session_id).eq('user_id', userId).eq('is_active', true).single();
    ...
    const userAnswers = [...(session.user_answers || []), { question_id, user_answer, strength: strength.strength, correct_answer: question.correct_answer, explanation: strength.explanation || question.correct_explanation || question.explanation || '', xp_earned: strength.xp, time_taken_seconds: started_at ? Math.round((Date.now() - new Date(started_at).getTime()) / 1000) : 0 }];
    ...
    await supabase.from('recall_sessions').update({ user_answers: userAnswers, current_index: newIndex, is_active: !isComplete, completed_at: isComplete ? new Date().toISOString() : null, updated_at: new Date().toISOString(), version: (session.version || 0) + 1 }).eq('session_id', session_id);
    if (strength.xp > 0) await addXp(userId, strength.xp, 'recall_answer', 'recall', { session_id, question_id }, session.unit_id, session.group_id, session.level);
    await drainEnergyForAnswer(userId);
    if (strength.strength === 'developing') {
      await recordWeakConcept(userId, 'recall', session.unit_id, session.group_id, session.level, strength.matched || question.correct_answer, false, question_id);
    }
    return { strength: strength.strength, xp_earned: strength.xp, correct_answer: question.correct_answer, explanation: strength.explanation || question.correct_explanation || question.explanation || '', is_complete: isComplete, current_index: newIndex, total_questions: questionIds.length };
  } catch (err) {
...
export async function handleCompleteSession(userId, body) {
  try {
    const { session_id } = body;
    const { data: session } = await supabase.from('recall_sessions').select('*').eq('session_id', session_id).eq('user_id', userId).single();
    if (!session) throw new Error('Session not found');
    await supabase.from('recall_sessions').update({ is_active: false, completed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('session_id', session_id);
    ...
    console.error('[handleCompleteSession]', err);
    throw err;
  }
}
...
export async function handleGetStats(userId) {
  try {
    ...
    console.error('[handleGetStats]', err);
    return { total_xp: 0, recall_level: 1, current_streak: 0, longest_streak: 0, total_sessions: 0, total_questions: 0 };
  }
}
...
export async function handleGetAchievements(userId) {
  try {
    ...
    console.error('[handleGetAchievements]', err);
    return [];
  }
}
...
export async function handleGetDashboard(userId) {
  try {
    ...
    return { ...stats, total_xp: xpData.total_xp, rank_title: xpData.rank_title, xp_progress: xpProgress };
  } catch (err) {
    console.error('[handleGetDashboard]', err);
    throw err;
  }
}
...
export async function getTopicsForLevel(unitId) {
  try {
    const { data: questions, error } = await supabase.from('recall_questions_bank').select('topic, unit_id').eq('unit_id', unitId).eq('is_active', true);
    if (error) return [];
    const topics = [...new Set((questions || []).map(q => q.topic))];
    const result = [];
    for (const topic of topics) {
      const { count } = await supabase.from('recall_questions_bank').select('id', { count: 'exact', head: true }).eq('unit_id', unitId).eq('topic', topic).eq('is_active', true);
      result.push({ topic_name: topic, unit_id: unitId, question_count: count || 0 });
    }
    return result;
  } catch (err) {
    console.error('[getTopicsForLevel]', err);
    return [];
  }
}
...
export async function handleSetLevel(userId, level) {
  try {
    await supabase.from('user_recall_stats').upsert({ user_id: userId, selected_level: level, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
    return { success: true };
  } catch (err) {
    console.error('[handleSetLevel]', err);
...
export async function handleGetLevel(userId) {
  try {
    ...
    console.error('[handleGetLevel]', err);
...
export function getUserClient(userId) {
  ...
  return createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, { global: { headers: { Authorization: `Bearer ${token}` } }, auth: { autoRefreshToken: false, persistSession: false } });
}

export function getAuthOnlyClient() {
  return createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
}
...
export async function getUserProfileName(userId) {
  ...
  try {
    ...
    console.error('[getUserProfileName]', err);
    return null;
  }
}
...
export async function auditLog({ actorId, actorRole, action, targetType, targetId, ip, metadata }) {
  try {
    await supabase.from('audit_log').insert({ actor_id: actorId || null, actor_role: actorRole || null, action, target_type: targetType || null, target_id: targetId ? String(targetId) : null, ip_address: ip || null, metadata: metadata || {}, created_at: new Date().toISOString() });
  } catch (err) {
...
export function checkPasswordBreached(password) {
  return new Promise((resolve) => {
    try {
      ...
      const req = https.get(`https://api.pwnedpasswords.com/range/${prefix}`, { timeout: 2500, headers: { 'User-Agent': 'AliverBiopharm-Security-Check' } }, (res) => {
        let body = '';
...
export async function isTeacherApproved(userId) {
  ...
  try {
    const { data: profile, error } = await supabase.from('user_profiles').select('role, is_approved_teacher, approved_track').eq('user_id', userId).maybeSingle();
    if (error || !profile) return false;
    if (profile.role !== 'teacher') return false;
    return profile.is_approved_teacher === true;
  } catch (err) {
    console.error('[isTeacherApproved]', err);
    return false;
  }
}
...
export function verifyCsrf(req, secret, userId, fingerprint) {
  ...
  if (!Number.isSafeInteger(ts)) throw new Error('Invalid CSRF token');
  const windowSize = 15 * 60 * 1000;
  const currentWindow = Math.floor(Date.now() / windowSize) * windowSize;
  const allowedWindows = [currentWindow, currentWindow - windowSize];
  if (!allowedWindows.includes(ts)) throw new Error('Invalid CSRF token');
  const combined = fingerprint ? `${secret}:${fingerprint}` : secret;
  const expectedHmac = crypto.createHmac('sha256', combined).update(String(ts)).digest('hex');
  if (providedHmac.length !== expectedHmac.length || !crypto.timingSafeEqual(Buffer.from(providedHmac, 'utf8'), Buffer.from(expectedHmac, 'utf8'))) throw new Error('Invalid CSRF token');
  return true;
}

export async function recordWeakConcept(userId, source, unitId, groupId, levelId, concept, isCorrect, questionId) {
  try {
    const { data: existing } = await supabase.from('user_weak_concepts_v2').select('*').eq('user_id', userId).eq('concept', concept).eq('unit_id', unitId).maybeSingle();
    if (existing) {
      const newTotal = (existing.total_attempts || 0) + 1;
      const newIncorrect = isCorrect ? (existing.incorrect_attempts || 0) : (existing.incorrect_attempts || 0) + 1;
      await supabase.from('user_weak_concepts_v2').update({
        total_attempts: newTotal,
        incorrect_attempts: newIncorrect,
      ...
      await supabase.from('user_weak_concepts_v2').insert({
        user_id: userId,
        source,
        level_id: levelId,
        group_id: groupId,
    ...
    console.error('[recordWeakConcept]', err);
    throw new Error('Failed to record weak concept');
  }
}
