...
export async function isAdmin(userId, ip) {
  ...
  const { data } = await supabase
    .from('admin_master')
    .select('admin_role, permissions, is_active, is_locked, ip_whitelist, id, admin_id, admin_email, mfa_enabled, passkey_enabled')
    .eq('admin_id', userId)
    .eq('is_active', true)
    .maybeSingle();
...
export async function updateTopicPerformance(userId, level, topic, masteryScore) {
  ...
  const currentMastery = topicCompletion[topic]?.mastery || 0;

  topicCompletion[topic] = {
    mastery: Math.max(currentMastery, masteryScore),
    last_practiced_at: new Date().toISOString(),
    attempts: (topicCompletion[topic]?.attempts || 0) + 1
  };
  ...
  if (error) {
    console.error('[updateTopicPerformance]', error.message);
  }
}
...
export function computeAccuracy(excellentCount, strongCount, totalQuestions) {
  ...
  const weightedCorrect = (Number(excellentCount) || 0) * 100 + (Number(strongCount) || 0) * 70;

  return Math.round((weightedCorrect / (total * 100)) * 100);
}
...
export async function updateTopicXp(userId, unitId, topic, xpDelta) {
  ...
  const amount = Math.trunc(Number(xpDelta) || 0);
...
export async function updateUserRecords(userId, { score, timeTakenSeconds, isPerfect }) {
  const { data, error } = await supabase.rpc('atomic_update_user_records', {
    p_user_id: userId,
    p_score: Math.trunc(Number(score) || 0),
    p_time_taken_seconds: Math.trunc(Number(timeTakenSeconds) || 0),
    p_is_perfect: !!isPerfect
  });

  if (error) {
    console.error('[updateUserRecords]', error.message);
...
export async function recordWeakConcept(userId, source, unitId, groupId, levelId, concept, isCorrect, questionId) {
  ...
  const { error } = await supabase.rpc('atomic_record_weak_concept', {
    p_user_id: userId,
    p_source: source,
    p_unit_id: unitId || null,
    p_group_id: groupId || null,
  ...
  if (error) {
    console.error('[recordWeakConcept]', error.message);
  }
}
...
export async function updateSpacedRepetition(userId, questionId, level, topic, strength) {
  ...
  const { data, error } = await supabase.rpc('atomic_update_spaced_repetition', {
    p_user_id: userId,
  ...
  if (error) {
    console.error('[updateSpacedRepetition]', error.message);
...
export async function getPlatformLeaderboard(levelId, limit) {
  ...
  const { data: profiles } = await supabase
    .from('user_profiles')
    .select('user_id, profile_picture_url, display_name, show_on_leaderboard')
    .in('user_id', userIds);
  ...
  return data
    ...
    .map((row) => ({
      user_id: row.user_id,
      display_name: profileMap.get(row.user_id)?.display_name || 'Anonymous Learner',
...
export function computeStreak(lastSessionDate, currentStreak) {
  ...
  if (lastSessionDate === today) return { streak: Number(currentStreak) || 1, isNewDay: false };

  const last = new Date(`${lastSessionDate}T00:00:00Z`);
  const now = new Date(`${today}T00:00:00Z`);
  const diffDays = Math.round((now - last) / 86400000);

  if (diffDays <= 1) return { streak: (Number(currentStreak) || 0) + 1, isNewDay: true };
...
export function getUserClient(userId) {
  ...
  return createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: `Bearer ${header}.${body}.${signature}` } },
    auth: { autoRefreshToken: false, persistSession: false }
  });
}
...
export async function getUserProfileName(userId) {
  ...
  try {
    ...
    return authUser?.user?.user_metadata?.full_name || null;
  } catch {
    return null;
  }
}
...
export async function recordSessionActivity(userId, sessionAnswers) {
  ...
  const excellentDelta = answers.filter((item) => item?.strength === 'excellent').length;
  const strongDelta = answers.filter((item) => item?.strength === 'strong').length;
  const developingDelta = answers.filter((item) => item?.strength === 'developing').length;
