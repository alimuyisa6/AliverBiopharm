...
async function selectQuestionsForUser(userId, level, topic, limit = 20) {
  ...
  if (selectedIds.length < limit) {
    ...
    let qbQuery = supabase
      ...
      .eq('level', level)
      .eq('is_active', true);
    if (topic) qbQuery = qbQuery.eq('topic', topic);
    qbQuery = qbQuery.not('id', 'in', selectedIds.length ? selectedIds : [0]);
    const { data: newQuestions } = await qbQuery;
...
export async function handleSubmitAnswer(userId, params, ip) {
  ...
  const newUserAnswers = [
    ...
    { question_id, answer: user_answer, strength: result.strength, xp_earned: result.xp, answered_at: new Date().toISOString(), started_at: started_at || null }
  ];
