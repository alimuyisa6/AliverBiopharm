 import { supabase, parseCookies, hashToken, getClientIp, generateSessionToken, generateCsrfToken, isAdmin, getAuthOnlyClient } from './core.js';
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST' && path === 'signup') {
    ...
    console.error('[SIGNUP] start', JSON.stringify({ email, hasPassword: !!password, hasTurnstileToken: !!turnstile_token }));
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    console.error('[SIGNIN] start', JSON.stringify({ email, hasPassword: !!password, hasTurnstileToken: !!turnstile_token }));
  ...
  if (req.method === 'POST' && path === 'change_password') {
    if (!ctx.authenticated) throw new SecurityError('Authentication required', 401);
    const ip = getClientIp(req);
    if (!rateLimiter.check(ip, ctx.userId, 'change_password')) throw new SecurityError('Too many requests', 429);
