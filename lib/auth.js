...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET' && path === 'get_user') {
    ...
    const [{ data: restriction }, { data: profile }] = await Promise.all([
      ...
      supabase
        .from('user_profiles')
        .select('role, track, class_name, onboarding_completed, is_approved_teacher, approved_by, approved_at, approved_track, approval_notes, active_level_id, active_group_id, profile_picture_url')
        .eq('user_id', ctx.userId)
        .maybeSingle()
    ]);
...
async function signup(req, res) {
  ...
  if (!password || password.length < 10) {
    throw new SecurityError('Password must be at least 10 characters', 400);
  }

  if (password.length > 128) {
    throw new SecurityError('Password must not exceed 128 characters', 400);
  }
  ...
  if (categories < 3) {
    throw new SecurityError('Password must contain at least 3 of: uppercase letter, lowercase letter, number, special character', 400);
  }

  if (!turnstile_token) {
    throw new SecurityError('Please complete the verification challenge.', 400);
  }
  ...
  if (!trimmedName || trimmedName.length < 2 || trimmedName.length > 100) {
    throw new SecurityError('Full name must be between 2 and 100 characters', 400);
  }

  if (/[<>]/.test(trimmedName)) {
    throw new SecurityError('Full name contains invalid characters', 400);
  }

  if (!role || !['student', 'teacher'].includes(role)) {
    throw new SecurityError('Role must be student or teacher', 400);
  }

  if (!level || typeof level !== 'string') {
    throw new SecurityError('Level is required', 400);
  }
  ...
  if (!curriculumLevel) {
    throw new SecurityError('Invalid curriculum level', 400);
  }
  ...
  if (!defaultGroup) {
    throw new SecurityError('No default group found for this level', 400);
  }
  ...
  if (error) {
    if (error.code === 'user_already_exists') {
      return res.status(200).json({
        user: null,
        message: 'If this email is valid, please check your inbox to confirm your account.'
      });
    }

    if (error.code === 'captcha_failed') {
      throw new SecurityError('Verification expired, please try again.', 400);
    }
...
async function signin(req, res) {
  ...
  if (!email || !password) {
    throw new SecurityError('Email and password required', 400);
  }
  ...
  if (restriction) {
    if (restriction.restriction_type === 'disabled') {
      throw new SecurityError('Your account has been permanently disabled. Contact support.', 403);
    }

    if (restriction.restriction_type === 'suspended') {
      throw new SecurityError(restriction.lock_reason || 'Your account has been suspended. Contact support.', 403);
    }
  ...
  if (adminCandidate?.ip_rejected) {
    await auditLog({
      actorId: data.user.id,
      action: 'signin_admin_ip_rejected',
      ip
    });
  ...
  if (ctxAdminProfile(session.user_id)) {
    await supabase
      .from('admin_master')
  ...
  await auditLog({
    ...
    metadata: {
      mfa: mfaWasVerified,
      passkey: passkeyWasVerified,
      admin_session: isAdminSession
    }
  });
...
async function changePassword(req, res, ctx) {
  ...
  if (!(await rateLimiter.check(ip, ctx.userId, 'change_password'))) {
  ...
  if (!current_password || !new_password) {
    throw new SecurityError('Current and new password are required', 400);
  }
  ...
  if (new_password === current_password) {
    throw new SecurityError('New password must be different from current password', 400);
  }
  ...
  if (!authUser?.user) {
    throw new SecurityError('Account not found', 404);
  }
...
async function handoffCreate(req, res, ctx) {
  ...
  if (!(await rateLimiter.check(ip, ctx.userId, 'handoff_create'))) {
    throw new SecurityError('Too many requests', 429);
  }
...
async function handoffExchange(req, res) {
  ...
  if (!(await rateLimiter.check(ip, null, 'handoff_exchange'))) {
    throw new SecurityError('Too many requests', 429);
  }
  ...
  if (row.creator_ip_network && row.creator_ip_network !== getIpNetwork(ip)) {
    ...
    await auditLog({
      actorId: row.user_id,
      action: 'handoff_network_mismatch',
      ip,
      metadata: {
        creator_ip_network: row.creator_ip_network,
        exchange_ip_network: getIpNetwork(ip)
      }
    });
  ...
  if (!adminData?.admin_role || adminData.is_locked || adminData.ip_rejected) {
  ...
  const session = await createUserSession(
    ...
    req.headers['user-agent'],
    req,
    !!adminData.mfa_enabled,
    true,
    !!adminData.passkey_enabled
  );
  ...
  await auditLog({
    actorId: row.user_id,
    actorRole: adminData.admin_role,
...
function setSessionCookie(res, token, maxAgeMs) {
  ...
  res.setHeader(
    'Set-Cookie',
    `session=${token}; HttpOnly; Secure; SameSite=None; Partitioned; Max-Age=${maxAgeSeconds}; Path=/`
  );
}
...
function clearSessionCookie(res) {
  res.setHeader(
    'Set-Cookie',
    'session=; HttpOnly; Secure; SameSite=None; Partitioned; Max-Age=0; Path=/'
  );
}
...
async function ctxAdminProfile(userId) {
