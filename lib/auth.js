...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET' && path === 'get_user') {
    ...
    const [{ data: restriction }, { data: profile }] = await Promise.all([
      ...
      supabase.from('user_profiles').select('role, track, class_name, onboarding_completed, is_approved_teacher, approved_by, approved_at, approved_track, approval_notes, active_level_id, active_group_id, profile_picture_url').eq('user_id', ctx.userId).maybeSingle()
    ]);
