 import { supabase, hashToken, parseCookies, getClientIp, generateSessionToken, generateCsrfToken, isAdmin, getAuthOnlyClient, maskEmail, auditLog, checkPasswordBreached, verifyTotp } from './core.js';
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET' && path === 'get_user') {
    ...
    return res.status(200).json({
      user: {
        ...
        profile: profile || { role: 'student', track: null, class_name: null, onboarding_completed: false }
      }
    });
  }
  ...
  if (req.method === 'POST' && path === 'signup') {
    ...
    const { email, password, turnstile_token, full_name, role, track, class_name } = body;
    console.error('[SIGNUP] start', JSON.stringify({ email: maskEmail(email), hasPassword: !!password, hasTurnstileToken: !!turnstile_token }));
    ...
    if (data.user) {
      const onboardingComplete = !!(role && track && class_name);
      const profilePayload = {
        user_id: data.user.id,
        role: role || 'student',
        track: track || 'O-Level',
        class_name: class_name || 'Form 1',
    ...
    if (data.session) {
      ...
      return res.status(200).json({ user: { id: data.user.id, email: data.user.email, full_name: trimmedName }, csrf_token });
    }
    return res.status(200).json({ user: null, message: 'If this email is valid, please check your inbox to confirm your account.' });
  }
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    const { data: restriction, error: restrictionError } = await supabase.from('user_restrictions').select('restriction_type, lock_reason, expires_at').eq('user_id', data.user.id).maybeSingle();
    ...
    const { error: terminateError } = await supabase.from('user_sessions').update({ is_active: false }).eq('user_id', data.user.id).eq('is_active', true);
    ...
    const session = await createUserSession(data.user.id, ip, req.headers['user-agent'], req, mfaWasVerified);
    setSessionCookie(res, session.access_token);
    ...
    await auditLog({ actorId: data.user.id, action: 'signin', ip, metadata: { mfa: mfaWasVerified } });
  ...
  if (req.method === 'POST' && path === 'change_password') {
    ...
    const { error: terminateError } = await supabase.from('user_sessions').update({ is_active: false }).eq('user_id', ctx.userId).eq('is_active', true).neq('id', ctx.sessionId);
  ...
  if (req.method === 'POST' && path === 'handoff_exchange') {
    ...
    const session = await createUserSession(row.user_id, ip, req.headers['user-agent'], req, !!adminData.mfa_enabled);
    setSessionCookie(res, session.access_token);
    const csrf_token = generateCsrfToken(session.csrf_secret);
...
async function createUserSession(userId, ip, userAgent, req, mfaVerified = false) {
