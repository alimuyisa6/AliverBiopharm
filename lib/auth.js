import { supabase, hashToken, parseCookies, getClientIp, getIpNetwork, generateSessionToken, generateCsrfToken, isAdmin, getAuthOnlyClient, maskEmail, auditLog, checkPasswordBreached, verifyTotp } from './core.js';
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET' && path === 'get_user') {
    ...
    const { data: profile } = await supabase
      .from('user_profiles')
      .select('role, track, class_name, onboarding_completed, is_approved_teacher, approved_by, approved_at, approved_track, approval_notes')
      .eq('user_id', ctx.userId)
      .maybeSingle();
  ...
  if (req.method === 'POST' && path === 'signup') {
    ...
    // Note: role/track/class_name are intentionally NOT accepted here.
    // A user's real level/class can only be set through the validated
    // save_onboarding endpoint (which checks class_sequence /
    // pharmacy_programs). Trusting client-supplied values at signup would
    // let a new account grant itself e.g. Pharmacy/Degree access before
    // onboarding or any approval.
    const { email, password, turnstile_token, full_name } = body;
    ...
    if (data.user) {
      // Placeholder only — role/track/class_name here grant no real access
      // because onboarding_completed is false. canAccessContent requires
      // onboarding_completed before honoring track/class for a student.
      const profilePayload = {
        user_id: data.user.id,
        role: 'student',
        track: 'O-Level',
        class_name: 'Form 1',
        onboarding_completed: false, // Always false to force onboarding
        is_approved_teacher: false,
        approved_track: null,
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    const { data: profile } = await supabase
      .from('user_profiles')
      .select('role, track, class_name, onboarding_completed, is_approved_teacher, approved_track')
      .eq('user_id', data.user.id)
      .maybeSingle();
    ...
    return res.status(200).json({
      ...
      csrf_token,
      // Tell frontend if teacher is pending approval
...
async function createUserSession(userId, ip, userAgent, req, mfaVerified = false, isAdminSession = false) {
  ...
  const { error } = await supabase.from('user_sessions').insert({
    ...
    mfa_verified: mfaVerified, ip_network: ipNetwork, network_mismatch_count: 0,
...
function setSessionCookie(res, token, maxAgeMs) {
  ...
  res.setHeader('Set-Cookie', `session=${token}; HttpOnly; Secure; SameSite=None; Max-Age=${maxAgeSeconds}; Path=/`);
}
