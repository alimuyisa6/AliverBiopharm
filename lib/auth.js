...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    if (adminCandidate && adminCandidate.ip_rejected) {
    ...
    if (adminCandidate && adminCandidate.passkey_enabled) {
      if (!passkey_token) {
        return res.status(200).json({ mfa_required: false, passkey_required: true });
      }
    ...
    const passkeyWasVerified = !!(adminCandidate && adminCandidate.passkey_enabled);
    ...
    await auditLog({ actorId: data.user.id, action: 'signin', ip, metadata: { mfa: mfaWasVerified, passkey: passkeyWasVerified, admin_session: isAdminSession } });
  ...
  if (req.method === 'POST' && path === 'handoff_exchange') {
    ...
    if (!adminData || !adminData.admin_role || adminData.is_locked || adminData.ip_rejected) {
      await auditLog({ actorId: row.user_id, action: 'handoff_exchange_denied', ip, metadata: { locked: !!adminData?.is_locked, ip_rejected: !!adminData?.ip_rejected } });
