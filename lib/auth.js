...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET' && path === 'get_user') {
    ...
    console.error('[GET_USER DEBUG]', JSON.stringify({
      userId: ctx.userId,
      hasAdminData: !!ctx.adminData,
      adminWarning: ctx.adminWarning
    }));
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    console.error('[SIGNIN RESTRICTION CHECK]', JSON.stringify({
      userId: data.user.id,
      restriction,
    ...
    try {
      const rawUrl = `${process.env.SUPABASE_URL}/rest/v1/user_restrictions?user_id=eq.${data.user.id}`;
      ...
      console.error('[RAW REST CHECK]', JSON.stringify({ status: rawRes.status, body: rawText }));
    } catch (e) {
      console.error('[RAW REST CHECK ERROR]', e.message);
    }
  ...
  if (req.method === 'POST' && path === 'handoff_create') {
    console.error('[HANDOFF DEBUG]', JSON.stringify({
      authenticated: ctx.authenticated,
      userId: ctx.userId,
      hasAdminData: !!ctx.adminData,
      adminWarning: ctx.adminWarning,
      adminLocked: ctx.adminLocked,
