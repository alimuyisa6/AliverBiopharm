 import { supabase, parseCookies, hashToken, getClientIp, verifyTurnstile, generateSessionToken, generateCsrfToken, isAdmin, getAuthOnlyClient } from './core.js';
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST' && path === 'signup') {
    ...
    console.error('[SIGNUP] turnstile result', JSON.stringify({ turnstileOk }));
  ...
  if (req.method === 'POST' && path === 'signin') {
    ...
    const turnstileOk = await verifyTurnstile(turnstile_token, ip);
    console.error('[SIGNIN] turnstile result', JSON.stringify({ turnstileOk }));
    if (!turnstileOk) throw new SecurityError('Captcha verification failed. Please try again.', 400);
