 import { supabase, parseCookies, hashToken, getClientIp, verifyTurnstile, generateSessionToken } from './core.js';
...
async function createUserSession(userId, ip, userAgent, req) {
  ...
  await supabase.from('user_sessions').insert({
    ...
    is_active: true, fingerprint, created_at: new Date().toISOString()
  });
  return { access_token: sessionToken, expires_at: expiresAt };
}

function setSessionCookie(res, token) {
  res.setHeader('Set-Cookie', `session=${token}; HttpOnly; Secure; SameSite=Strict; Max-Age=${7 * 24 * 60 * 60}; Path=/`);
}
