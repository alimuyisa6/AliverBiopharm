...
import { getUnitAccessInfo } from './curriculum.js';
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      case 'check': return checkAccess(req, res, ctx);
      case 'grants': requireSuperAdmin(ctx); return listGrants(req, res);
      case 'restrictions': requireSuperAdmin(ctx); return listRestrictions(req, res);
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      case 'grant': return grantPremium(body, res, ctx);
      case 'revoke_grant': return revokeGrant(body, res);
      case 'restrict': return restrictContent(body, res, ctx);
      case 'remove_restriction': return removeRestriction(body, res);
      default: throw new SecurityError('Invalid action', 400);
    }
  }
...
export async function checkUnitBlockAccess(userEmail, userId, unitId, blockNumber) {
  const unitInfo = await getUnitAccessInfo(unitId);
  if (!unitInfo) return { allowed: false, reason: 'unit_not_found' };

  const restricted = await isRestricted(userId, 'unit', unitId, 'view');
  if (restricted) return { allowed: false, reason: 'restricted' };

  if (unitInfo.is_premium) {
    const granted = await hasPremiumAccess(userEmail, 'unit', unitId);
    if (!granted) return { allowed: false, reason: 'premium_locked', scope: 'unit' };
    return { allowed: true };
  }
  ...
  if (blockNumber !== undefined && blockNumber !== null) {
    ...
    if (block?.is_premium) {
      ...
      if (!granted) return { allowed: false, reason: 'premium_locked', scope: 'block' };
    }
  }
...
export async function recordAbuseProbe(userId, flagType) {
  ...
  if (existing) {
    ...
    await supabase.from('abuse_flags').update({ count: newCount }).eq('id', existing.id);
  ...
  await supabase.from('abuse_flags').insert({ user_id: userId, flag_type: flagType, count: 1, window_start: new Date().toISOString() });
  return 1;
}
...
async function checkAccess(req, res, ctx) {
  ...
  if (access_type === 'download') {
    result = await checkDownloadAccess(email, ctx.userId, content_type, content_id, req.query.is_premium === 'true');
  } else {
    result = await checkContentAccess(email, ctx.userId, content_type, content_id, req.query.is_premium === 'true');
  }
...
async function listGrants(req, res) {
  ...
  let query = supabase.from('premium_grants').select('*').order('granted_at', { ascending: false });
  if (email) query = query.eq('email', email.trim().toLowerCase());
  const { data, error } = await query;
  if (error) throw new SecurityError('Failed to fetch grants', 500);
  return res.status(200).json(data || []);
}

async function listRestrictions(req, res) {
  const { user_id } = req.query;
  let query = supabase.from('content_restrictions').select('*').order('created_at', { ascending: false });
  if (user_id) query = query.eq('user_id', user_id);
  const { data, error } = await query;
  if (error) throw new SecurityError('Failed to fetch restrictions', 500);
  return res.status(200).json(data || []);
}

async function grantPremium(body, res, ctx) {
  const { email, content_type, content_id, expires_at, notes } = body;
  if (!email || !content_type || !content_id) throw new SecurityError('email, content_type, content_id required', 400);
  if (!['unit', 'block', 'pdf', 'note', 'past_paper'].includes(content_type)) {
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) throw new SecurityError('Failed to grant premium access', 500);
...
async function revokeGrant(body, res, ctx) {
  ...
  if (error) throw new SecurityError('Failed to revoke grant', 500);
...
async function restrictContent(body, res, ctx) {
  ...
  if (!['view', 'download'].includes(restriction_type)) throw new SecurityError('Invalid restriction_type', 400);

  const { data, error } = await supabase
    .from('content_restrictions')
    .insert({
      user_id, content_type, content_id: content_id || null,
      restriction_type, reason: reason || null,
      restricted_by: ctx.userId,
      expires_at: expires_at || null
    })
    .select()
    .single();
  if (error) throw new SecurityError('Failed to restrict content', 500);
...
async function removeRestriction(body, res, ctx) {
  const { id } = body;
  if (!id) throw new SecurityError('id required', 400);
  const { error } = await supabase.from('content_restrictions').delete().eq('id', id);
  if (error) throw new SecurityError('Failed to remove restriction', 500);
