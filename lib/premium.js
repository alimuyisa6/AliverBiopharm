...
// A user unlocks premium content two ways: a per-content grant in
// premium_grants (unlocks one piece of content), or a platform-wide
// active subscription (unlocks everything marked premium). Either is
// sufficient.
async function hasPremiumEntitlement(email, userId, contentType, contentId) {
  const [grant, subscription] = await Promise.all([
    hasPremiumAccess(email, contentType, contentId),
    hasActiveSubscription(userId),
  ]);

  return grant || subscription.has_premium;
}
...
export async function checkUnitBlockAccess(userEmail, userId, unitId, blockNumber) {
  ...
  if (unit.is_premium) {
    const granted = await hasPremiumEntitlement(userEmail, userId, 'unit', unitId);
  ...
  if (blockNumber !== undefined && blockNumber !== null) {
    ...
    if (block?.is_premium) {
      const granted = await hasPremiumEntitlement(userEmail, userId, 'block', unitId);
...
export async function checkContentAccess(userEmail, userId, contentType, contentId, isPremium) {
  ...
  if (isPremium) {
    const granted = await hasPremiumEntitlement(userEmail, userId, contentType, contentId);
