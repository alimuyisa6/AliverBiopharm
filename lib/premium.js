...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    ...
    case 'revoke_grant':        return revokeGrant(body, res, ctx);
    case 'restrict':            return restrictContent(body, res, ctx);
    case 'remove_restriction':  return removeRestriction(body, res, ctx);
    default: throw new SecurityError('Invalid action', 400);
  }
}
...
async function getUnitGroupId(contentType, contentId) {
  ...
  // note, pdf, past_paper – join through unit
...
async function ensureContentInUserScope(ctx, contentType, contentId) {
  if (!ctx.authenticated || ctx.adminData) return; // admins bypass scope check
  ...
  if (!CONTENT_UNIT_QUERY[contentType]) return; // type not mapped, allow

  const contentGroupId = await getUnitGroupId(contentType, contentId);
  if (!contentGroupId) return; // if we can't resolve, allow for now (or could block)
...
export async function checkUnitBlockAccess(userEmail, userId, unitId, blockNumber) {
  // (unchanged except we don't enforce curriculum scope here – caller must do it)
  const { data: unit } = await supabase
    .from('curriculum_units')
