  ...
  try {
    ...
    switch (path) {
      case 'session': {
        ...
        result = await handleGetSession(ctx.userId, { level: level || userLevel, topic });
        break;
      }
      ...
      case 'session_check': {
        ...
        result = await handleSessionCheck(ctx.userId, { level: level || userLevel, topic });
        break;
      }
      ...
      case 'topics': {
        ...
        result = await getTopicsForLevel(level);
        break;
      }
...
// Helper function to get user's effective level
