...
async function selectQuestionsForUser(userId, level, topic, limit = 20) {
  try {
    ...
    if (topic) dueQuery = dueQuery.eq('topic', topic);
    const { data: due } = await dueQuery;
    const dueIds = (due || []).map(r => Number(r.question_id));
    let selectedIds = [...dueIds];
    ...
    if (selectedIds.length < limit) {
      ...
      let newQuery = supabase
        ...
        .eq('level', level)
        .eq('is_active', true);
      if (topic) newQuery = newQuery.eq('topic', topic);
      if (selectedIds.length > 0) newQuery = newQuery.not('id', 'in', selectedIds);
      const { data: newQuestions } = await newQuery;
      const newIds = shuffleArray((newQuestions || []).map(q => Number(q.id))).slice(0, needed);
      selectedIds = [...selectedIds, ...newIds];
    }

    if (!selectedIds.length) return [];

    const { data: weak } = await supabase.from('user_weak_concepts').select('concept').eq('user_id', userId);
    const weakSet = new Set((weak || []).map(w => w.concept));
    const { data: questions } = await supabase
      .from('recall_questions_bank')
      .select('id, correct_answer')
      .in('id', selectedIds);
    const questionMap = new Map((questions || []).map(q => [Number(q.id), q.correct_answer]));

    const weakFirst = [...selectedIds].sort((a, b) => {
      const aWeak = weakSet.has(questionMap.get(a)) ? 1 : 0;
      const bWeak = weakSet.has(questionMap.get(b)) ? 1 : 0;
      return bWeak - aWeak;
    });
    ...
    for (const id of weakFirst) {
      if (!used.has(id)) { final.push(id); used.add(id); }
    }
    return final.slice(0, limit);
  } catch (e) {
    console.error('[SR] selectQuestionsForUser failed:', e.message);
    ...
    return shuffleArray((data || []).map(q => Number(q.id))).slice(0, limit);
  }
}
...
export async function handleGetSession(userId, { level, topic }) {
  ...
  const allQuestionIds = await selectQuestionsForUser(userId, level, topic, 20);
  if (!allQuestionIds.length) throw new Error('No questions available for this topic yet.');
  ...
  const { data: questions } = await supabase
    ...
    .in('id', firstBatchIds);
  const ordered = firstBatchIds.map(id => questions?.find(q => Number(q.id) === id)).filter(Boolean);
  return {
