...
async function getDashboard(req, res, ctx) {
  const [statsRow, scope] = await Promise.all([
...
async function listLeaderboard(req, res, ctx) {
  ...
  const entries = await getLeaderboardEntries(level || null, limit || 10);
...
async function submitAnswer(body, res, ctx) {
  ...
  const [newTotalXp] = await Promise.all([
    strength.xp > 0 ? addXp(ctx.userId, strength.xp, 'recall_answer', session_id, question_id) : Promise.resolve(null),
...
async function completeSession(body, res, ctx) {
  ...
  await updateTopicPerformance(ctx.userId, session.level, session.topic, report.mastery_score);
  const streakInfo = await recordSessionActivity(ctx.userId, userAnswers);
  ...
  const { data: freshStats } = await supabase
    ...
    .eq('user_id', ctx.userId)
    .maybeSingle();
  const newlyAwarded = freshStats ? await checkAndAwardAchievements(ctx.userId, freshStats) : [];
