import { supabase, addXp, canAccessLevel, isAdmin, isValidLevel } from './core.js';
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'first_visit': return firstVisit(req, res, ctx);
      case 'get_selected_level': return getSelectedLevel(req, res, ctx);
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      ...
      case 'set_selected_level': return setSelectedLevel(body, res, ctx);
...
async function getSession(req, res, ctx) {
  ...
  const { data: unit } = await supabase
    ...
    .select('id, name, curriculum_groups(level_id)')
    .eq('id', unit_id)
    .maybeSingle();
  if (!unit) throw new SecurityError('Unit not found', 404);

  const canAccess = await canAccessLevel(ctx.userId, unit.curriculum_groups?.level_id);
  ...
  const { data: session } = await supabase
    ...
    .eq('level', unit.curriculum_groups?.level_id)
    .eq('topic', unit.name)
    .eq('is_active', true)
...
async function getAchievements(req, res, ctx) {
  ...
  const { data: achievements } = ids.length ? await supabase.from('achievements').select('*').in('id', ids) : { data: [] };
...
async function getTopics(req, res, ctx) {
  ...
  else if (level_id) query = query.filter('curriculum_groups.level_id', 'eq', level_id);

  const { data: units } = await query;
  if (!units) return res.status(200).json([]);

  // For each unit, get count of recall questions
  const topics = [];
  for (const unit of units) {
    const { count } = await supabase
      ...
      .eq('unit_id', unit.id)
      .eq('is_active', true);
    if (count) topics.push({
      unit_id: unit.id,
      topic_name: unit.name,
      question_count: count,
      level: level_id || null,
      class_name: group_id || null
    });
  }
...
async function firstVisit(req, res, ctx) {
  const { level } = req.query;
  if (!isValidLevel(level)) throw new SecurityError('Invalid level', 400);
  const canAccess = await canAccessLevel(ctx.userId, level);
  if (!canAccess) throw new SecurityError('Access denied', 403);

  // Simple flag: no session ever created for this level
  ...
  return res.status(200).json({ firstVisit: !anySession || anySession.length === 0 });
}

async function getSelectedLevel(req, res, ctx) {
  const { data } = await supabase
    .from('user_recall_stats')
    .select('selected_level')
    .eq('user_id', ctx.userId)
    .maybeSingle();
  return res.status(200).json({ selected_level: data?.selected_level || null });
}

async function setSelectedLevel(body, res, ctx) {
  const { level } = body;
  if (!isValidLevel(level)) throw new SecurityError('Invalid level', 400);
  const canAccess = await canAccessLevel(ctx.userId, level);
  if (!canAccess) throw new SecurityError('Access denied', 403);
  ...
  await supabase
    .from('user_recall_stats')
    .upsert({ user_id: ctx.userId, selected_level: level, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
...
async function submitAnswer(body, res, ctx) {
  ...
  if (!session_id || !question_id || user_answer === undefined) throw new SecurityError('session_id, question_id, user_answer required', 400);
...
// Notification helpers embedded here but using the notifications library
