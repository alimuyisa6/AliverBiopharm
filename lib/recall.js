...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    case 'session':         return await getSession(req, res, ctx);
    case 'session_check':   return await checkSession(req, res, ctx);
    case 'stats':           return await getStats(req, res, ctx);
    case 'achievements':    return await getAchievements(req, res, ctx);
    case 'dashboard':       return await getDashboard(req, res, ctx);
    case 'topics':          return await getTopics(req, res, ctx);
    case 'due_reviews':     return await getDueReviews(req, res, ctx);
    case 'notifications':   return await getNotifications(req, res, ctx);
    case 'notification_prefs': return await getNotificationPrefs(req, res, ctx);
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    case 'start':                 return await startSession(body, res, ctx);
    case 'continue':              return await continueSession(body, res, ctx);
    case 'answer':                return await submitAnswer(body, res, ctx);
    case 'complete':              return await completeSession(body, res, ctx);
    case 'notification_read':     return await markNotificationRead(body, res, ctx);
    case 'notification_read_all': return await markAllNotificationsRead(res, ctx);
    case 'notification_dismiss':  return await dismissNotification(body, res, ctx);
    case 'notification_prefs_update': return await updateNotificationPrefs(body, res, ctx);
    default: throw new SecurityError('Invalid action', 400);
  }
}
...
async function getActiveGroupUnits(ctx) {
  try {
    ...
    console.error('[getActiveGroupUnits]', err);
    throw new SecurityError('Failed to retrieve curriculum units', 500);
  }
}
...
async function getSession(req, res, ctx) {
  try {
    ...
    console.error('[recall:getSession]', err);
    if (err instanceof SecurityError) throw err;
    throw new SecurityError('Failed to retrieve session', 500);
  }
}
...
async function checkSession(req, res, ctx) {
  try {
    ...
    console.error('[recall:checkSession]', err);
    if (err instanceof SecurityError) throw err;
    throw new SecurityError('Failed to check session', 500);
  }
}
...
async function getStats(req, res, ctx) {
  try {
    ...
    console.error('[recall:getStats]', err);
    throw new SecurityError('Failed to retrieve stats', 500);
  }
}
...
async function getAchievements(req, res, ctx) {
  try {
    ...
    console.error('[recall:getAchievements]', err);
    throw new SecurityError('Failed to retrieve achievements', 500);
  }
}
...
async function getDashboard(req, res, ctx) {
  try {
    ...
    console.error('[recall:getDashboard]', err);
    throw new SecurityError('Failed to load dashboard', 500);
  }
}
  ...
  try {
    ...
    for (const unit of allowedUnits) {
      const { count } = await supabase
        ...
        .eq('is_active', true);
      if (count) {
        units.push({ unit_id: unit.id, topic_name: unit.name, question_count: count });
      }
    }
    return res.status(200).json({ level, units });
  } catch (err) {
    console.error('[recall:getTopics]', err);
    throw new SecurityError('Failed to retrieve topics', 500);
  }
}
...
async function getDueReviews(req, res, ctx) {
  try {
    ...
    console.error('[recall:getDueReviews]', err);
    throw new SecurityError('Failed to retrieve reviews', 500);
  }
}
...
async function startSession(body, res, ctx) {
  try {
    ...
    const groupId = unit.curriculum_groups?.level_id ? unit.group_id : null;
    ...
    console.error('[recall:startSession]', err);
    if (err instanceof SecurityError) throw err;
...
async function continueSession(body, res, ctx) {
  try {
    ...
    console.error('[recall:continueSession]', err);
    if (err instanceof SecurityError) throw err;
    throw new SecurityError('Failed to continue session', 500);
  }
}
...
async function submitAnswer(body, res, ctx) {
  try {
    ...
    const newTotalXp = strength.xp > 0
      ? await addXp(ctx.userId, strength.xp, 'recall_answer', 'recall', { session_id, question_id }, session.unit_id, session.group_id, session.level)
      : null;
    ...
    if (strength.strength === 'developing') {
      await recordWeakConcept(ctx.userId, 'recall', session.unit_id, session.group_id, session.level, strength.matched || question.correct_answer, false, question_id);
    }
    ...
    console.error('[recall:submitAnswer]', err);
    if (err instanceof SecurityError) throw err;
    throw new SecurityError('Failed to submit answer', 500);
  }
}
...
async function completeSession(body, res, ctx) {
  try {
    ...
    const sessionXpEarned = userAnswers.reduce((sum, a) => sum + (a.xp_earned || 0), 0);
    ...
    await Promise.all([
      ...
      sessionXpEarned > 0 ? updateTopicXp(ctx.userId, session.unit_id, session.topic, sessionXpEarned) : Promise.resolve(null),
    ]);

    const scope = await getUserCurriculumScope(ctx.userId);
    await updateDailyChallengeProgress(ctx.userId, scope?.active_group_id, 'sessions_completed', 1).catch(() => null);
    ...
    console.error('[recall:completeSession]', err);
    if (err instanceof SecurityError) throw err;
...
async function getNotifications(req, res, ctx) {
  try {
    ...
    console.error('[recall:getNotifications]', err);
    if (err instanceof SecurityError) throw err;
    throw new SecurityError('Failed to fetch notifications', 500);
  }
}
...
async function markNotificationRead(body, res, ctx) {
  try {
    ...
    console.error('[recall:markNotificationRead]', err);
    throw new SecurityError('Failed to mark notification as read', 500);
  }
}
...
async function markAllNotificationsRead(res, ctx) {
  try {
    ...
    console.error('[recall:markAllNotificationsRead]', err);
    throw new SecurityError('Failed to mark all notifications as read', 500);
  }
}
...
async function dismissNotification(body, res, ctx) {
  try {
    ...
    console.error('[recall:dismissNotification]', err);
    throw new SecurityError('Failed to dismiss notification', 500);
  }
}
...
async function getNotificationPrefs(req, res, ctx) {
  try {
    ...
    console.error('[recall:getNotificationPrefs]', err);
    throw new SecurityError('Failed to fetch notification preferences', 500);
  }
}
...
async function updateNotificationPrefs(body, res, ctx) {
  try {
    ...
    console.error('[recall:updateNotificationPrefs]', err);
    throw new SecurityError('Failed to update notification preferences', 500);
  }
}
