...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    case 'reviews':               return listReviews(req, res);
    case 'my_blocked':            requireAuth(ctx); return listBlocked(req, res, ctx);
    case 'my_minor_status':       requireAuth(ctx); return getMyMinorStatus(req, res, ctx);
    case 'admin_reports':         requireAdmin(ctx); return adminListReports(req, res);
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    case 'submit_review':          requireAuth(ctx); return submitReview(body, res, ctx);
    case 'report_user':            requireAuth(ctx); return reportUser(body, res, ctx);
    case 'block_user':             requireAuth(ctx); return blockUser(body, res, ctx);
    case 'unblock_user':           requireAuth(ctx); return unblockUser(body, res, ctx);
    case 'record_parental_consent':requireAuth(ctx); return recordParentalConsent(body, res, ctx);
    case 'admin_resolve_report':   requireAdmin(ctx); return adminResolveReport(body, res, ctx);
    case 'admin_set_verification_tier': requireAdmin(ctx); return adminSetVerificationTier(body, res, ctx);
    default: throw new SecurityError('Invalid action', 400);
  }
}

// ---- Minor safety ----
// Anyone flagged is_minor on their user_profile is gated behind a verified
// parental/guardian consent record before they can do anything that exposes
// them to direct contact (messaging a tutor, receiving a contact request,
// video sessions). This is intentionally strict: default-deny until consent
// is on file and verified.
...
async function getMyMinorStatus(req, res, ctx) {
  ...
  if (!profile?.is_minor) return res.status(200).json({ is_minor: false, gated: false });
...
async function recordParentalConsent(body, res, ctx) {
  ...
  const { data: profile } = await supabase
    ...
    .eq('user_id', ctx.userId)
    .maybeSingle();
  if (!profile?.is_minor) throw new SecurityError('Parental consent only applies to minor accounts', 400);

  // Consent starts as "pending" — a verification email/flow to the guardian
  // address must confirm it before isMinorSafetyBlocked() will clear.
  // (Actual email dispatch happens in the notifications/email service.)
  const { data: consent } = await supabase
    .from('parental_consents')
    .insert({
  ...
  return res.status(200).json({ success: true, consent_id: consent.id, status: 'pending' });
}

// ---- Reviews ----
// Reviews may only be left by someone with a completed, accepted contact
// request/session with the target — never open to anyone.

async function submitReview(body, res, ctx) {
  const { tutor_id, rating, comment } = body;
  if (!tutor_id || !rating) throw new SecurityError('tutor_id and rating required', 400);
  if (rating < 1 || rating > 5) throw new SecurityError('rating must be between 1 and 5', 400);
  if (comment && comment.length > 2000) throw new SecurityError('comment too long', 400);
...
async function reportUser(body, res, ctx) {
  ...
  if (!reported_user_id || !reason) throw new SecurityError('reported_user_id and reason required', 400);
  if (!REPORT_REASONS.includes(reason)) throw new SecurityError('Invalid reason', 400);
  if (reported_user_id === ctx.userId) throw new SecurityError('Cannot report yourself', 400);
  ...
  // Safety-critical reasons get flagged for expedited review rather than
  // sitting in the normal queue.
  const urgent = ['safety_concern', 'harassment'].includes(reason);
  if (urgent) {
    await supabase.from('user_reports').update({ priority: 'urgent' }).eq('id', report.id);
  }
...
async function adminResolveReport(body, res, ctx) {
  ...
  if (!report_id || !resolution) throw new SecurityError('report_id and resolution required', 400);
  if (!['dismissed', 'actioned'].includes(resolution)) throw new SecurityError('Invalid resolution', 400);
  ...
  if (resolution === 'actioned' && action_taken === 'suspend') {
    await supabase.from('user_profiles').update({ account_status: 'suspended' }).eq('user_id', report.reported_user_id);
...
async function adminSetVerificationTier(body, res, ctx) {
  ...
  if (!profile_id || !tier) throw new SecurityError('profile_id and tier required', 400);
  if (!VERIFICATION_TIERS.includes(tier)) throw new SecurityError('Invalid tier', 400);
  ...
  await supabase.from('tutor_profiles').update({ verification_tier: tier }).eq('id', profile_id);
