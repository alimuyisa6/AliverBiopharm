...
// How long a just-rotated token hash stays acceptable. Covers concurrent
// in-flight requests (e.g. several dashboard fetches firing in parallel on
// page load) that are still holding the pre-rotation cookie.
...
export async function createAuthenticatedContext(req, res) {
  ...
  // Match the session by CURRENT hash, or by its PREVIOUS hash if that
  // token was rotated within the last TOKEN_ROTATION_GRACE_MS. Without the
  // second branch, any request still holding the pre-rotation cookie (e.g.
  // a sibling request that fired in parallel a moment before another
  // request triggered rotation) gets silently treated as logged out.
  ...
  // Only rotate if the CURRENT hash still equals the hash we validated with
  // (i.e. skip rotation entirely if we got here via the grace-window branch,
  // since that means another concurrent request already rotated this
  // session a moment ago). The .eq('session_token_hash', hashedToken) guard
  // also acts as a compare-and-swap: if two requests somehow race past the
  // sessionAge check simultaneously, only the first UPDATE actually matches
  // a row — the loser's update affects zero rows and is a no-op.
  if (
