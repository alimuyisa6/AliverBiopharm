...
export async function createAuthenticatedContext(req, res) {
  ...
  if (!sessionData.fingerprint || sessionData.fingerprint !== currentFingerprint) {
    await supabase
      ...
      .update({
        is_active: false,
        terminated_reason: 'fingerprint_mismatch',
        terminated_at: new Date().toISOString()
      })
    ...
    await auditLog({
      ...
      action: 'session_fingerprint_rejected',
      ip: clientIp
    });
  ...
  if (sessionData.ip_network && sessionData.ip_network !== currentIpNetwork) {
    ...
    if (newCount >= NETWORK_MISMATCH_THRESHOLD) {
      await supabase
        ...
        .update({
          is_active: false,
          terminated_reason: 'network_mismatch_threshold',
          terminated_at: new Date().toISOString()
        })
        .eq('id', sessionData.id);
...
export class RateLimiter {
  ...
  async check(ip, userId, action = null) {
    ...
    if (REDIS_ENABLED) {
      return this.checkRedis(ip, userId, action);
    }
