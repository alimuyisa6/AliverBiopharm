...
export function getSessionFingerprint(req, userId) {
  ...
  const raw = `${userId}:${userAgent}:${SESSION_FINGERPRINT_SALT}`;
...
export async function createAuthenticatedContext(req, res) {
  ...
  // Get client IP early so it's available for both fingerprint-mismatch and normal paths
  const clientIp = coreGetClientIp(req);

  // FIX: Still fetch admin data even when fingerprint doesn't match
  // Previously this returned adminData: null unconditionally, breaking admin access
  ...
  // FIX: Pass client IP to isAdmin so IP whitelist checks work correctly
  const adminData = await coreIsAdmin(session.user_id, clientIp);
