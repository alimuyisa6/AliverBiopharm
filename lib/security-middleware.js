...
export function getSessionFingerprint(req, userId) {
  ...
  const raw = `${userId}:${userAgent}:${acceptLanguage}:${ip}:${SESSION_FINGERPRINT_SALT}`;
...
export function enforceSecurityHeaders(req, res) {
  ...
  if (!res.getHeader('Content-Security-Policy')) {
    res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; frame-src 'none'; object-src 'none'");
  }
}
...
export async function createAuthenticatedContext(req, res) {
  ...
  if (!token) {
    return { userId: null, adminData: null, csrfSecret: null, sessionId: null, authenticated: false, fingerprint: null, adminLocked: false };
  }
  ...
  if (sessionData.fingerprint && sessionData.fingerprint !== currentFingerprint) {
    await supabase.from('user_sessions').update({ is_active: false, terminated_reason: 'fingerprint_mismatch', terminated_at: new Date().toISOString() }).eq('id', sessionData.id);
    res.setHeader('Set-Cookie', 'session=; HttpOnly; Secure; SameSite=Strict; Max-Age=0; Path=/');
    return { userId: null, adminData: null, csrfSecret: null, sessionId: null, authenticated: false, fingerprint: null, adminLocked: false, fingerprintRejected: true };
  }

  if (!sessionData.fingerprint) {
    await supabase.from('user_sessions').update({ fingerprint: currentFingerprint }).eq('id', sessionData.id);
  }
  ...
  if (sessionAge > MAX_SESSION_AGE_MS / 2 && sessionAge <= MAX_SESSION_AGE_MS) {
    ...
    await supabase.from('user_sessions').update({ session_token_hash: newHashedToken, expires_at: newExpiresAt, fingerprint: currentFingerprint }).eq('id', sessionData.id);
    res.setHeader('Set-Cookie', `session=${newToken}; HttpOnly; Secure; SameSite=Strict; Max-Age=${Math.floor(MAX_SESSION_AGE_MS / 1000)}; Path=/`);
  }

  const adminData = await coreIsAdmin(session.user_id, ip);
...
export class RateLimiter {
  ...
  cleanup() {
    ...
    for (const [key, window] of this.windows.entries()) {
      if (now - window.lastAccess > Math.max(this.IP_BURST_WINDOW_MS, this.USER_MINUTE_WINDOW_MS, this.AUTH_ATTEMPT_WINDOW_MS) * 2) {
