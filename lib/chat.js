 import { supabase, isAdmin as coreIsAdmin } from './core.js';
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'admin_get_pending_requests': return adminGetPendingRequests(req, res, ctx);
      case 'admin_get_active_chats': return adminGetActiveChats(req, res, ctx);
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      ...
      case 'admin_accept_chat': return adminAcceptChat(body, res, ctx);
      case 'admin_reject_chat': return adminRejectChat(body, res, ctx);
      case 'admin_update_presence': return adminUpdatePresence(body, res, ctx);
      default: throw new SecurityError('Invalid action', 400);
    }
  }
...
async function getChatMessages(req, res, ctx) {
  ...
  const adminData = await coreIsAdmin(ctx.userId, ctx.fingerprint || 'unknown');
  if (room.user_id !== ctx.userId && !adminData) throw new SecurityError('Access denied', 403);
...
async function sendChatMessage(body, res, ctx) {
  ...
  if (!room) throw new SecurityError('Room not found', 404);
  if (room.user_id !== ctx.userId && !ctx.adminData) throw new SecurityError('Access denied', 403);
  const senderType = ctx.adminData ? 'admin' : 'user';
...
async function adminAcceptChat(body, res, ctx) {
  ...
  const { room_id } = body;
  if (!room_id) throw new SecurityError('room_id required', 400);
  const { error } = await supabase.from('chat_rooms').update({ status: 'active', assigned_admin: ctx.adminData.id }).eq('id', room_id);
