...
async function requestChat(body, res, ctx) {
  ...
  if (existing) {
    return res.status(200).json({ room_id: existing.id, status: existing.status });
  }
...
async function getChatMessages(req, res, ctx) {
  ...
  const { data: room } = await supabase
    .from('chat_rooms')
    .select('user_id, assigned_admin')
    .eq('id', room_id)
    .maybeSingle();

  if (!room) throw new SecurityError('Room not found', 404);
  if (room.user_id !== ctx.userId && !isFullyAuthorizedAdmin(ctx)) {
    throw new SecurityError('Access denied', 403);
  }

  const { data, error } = await supabase
    .from('chat_messages')
    .select('id, sender_type, content, created_at, deleted_by_user')
    .eq('room_id', room_id)
...
async function sendChatMessage(body, res, ctx) {
  ...
  if (room.user_id !== ctx.userId && !senderIsAdmin) {
    throw new SecurityError('Access denied', 403);
  }
  ...
  if (room.status === 'requested' && senderType === 'admin') {
    updatePromises.push(
      supabase
        .from('chat_rooms')
        .update({ status: 'active', assigned_admin: ctx.userId })
        .eq('id', room_id)
    );
  }
  if (senderType === 'admin') {
    updatePromises.push(createNotification(room.user_id, 'chat_message_received', {}));
  }
...
async function updateUserPresence(req, res, ctx) {
  await supabase
    .from('user_presence')
    .upsert(
      { user_id: ctx.userId, last_seen: new Date().toISOString() },
      { onConflict: 'user_id' }
    );
  return res.status(200).json({ success: true });
}
...
async function adminGetPendingRequests(req, res, ctx) {
  ...
  const rooms = await Promise.all(
    (data || []).map(async (room) => {
      const email = await getUserEmail(room.user_id);
      return { ...room, user_email: email };
    })
  );
...
async function adminAcceptChat(body, res, ctx) {
  ...
  const { data: room, error } = await supabase
    ...
    .update({
      status: 'active',
      assigned_admin: ctx.adminData.id,
    })
    .eq('id', room_id)
    .select('user_id')
    .single();
...
async function adminGetActiveChats(req, res, ctx) {
  const { data, error } = await supabase
    .from('chat_rooms')
    .select('id, user_id, status, created_at, requested_at')
    .eq('status', 'active')
