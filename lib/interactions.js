...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'platform-stats':            return getGlobalPlatformStats(req, res);
...
// ---------- global platform stats (site-wide content counts, distinct from per-user stats) ----------
...
// ---------- unified user stats (real schema: user_platform_stats / user_recall_stats / user_quiz_activity / note_reading_stats) ----------

async function getUserStreak(req, res, ctx) {
  const platform = await getPlatformStats(ctx.userId);
  return res.status(200).json({ count: platform.current_streak || 0 });
}
...
async function getRecallSummary(userId) {
  const { data } = await supabase
    ...
    .select('total_sessions, best_mastery, mastery')
    .eq('user_id', userId)
    .maybeSingle();
  if (!data) return { total_sessions: 0, best_mastery: 0, topics_practiced: 0 };
  return {
...
async function getQuizSummary(userId) {
  const { data } = await supabase
    ...
    .limit(30);
  const rows = data || [];
  if (rows.length === 0) return { blocks_completed: 0, recent_pass_rate: 0, topics_attempted: 0 };
  const uniqueBlocks = new Set(rows.map(r => `${r.topic}:${r.block_number}`));
  return {
    blocks_completed: uniqueBlocks.size,
    recent_pass_rate: Math.round((rows.filter(r => r.passed).length / rows.length) * 100),
...
async function getUserDashboard(req, res, ctx) {
  ...
  return res.status(200).json({
    ...
    recall: { available: true, ...recall },
    quiz: { available: true, ...quiz },
    notes: { available: true, ...notes },
...
async function getContinueReading(req, res, ctx) {
  ...
  const notes = await getNotesSummary(ctx);
  return res.status(200).json(notes.continue_reading.slice(0, Number(limit) || 5));
}
