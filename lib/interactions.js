...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'dashboard': return getUserDashboard(req, res, ctx);
      case 'daily-challenge': return getDailyChallenge(req, res, ctx);
      case 'weak-areas': return getWeakAreas(req, res, ctx);
      case 'learning-paths': return getLearningPaths(req, res, ctx);
      case 'personal-records': return getPersonalRecords(req, res, ctx);
...
// ── Public Stats & Dashboard endpoints ─────────────────────────────
...
async function getLeaderboard(req, res) {
  ...
  // Placeholder; replace with actual RPC or query
  return res.status(200).json([]);
}
...
async function getUserDashboard(req, res, ctx) {
  requireAuth(ctx);
  return res.status(200).json({ xp: 0, streak: 0 });
}

async function getDailyChallenge(req, res, ctx) {
  requireAuth(ctx);
  return res.status(200).json({ title: '', completed: false });
}

async function getWeakAreas(req, res, ctx) {
  requireAuth(ctx);
  return res.status(200).json({ weak_topics: [] });
}
...
async function getPersonalRecords(req, res, ctx) {
  requireAuth(ctx);
  return res.status(200).json({ highest_score: 0, fastest_completion: 0, perfect_blocks: 0 });
}

// ── Reaction write handlers ───────────────────────────────────────
...
async function addComment(body, res, ctx) {
  ...
  validateContentType(content_type);
  const { data } = await supabase.from('content_comments').insert({
    user_id: ctx.userId, content_type, content_id, parent_comment_id: parent_comment_id || null, body: commentBody.trim()
  }).select().single();
  if (!data) throw new SecurityError('Failed to add comment', 500);
  await refreshEngagementSummary(content_type, content_id);
