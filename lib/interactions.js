...
const VALID_CONTENT_TYPES = ['note', 'quiz_batch', 'flashcard_deck', 'pdf_resource', 'past_paper', 'glossary_term', 'classroom', 'lab_case', 'lab_pathway', 'curriculum_unit', 'curriculum_group', 'weekly_challenge', 'video', 'article', 'content_collection'];
...
async function getSummary(req, res) {
  ...
  return res.status(200).json(data || {
    like_count: 0, love_count: 0, helpful_count: 0, bookmark_count: 0, helpful_yes_count: 0, helpful_no_count: 0, comment_count: 0
  });
}
...
async function addComment(body, res, ctx) {
  ...
  const { data, error } = await supabase.from('content_comments').insert({
    user_id: ctx.userId, content_type, content_id, parent_comment_id: parent_comment_id || null, body: commentBody.trim()
  }).select().single();
  if (error) throw new SecurityError('Failed to add comment', 500);
  await refreshEngagementSummary(content_type, content_id);
  if (parent_comment_id) {
    const { data: parent } = await supabase.from('content_comments').select('user_id').eq('id', parent_comment_id).maybeSingle();
    if (parent && parent.user_id !== ctx.userId) await createNotification(parent.user_id, 'comment_reply', {});
  }
...
async function refreshEngagementSummary(contentType, contentId) {
  ...
  for (const r of reactions || []) if (counts.hasOwnProperty(r.reaction_type)) counts[r.reaction_type]++;
  await supabase.from('content_engagement_summary').upsert({
    content_type: contentType, content_id: contentId,
    ...counts, comment_count: commentCount || 0, updated_at: new Date().toISOString()
