...
async function getResourceInteractions(req, res) {
  ...
  if (comments) {
    for (const c of comments) {
      try { const { data: { user } } = await supabase.auth.admin.getUserById(c.user_id); commentList.push({ comment: c.metadata?.comment || '', user_name: user?.email ? user.email.split('@')[0].replace(/[._]/g, ' ').replace(/\b\w/g, x => x.toUpperCase()) : 'User', created_at: c.created_at }); } catch { commentList.push({ comment: c.metadata?.comment || '', user_name: 'User', created_at: c.created_at }); }
    }
  }
...
async function getPublicStats(req, res) {
  try {
    ...
    return res.status(200).json({ resources_count: resCount.count || 0, downloads_count: downCount.count || 0, quiz_attempts: quizCount.count || 0, users_count: authUsers.data?.users?.length || 0 });
  } catch {
