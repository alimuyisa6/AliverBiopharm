...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'continue-reading':          requireAuth(ctx); return getContinueReading(req, res, ctx);
...
async function getUserDashboard(req, res, ctx) {
  ...
  const [xpData, streakRes, badgesRes, topicStatsRes, unitsRes, resumeRes] = await Promise.all([
    ...
    supabase.from('user_achievements').select('achievement_id', { count: 'exact', head: true }).eq('user_id', ctx.userId),
    supabase.from('user_topic_stats').select('topic, xp').eq('user_id', ctx.userId),
    scope?.active_group_id
      ? supabase.from('curriculum_units').select('id').eq('group_id', scope.active_group_id).eq('is_active', true)
      : Promise.resolve({ data: [] }),
    supabase.from('user_quiz_resume_state').select('state').eq('user_id', ctx.userId).maybeSingle(),
  ]);
  ...
  const completedTopics = topicStats.filter(t => (t.xp || 0) >= TOPIC_COMPLETE_XP).length;
  const totalTopics = (unitsRes.data || []).length;

  let nextGoal = null;
  const resumeState = resumeRes.data?.state;
  if (resumeState && resumeState.topic) {
    nextGoal = { topic: resumeState.topic, block: resumeState.block || 1 };
  } else if (topicStats.length) {
    const weakest = [...topicStats].sort((a, b) => (a.xp || 0) - (b.xp || 0))[0];
    nextGoal = { topic: weakest.topic, block: 1 };
  }
...
async function getWeakAreas(req, res, ctx) {
  ...
  const weakTopics = (data || []).map(t => t.topic);
  const recommendedBlock = weakTopics.length ? { topic: weakTopics[0], block: 1 } : null;

  return res.status(200).json({ weak_topics: weakTopics, recommended_block: recommendedBlock });
}
...
async function getContinueReading(req, res, ctx) {
  ...
  for (const row of (data || [])) {
    if (row.metadata?.content_type && row.metadata.content_type !== 'note') continue;
    if (seen.has(row.resource_id)) continue;
    seen.add(row.resource_id);

    const progress = row.metadata?.progress_percentage;
    if (progress !== undefined && progress >= 100) continue;
...
async function recordView(body, res, ctx) {
  const { resource_id, content_type = 'note', progress_percentage } = body;
