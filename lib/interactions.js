...
async function getRecentViews(req, res, ctx) {
  const { data } = await supabase.from('content_views').select('content_type, content_id, created_at').eq('user_id', ctx.userId).order('created_at', { ascending: false }).limit(10);
  return res.status(200).json(data || []);
}

async function getUserRatings(req, res, ctx) {
  const { data } = await supabase.from('content_ratings').select('content_type, content_id, rating').eq('user_id', ctx.userId);
  const map = {};
  (data || []).forEach(r => { map[r.content_id] = r.rating; });
...
async function recordView(body, res, ctx) {
  await supabase.from('content_views').insert({ content_type: body.content_type || 'note', content_id: body.resource_id, user_id: ctx.userId });
  return res.status(200).json({ success: true });
}
async function recordDownload(body, res, ctx) { return res.status(200).json({ success: true }); }
...
async function submitRating(body, res, ctx) { return res.status(200).json({ success: true }); }
...
async function saveAchievement(body, res, ctx) { return res.status(200).json({ success: true }); }
async function saveQuizState(body, res, ctx) { return res.status(200).json({ success: true }); }
async function trackEvent(body, res, ctx) { return res.status(200).json({ success: true }); }
