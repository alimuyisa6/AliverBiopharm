...
export async function handler(req, res, path, ctx) {
  const { userId, ip } = ctx;
  const publicPaths = ['get_public_stats', 'platform-stats'];
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      case 'toggle_favorite': return toggleFavorite(body, res, userId);
      case 'record_view': return recordView(body, res, userId);
      case 'record_download': return recordDownload(body, res, userId);
      case 'record_daily_visit': return recordDailyVisit(req, res, userId);
      case 'submit_rating': return submitRating(body, res, userId);
      case 'like_resource': return likeResource(body, res, userId);
      case 'comment_resource': return commentResource(body, res, userId);
      case 'submit_mood': return submitMood(body, res, userId);
      case 'save_achievement': return saveAchievement(body, res, userId);
      case 'update_user_presence': return updateUserPresence(req, res, userId);
      case 'save_quiz_state': return saveQuizState(body, res, userId);
      case 'track_event': return trackEvent(body, res, userId);
  ...
  if (req.method === 'GET') {
    switch (path) {
      ...
      case 'dashboard': return res.status(200).json(await getUserDashboard(userId));
      case 'daily-challenge': return res.status(200).json(await getDailyChallenge(userId));
      case 'weak-areas': return res.status(200).json(await getWeakAreas(userId));
      case 'learning-paths': {
      ...
      case 'personal-records': return res.status(200).json(await getPersonalRecords(userId));
      default: return res.status(400).json({ error: 'Invalid action' });
    }
  }
...
async function toggleFavorite(body, res, userId) {
...
async function submitRating(body, res, userId) {
...
async function likeResource(body, res, userId) {
  const { resource_id } = body;
  const { data: existing } = await supabase.from('user_interactions').select('id').eq('user_id', userId).eq('resource_id', resource_id).eq('interaction_type', 'favorite').maybeSingle();
  if (existing) {
    await supabase.from('user_interactions').delete().eq('id', existing.id);
...
async function commentResource(body, res, userId) {
...
async function submitMood(body, res, userId) {
...
async function saveAchievement(body, res, userId) {
  const { badge } = body;
...
async function saveQuizState(body, res, userId) {
  const { state } = body;
...
async function trackEvent(body, res, userId) {
