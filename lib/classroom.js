...
async function canAccessLevel(userId, level, ctx) {
  // Admin can access all
  if (ctx.adminData) return true;
  ...
  if (profile.role === 'teacher') {
    ...
    // Pharmacy teachers with ALL can access all
...
async function listClassrooms(req, res, ctx) {
  ...
  if (ctx.authenticated && !ctx.adminData) {
    const canAccess = await canAccessLevel(ctx.userId, level, ctx);
...
async function getLiveFeed(req, res, ctx) {
  // If authenticated, only show user's level content
  ...
  if (ctx.authenticated && !ctx.adminData) {
    const { data: profile } = await supabase
      ...
      .maybeSingle();
    if (profile) levelFilter = profile.track;
  }
...
async function getRoom(req, res, ctx) {
  ...
  // Check if user can access this room's level
  if (ctx.authenticated && !ctx.adminData) {
    const canAccess = await canAccessLevel(ctx.userId, data.level, ctx);
    if (!canAccess) throw new SecurityError('You do not have access to this level', 403);
  }
...
async function getMessages(req, res) {
  ...
  for (const msg of (data || [])) {
    const msgData = { ...msg };
    const profileName = await getUserProfileName(msg.user_id);
    msgData.sender_name = profileName || 'User';
...
async function getParticipants(req, res) {
  ...
  for (const p of (data || [])) {
    const pData = { ...p };
    const profileName = await getUserProfileName(p.user_id);
    pData.user_name = profileName || 'User';
...
async function isApprovedTutor(userId, level) {
  // Check user_profiles first (teacher approval)
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('is_approved_teacher, approved_track, role')
    .eq('user_id', userId)
    .maybeSingle();
  ...
  if (profile && profile.role === 'teacher') {
    if (!profile.is_approved_teacher) return false;
    if (profile.approved_track === 'ALL') return true;
    if (profile.approved_track === level) return true;
  }

  // Fall back to tutor_applications for legacy
  const { data } = await supabase
    .from('tutor_applications')
    .select('id, status')
    .eq('user_id', userId)
...
async function createRoom(body, res, ctx) {
  ...
  // Check if user is a rejected tutor
  ...
  // Check if user is approved (admin OR approved teacher OR approved tutor)
  const isAdmin = !!ctx.adminData;
  const isApproved = await isApprovedTutor(ctx.userId, level);

  if (!isAdmin && !isApproved) {
    throw new SecurityError('Only approved tutors, teachers, and admins can create rooms', 403);
  }
...
async function joinClassroom(body, res, ctx) {
  ...
  const { data: room } = await supabase
    .from('classrooms')
    .select('id, status, participant_count, max_participants, level')
    .eq('id', room_id)
    .maybeSingle();
  ...
  // Check if user can access this room's level
  if (!ctx.adminData) {
    const canAccess = await canAccessLevel(ctx.userId, room.level, ctx);
    if (!canAccess) throw new SecurityError('You do not have access to this level', 403);
  }
...
async function applyAsTutor(body, res, ctx) {
  ...
  // Update user_profiles to mark as teacher (pending approval)
  await supabase
    .from('user_profiles')
    .update({
      role: 'teacher',
...
async function reviewTutorApplication(body, res, ctx) {
  ...
  const { application_id, action, rejection_reason, interview_scheduled_at, hourly_rate, display_name, approved_track } = body;
  ...
  // Get application to know user_id
  const { data: appData } = await supabase
    .from('tutor_applications')
    .select('user_id, level, class_name')
    .eq('id', application_id)
    .maybeSingle();
  if (!appData) throw new SecurityError('Application not found', 404);
  ...
  if (action === 'schedule') {
    ...
    const track = approved_track || appData.level || 'O-Level';
    updates.status = 'approved';
    updates.display_name = display_name;
    updates.is_approved = true;
    if (hourly_rate) updates.hourly_rate = hourly_rate;

    // Update user_profiles to mark as approved teacher
    await supabase
      .from('user_profiles')
      .update({
    ...
    // Update user_profiles to mark as rejected
    await supabase
      .from('user_profiles')
      .update({
        is_approved_teacher: false,
  ...
  if (action === 'reject') {
    await supabase
      .from('classrooms')
      .update({ status: 'ended', ended_at: new Date().toISOString() })
      .eq('tutor_id', appData.user_id)
      .not('status', 'in', '("ended","offline")');
  }

  await createNotification(appData.user_id, 'tutor_application_update', { status: data.status });
...
async function adminListApplications(req, res, ctx) {
  ...
  for (const app of (data || [])) {
    const appData = { ...app };
    const profileName = await getUserProfileName(app.user_id);
    appData.applicant_email = profileName || 'Unknown';
...
async function adminListComplaints(req, res, ctx) {
  ...
  for (const c of (data || [])) {
    ...
    cData.complainant_email = profileName || 'Unknown';
