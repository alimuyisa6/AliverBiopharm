...
async function enrichRoomsWithTopicAndClass(rooms) {
  ...
  let groupMap = {};
  if (groupIds.length) {
    const { data: groups } = await supabase.from('curriculum_groups').select('id, name').in('id', groupIds);
...
async function listClassrooms(req, res, ctx) {
  ...
  return res.status(200).json(enriched.map(r => ({ ...r, tutor_name: tutorNames[r.tutor_id] || null })));
}
...
async function createRoom(body, res, ctx) {
  ...
  if (!profile?.is_approved_teacher && !isFullyAuthorizedAdmin(ctx)) throw new SecurityError('Only approved tutors and admins can create rooms', 403);
...
async function endRoom(body, res, ctx) {
  ...
  if (!room) throw new SecurityError('Room not found', 404);
  if (room.tutor_id !== ctx.userId && !isFullyAuthorizedAdmin(ctx)) throw new SecurityError('Only the tutor or admin can end the room', 403);
...
async function applyAsTutor(body, res, ctx) {
  ...
  if (!class_name || !subjects?.length) throw new SecurityError('level, class_name, subjects required', 400);
  const { data: profile } = await supabase.from('user_profiles').select('track').eq('user_id', ctx.userId).maybeSingle();
  if (profile?.track) {
    if (!level || level !== profile.track) throw new SecurityError('Submitted level does not match your signup level', 400);
  }
  ...
  if (!profile?.track) {
    ...
    await supabase.from('user_profiles').update({ role: 'teacher', class_name, is_approved_teacher: false }).eq('user_id', ctx.userId);
  }
