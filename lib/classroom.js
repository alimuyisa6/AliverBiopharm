...
async function getTopics(req, res) {
  const { level, group_id } = req.query;
  if (!level && !group_id) throw new SecurityError('level or group_id required', 400);
  ...
  if (group_id) {
    ...
    query = query.filter('curriculum_groups.level_id', 'eq', level);
  }
...
async function listClassrooms(req, res, ctx) {
  ...
  for (const tid of tutorIds) {
    const { data: tutor } = await supabase.from('tutor_applications').select('display_name').eq('user_id', tid).eq('status', 'approved').maybeSingle();
    tutorNames[tid] = tutor?.display_name || null;
  }
  const withNames = reconciled.map(r => ({ ...r, tutor_name: tutorNames[r.tutor_id] || null }));
...
async function getLiveFeed(req, res, ctx) {
  ...
  if (ctx.authenticated) {
    ...
    if (scope && scope.level) {
      const { data: units } = await supabase.from('curriculum_units').select('id').filter('curriculum_groups.level_id', 'eq', scope.level);
      unitIds = (units || []).map(u => u.id);
    }
  }
...
async function saveOnboarding(body, res, ctx) {
  ...
  await supabase.from('classroom_onboarding').insert({
    user_id: ctx.userId, level, class_name, selected_topic: topic || null, has_completed_onboarding: true
  });
...
async function createRoom(body, res, ctx) {
  ...
  const isApproved = await isApprovedTutor(ctx.userId, null);
  if (!isApproved && !ctx.adminData) throw new SecurityError('Only approved tutors and admins can create rooms', 403);
  ...
  await supabase.from('classroom_participants').insert({
    room_id: data.id, user_id: ctx.userId, role: ctx.adminData ? 'admin' : 'tutor', is_muted: false
  });
  return res.status(200).json(data);
}
...
async function leaveClassroom(body, res, ctx) {
  ...
  if (room) {
    await supabase.from('classrooms').update({ participant_count: Math.max(0, (room.participant_count || 1) - 1) }).eq('id', room_id);
  }
...
async function applyAsTutor(body, res, ctx) {
  ...
  await supabase.from('user_profiles').update({ role: 'teacher', track: level, class_name, is_approved_teacher: false, updated_at: new Date().toISOString() }).eq('user_id', ctx.userId);
  await supabase.from('tutor_applications').insert({ user_id: ctx.userId, level, class_name, subjects, qualifications: qualifications || '', experience: experience || '', status: 'pending' });
  return res.status(200).json({ success: true });
}
...
async function reviewTutorApplication(body, res, ctx) {
  ...
  const { data: app } = await supabase.from('tutor_applications').select('user_id').eq('id', application_id).maybeSingle();
  if (!app) throw new SecurityError('Application not found', 404);

  if (action === 'approve') {
    await supabase.from('tutor_applications').update({ status: 'approved', display_name: extra.display_name || null, hourly_rate: extra.hourly_rate || null }).eq('id', application_id);
    ...
    await supabase.from('tutor_applications').update({ status: 'rejected', rejection_reason: extra.rejection_reason || '' }).eq('id', application_id);
    await supabase.from('user_profiles').update({ is_approved_teacher: false, approved_track: null, approval_notes: extra.rejection_reason || 'Rejected' }).eq('user_id', app.user_id);
  } else {
...
async function adminListRooms(req, res, ctx) {
  ...
  if (level) query = query.filter('curriculum_units.curriculum_groups.level_id', 'eq', level);
  const { data } = await query;
  return res.status(200).json(data || []);
}
...
async function isApprovedTutor(userId, level) {
  const { data } = await supabase.from('user_profiles').select('is_approved_teacher, approved_track').eq('user_id', userId).maybeSingle();
  if (!data?.is_approved_teacher) return false;
  if (data.approved_track === 'ALL') return true;
  if (level && data.approved_track !== level) return false;
  return true;
}

export async function getUserCurriculumScope(userId) {
  const { data: profile } = await supabase.from('user_profiles').select('role, track, class_name, is_approved_teacher, approved_track').eq('user_id', userId).maybeSingle();
  if (!profile) return null;
  if (profile.role === 'teacher') {
    if (!profile.is_approved_teacher) return { level: null, groupName: null, showAll: false, pending: true };
    if (profile.approved_track === 'ALL') return { level: null, groupName: null, showAll: true };
    return { level: profile.approved_track || profile.track, groupName: profile.class_name, showAll: false };
  }
  return { level: profile.track, groupName: profile.class_name, showAll: false };
}
