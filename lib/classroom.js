...
async function listClassrooms(req, res) {
  ...
  // Filter out rooms from rejected tutors
  const filtered = [];
  for (const room of (data || [])) {
    if (room.tutor_id) {
      const { data: app } = await supabase
        ...
        .eq('status', 'approved')
        .maybeSingle();
      if (!app) continue; // Skip rooms from non-approved/rejected tutors
    }
    filtered.push(room);
  }
...
async function getLiveFeed(req, res) {
  ...
  // Filter out rooms from non-approved tutors
  const filtered = [];
  for (const room of (data || [])) {
    if (room.tutor_id) {
      const { data: app } = await supabase
...
async function applyAsTutor(body, res, ctx) {
  ...
  // Permanently block rejected applicants
  if (await isRejectedTutor(ctx.userId)) {
...
async function reviewTutorApplication(body, res, ctx) {
  ...
  // If rejected, end all active rooms by this tutor
  if (action === 'reject') {
    await supabase
      .from('classrooms')
      .update({ status: 'ended', ended_at: new Date().toISOString() })
