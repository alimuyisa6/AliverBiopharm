...
async function fetchUserNames(userIds) {
  ...
  const results = await Promise.all(
    userIds.map(async (uid) => {
      try {
        ...
        return { uid, name: user?.user_metadata?.full_name || 'User' };
      } catch {
  ...
  const map = {};
  results.forEach(({ uid, name }) => { map[uid] = name; });
  return map;
}

async function enrichRoomsWithTopicAndClass(rooms) {
  const unitIds = [...new Set(rooms.map(r => r.unit_id).filter(Boolean))];
  ...
  const groupIds = [...new Set((units || []).map(u => u.group_id).filter(Boolean))];
  let groupMap = {};
  if (groupIds.length) {
...
function withDuration(room) {
  if (room.status === 'live' || room.status === 'open_floor') {
    if (room.started_at) {
      room.live_duration_seconds = Math.max(
        0,
        Math.floor((Date.now() - new Date(room.started_at).getTime()) / 1000)
      );
    }
  } else if (room.status === 'upcoming' && room.scheduled_at) {
    room.starts_in_seconds = Math.max(
      0,
      Math.floor((new Date(room.scheduled_at).getTime() - Date.now()) / 1000)
    );
  }
  return room;
}
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    ...
    case 'tutor_review':         requireAdmin(ctx); return reviewTutorApplication(body, res, ctx);
...
async function getLevels(req, res) {
  ...
  if (!levels) return res.status(200).json([]);
...
async function listClassrooms(req, res, ctx) {
  ...
  if (tutorIds.length) {
    const { data: apps } = await supabase
      ...
      .eq('status', 'approved');
    (apps || []).forEach(app => {
      tutorNames[app.user_id] = app.display_name || null;
    });
    tutorIds.forEach(tid => {
      if (!tutorNames[tid]) tutorNames[tid] = null;
    });
  }

  const enriched = await enrichRoomsWithTopicAndClass(reconciled);
  return res.status(200).json(
    enriched.map(r => ({ ...r, tutor_name: tutorNames[r.tutor_id] || null }))
  );
}
...
async function getMessages(req, res) {
  ...
  return res.status(200).json(
    data.map(m => ({ ...m, sender_name: nameMap[m.user_id] || 'User' }))
  );
}
...
async function createRoom(body, res, ctx) {
  ...
  await supabase
    .from('classroom_participants')
    .insert({ room_id: data.id, user_id: ctx.userId, role: 'tutor', is_muted: false });
...
async function joinClassroom(body, res, ctx) {
  ...
  // Minor safety gate: joining puts a student into live audio/video/chat
  // with a tutor and other participants they don't control. A minor
  // without verified parental/guardian consent is blocked here, before
  // they ever become a participant — everything downstream (messaging,
  // hand-raising, being unmuted) requires an active join, so gating here
  // covers the whole live-interaction surface in one place.
  ...
  // Block-list enforcement: if either side has blocked the other, they
  // don't end up live in the same room together, no matter which of them
  // is the tutor.
  ...
  if (existing) {
    await supabase
      .from('classroom_participants')
      .update({ left_at: null, is_muted: true, hand_raised: false })
      .eq('id', existing.id);
  } else {
    await supabase
      .from('classroom_participants')
      .insert({ room_id, user_id: ctx.userId, role: 'learner', is_muted: true });
  }
...
async function shareResource(body, res, ctx) {
  ...
  if (!room_id || !file_id) throw new SecurityError('room_id and file_id required', 400);
  ...
  // Never trust a client-supplied URL for something shown to everyone in
  // the room (including minors) — resolve it server-side against a file
  // the sharer actually owns, same pattern as tutors.js's uploadVerification.
  ...
  if (action === 'approve') {
    ...
    await supabase
      ...
      .update({
        ...
        approved_track: extra.approved_track || app.level,
        approved_by: ctx.userId,
        approved_at: new Date().toISOString(),
      })
