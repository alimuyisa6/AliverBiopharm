...
function assertValidContentType(content_type) {
  if (!VALID_CONTENT_TYPES.has(content_type)) {
...
async function getContentDetail(req, res, ctx) {
  ...
  const commonSelect =
    type === 'note'
      ? '*, curriculum_units(name, curriculum_groups(name, level_id))'
      : '*';
...
async function rateContent(body, res, ctx) {
  ...
  if (
    ...
    (typeof difficulty_rating !== 'number' || difficulty_rating < 1 || difficulty_rating > 5)
  ) {
    throw new SecurityError('difficulty_rating must be a number between 1 and 5', 400);
  }
