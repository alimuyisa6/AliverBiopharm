...
import { requireAuth, requireAdmin, SecurityError } from './security-middleware.js';
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'POST') {
    ...
    switch (path) {
      ...
      case 'view': return recordView(body, res, ctx);
      default: throw new SecurityError('Invalid action', 400);
    }
  }
...
async function getContentDetail(req, res, ctx) {
  ...
  switch (type) {
    case 'note':
      if (id) content = await supabase.from('notes').select('*').eq('id', id).eq('is_active', true).maybeSingle();
      else content = await supabase.from('notes').select('*').eq('slug', slug).eq('is_active', true).maybeSingle();
      break;
    case 'article':
    ...
    case 'video':
      ...
      break;
    case 'flashcard_deck':
      if (id) content = await supabase.from('flashcard_decks').select('*').eq('id', id).eq('is_active', true).maybeSingle();
      else throw new SecurityError('slug not supported for flashcard decks', 400);
      break;
    default:
  ...
  const [engagement, reactions, userReactions] = await Promise.all([
    ...
    ctx.authenticated ? supabase.from('content_reactions').select('reaction_type').eq('content_type', type).eq('content_id', item.id.toString()).eq('user_id', ctx.userId) : Promise.resolve({ data: [] })
  ]);
...
async function getRelatedContent(req, res) {
  ...
  const { data: explicit } = await supabase
    ...
    .eq('source_reference_id', (await supabase.from('content_references').select('id').eq('content_type', type).eq('content_id', id).maybeSingle())?.data?.id)
