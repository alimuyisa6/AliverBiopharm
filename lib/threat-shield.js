...
const SCANNER_UA_PATTERNS = [
  /sqlmap/i, /nikto/i, /nmap/i, /masscan/i, /gobuster/i, /dirbuster/i, /wpscan/i,
  /acunetix/i, /nessus/i, /nuclei/i, /zgrab/i, /havij/i, /w3af/i, /dirb\//i, /feroxbuster/i
];
...
export function getReputationScore(ip) {
  const entry = ipReputation.get(ip);
  if (!entry || Date.now() - entry.windowStart > REPUTATION_WINDOW_MS) return 0;
  return entry.score;
}
...
function scanObjectDeep(obj, reasons, prefix = '', depth = 0) {
  ...
  if (Array.isArray(obj)) {
    for (let i = 0; i < obj.length && i < 50; i++) total += scanObjectDeep(obj[i], reasons, `${prefix}[${i}]`, depth + 1);
    return total;
  }
  if (typeof obj === 'object') {
    for (const key of Object.keys(obj).slice(0, 50)) total += scanObjectDeep(obj[key], reasons, prefix ? `${prefix}.${key}` : key, depth + 1);
  }
  return total;
}
...
export async function scoreRequest(req, body, ctx) {
  ...
  } else if (SCANNER_UA_PATTERNS.some(p => p.test(userAgent))) {
    score += 5;
  ...
  try { score += scanValue(decodeURIComponent(rawPath), reasons, 'path'); } catch { score += scanValue(rawPath, reasons, 'path'); }
  if (rawQuery) {
    try { score += scanValue(decodeURIComponent(rawQuery), reasons, 'query'); } catch { score += scanValue(rawQuery, reasons, 'query'); }
  }
  ...
  if (ctx && !ctx.userId && req.query && req.query.module === 'admin') {
    score += 3;
...
export async function isIpBlocked(ip) {
  ...
  let blocked = false;
  if (data) {
    if (data.is_permanent) blocked = true;
    else if (data.expires_at && new Date(data.expires_at) > new Date()) blocked = true;
  }
...
export async function recordSecurityEvent({ eventType, severity, ip, ipNetwork, userId, path, module: moduleName, method, score, reasons, userAgent, actionTaken }) {
  try {
    await supabase.from('security_events').insert({
      event_type: eventType,
      severity,
      ip_address: ip || null,
      ip_network: ipNetwork || null,
      user_id: userId || null,
      path: path || null,
    ...
    console.error('[ThreatShield] failed to record event:', e.message);
  }
}

export async function createSecurityAlert({ alertType, severity, ip, userId, summary, details }) {
  try {
    ...
    console.error('[ThreatShield] failed to create alert:', e.message);
  }
}
...
export async function evaluateAndRespond({ req, body, ctx, ip, moduleName, path }) {
  if (await isIpBlocked(ip)) {
    return { blocked: true, reason: 'ip_blocked', message: getVagueErrorMessage() };
  }

  const { score, reasons } = await scoreRequest(req, body, ctx);
  if (score <= 0) return { blocked: false, score: 0 };
  ...
  if (score >= 10 || reputationTotal >= 15) {
    ...
    if (ctx && ctx.userId) await killAllSessionsForUser(ctx.userId, 'threat_auto_block');
  ...
  if (actionTaken === 'ip_blocked_and_sessions_killed') {
    return { blocked: true, reason: 'auto_blocked', message: getVagueErrorMessage() };
  }
