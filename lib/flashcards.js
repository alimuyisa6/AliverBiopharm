 import { supabase, canAccessLevel } from './core.js';
...
export async function handler(req, res, path, ctx) {
  if (req.method === 'GET') {
    switch (path) {
      case 'list':             return listFlashcards(req, res, ctx);
      case 'decks':            return getDecks(req, res, ctx);
...
async function listFlashcards(req, res, ctx) {
  const { level, discipline, class_programme, confidence } = req.query;

  if (ctx.authenticated && level) {
...
async function getDecks(req, res, ctx) {
  const { level, discipline, class_programme, confidence } = req.query;
...
const CONFIDENCE_RANK = { Beginner: 0, Fair: 1, Good: 2, Great: 3, Expert: 4 };
...
async function getAdaptiveDecks(req, res, ctx) {
  ...
  if (!state?.selected_level || !state?.selected_discipline || !state?.selected_class) {
    return res.status(200).json([]);
  }

  const canAccess = await canAccessLevel(ctx.userId, state.selected_level);
  if (!canAccess) throw new SecurityError('You do not have access to this level', 403);
  ...
  let query = supabase
    ...
    .eq('discipline', state.selected_discipline)
    .eq('class_programme', state.selected_class);
  ...
  if (state.confidence_level && CONFIDENCE_RANK[state.confidence_level] !== undefined) {
    const targetRank = CONFIDENCE_RANK[state.confidence_level];
    decks.sort((a, b) => {
      const rankA = a.difficulty_confidence ? CONFIDENCE_RANK[a.difficulty_confidence] : null;
      const rankB = b.difficulty_confidence ? CONFIDENCE_RANK[b.difficulty_confidence] : null;
      const distA = rankA === null ? 99 : Math.abs(rankA - targetRank);
      const distB = rankB === null ? 99 : Math.abs(rankB - targetRank);
      return distA - distB;
    });
  }
