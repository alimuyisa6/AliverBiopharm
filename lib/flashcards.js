...
async function getDeck(req, res, ctx) {
  ...
  const { data: deck } = await supabase.from('flashcard_decks').select('*').eq('id', deck_id).maybeSingle();
...
async function getDecksSummary(req, res, ctx) {
  const { unit_id } = req.query;
  let query = supabase.from('flashcard_decks').select('id, title, description, category, unit_id, card_types').eq('is_active', true).order('created_at', { ascending: false });
  if (unit_id) query = query.eq('unit_id', unit_id);
  const { data } = await query;
  return res.status(200).json(data || []);
}
...
async function getAdaptiveDecks(req, res, ctx) {
  ...
  if (!scope || (!scope.showAll && !scope.level)) return res.status(200).json([]);

  let query = supabase.from('flashcard_decks').select('id, title, description, category, unit_id, card_types, difficulty_confidence').eq('is_active', true);
  if (!scope.showAll) {
    query = query.eq('level', scope.level);
    if (scope.groupName) query = query.eq('class_programme', scope.groupName);
  }
